...
public class CcddVariableHandler {
    ...
    // root
    // structures, and the overall structure sizes. The index of a value in this
    // list corresponds
    // to the index of the variable path in the structureAndVariablePaths list
    private List<Integer> structureAndVariableOffsets;

    // List that indicates if the corresponding structure and variable path is a
    // variable. False if
    // the path is for a non-root table and its children. The index of a value in
    // this list
    // corresponds to the index of the variable path in the
    // structureAndVariablePaths list
    private List<Boolean> isVariable;

    // List containing a converted variable name list and the separators used to
    // create the list.
    // Until forced to empty the list, the application stores each list if one with
    // the specified
    // separators doesn't already exist
    private List<ConversionListStorage> conversionLists;

    // List containing all of the program-formatted variable paths, and their
    // corresponding name
    // after converting any commas and brackets based on the specified separator
    // characters. The
    // index of a value in this list corresponds to the index of the variable path
    // in the
    // structureAndVariablePaths list
    ...
    private class ConversionListStorage {
         ...
         *
         * @param varPathSeparator      character(s) to place between variables path
         *                              members
         *
         * @param excludeDataTypes      true to remove the data types from the variable
         *                              path + name
         *
         * @param typeNameSeparator     character(s) to place between data types and
         *                              variable names
         *
         * @param convertedVariableName converted variable name list built using the
         ...
         *
         * @return The converted variable name list built using the associated
         *         separators
         *****************************************************************************************/
         ...
         *                          variable names
         *
         * @return true if the separators provided match those used to create the
         *         associated converted variable name list
         *****************************************************************************************/
        protected boolean isSeparatorsEqual(String varPathSeparator, boolean excludeDataTypes,
     ...
     *
     * @param dataTypeHandler reference to a data type handler; null to use the one
     *                        in the main class
     *
     * @param macroHandler    reference to a macro handler; null to use the one in
     *                        the main class
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Get the reference to the table tree of instance structure tables, including
     * the primitive variables
     *
     * @return Reference to the table tree of instance structure tables, including
     *         the primitive variables
     *********************************************************************************************/
     ...
     *
     * @return Reference to the list of structure and variable paths for valid
    ...
    /**********************************************************************************************
     * Get the regular expression for matching a sizeof() call for the specified
     * data type
     *
     ...
     *
     * @return Regular expression for matching a sizeof() call for the specified
     *         data type
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Check if the supplied text contains a sizeof() call for the specified data
     * type
     *
     * @param text       string in which to search for the sizeof() call
     *
    ...
    /**********************************************************************************************
     * Replace each instance of sizeof(data type) in the specified string with its
     * numeric value
     *
     * @param expression       text in which to replace any sizeof() calls
     *
     * @param invalidDataTypes List containing the invalid data types when
     *                         evaluating sizeof() calls; null if there are no data
     *                         type constraints for a sizeof() call
     *
     * @return Input string with each instance of sizeof(data type) replaced by its
     *         numeric value
     *********************************************************************************************/
    protected String replaceSizeofWithValue(String expression, List<String> invalidDataTypes) {
        ...
        while (expression != null && hasSizeof(expression)) {
            ...
            // Get the size of the data type in bytes and replace the sizeof() call with
            // this
            // value
     ...
     *
     * @return true if the text string in the previous replaceSizeofWithValue() call
     *         contained an invalid data type reference
     *********************************************************************************************/
     ...
     *
     * @return Size in bytes required to store the data type; returns 0 if the data
     *         type doesn't exist
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Get the byte offset of the specified variable relative to its root structure.
     * The variable's path, including data type and variable name, is used to verify
     * that the specified target has been located; i.e., not another variable with
     * the same name
     *
     * @param targetVariable a comma separated string of the root structure and each
     *                       data type and variable name of each variable in the
     *                       current search path. The bit length may be omitted for
     *                       bit-wise variables
     *
     * @return The byte offset to the target prototype structure, or variable
     *         relative to its root structure; returns -1 if the prototype structure
     *         name or root-variable path combination is invalid
     *********************************************************************************************/
    protected int getVariableOffset(String targetVariable) {
        int offset = -1;

        // Get the index into the variable path list for the specified
        // structure/variable. A
        ...
        if (index != -1) {
            ...
            // The target is a prototype/root structure
                ...
                // The offset for a prototype/root structure is always 0; the offset list value
                // for
    ...
    /**********************************************************************************************
     * Using a variable tree create three lists: (1) references to every structure
     * and variable (keeping the child structures and variables in the order in
     * which they appear relative to their root structure), (2) offsets for the
     * variables relative to their root structure, or the total structure size in
     * bytes if the path is for a root structure, and (3) flags indicating if the
     * variable is not for a non-root structure or its children. The conversion list
     * is reset, so the next request for a converted variable path triggers
    ...
    /**********************************************************************************************
     * Adjust the offset to the current variable based on the last variable's byte
     * size and any bit packing
     *
     * @param dataType  variable's data type
     *
     * @param bitLength string representing the number of bits used by variable;
    ...
    private int adjustVariableOffset(String dataType, String bitLength, int offset) {
        ...
        // Get the bit length associated with the variable; use 0 if no bit length is
        // specified
        ...
        // Check if the current or previous variable has no bit length specified, the
        // data type
     ...
     *
     * @param progDefinedPath variable path + name in the application's internal
     *                        format
     *
     * @param alternateName   variable path converted using the separator
     *                        characters, or a user-defined name
     *
    ...
    protected boolean isVariablePathInUse(String progDefinedPath, String alternateName) {
        ...
        // Check if the name was found (i.e., the name matches one manually set by the
        // user)
        if (index != -1) {
            // Check if the supplied program-formatted path doesn't correspond with of the
            // user-defined name - if the program-formatted path is the 'key' for the
            // user-defined
    ...
    /**********************************************************************************************
     * Get the variable path for the specified table path, variable name, and data
     * type
     *
     ...
     *                          variable names
     *
     * @param includeCustom     true to substitute the user-defined variable path
     *                          (if present); false to ignore the user-defined path
     *                          and use the auto-generated one based on the
     *                          conversion flags
     *
     * @return Variable path for the specified table path, variable name, and data
     *         type
     *********************************************************************************************/
     ...
     *         separated by the specified separator character(s); if a user-defined
     *         path exists then it is returned in place of the auto-generated one.
     *         Returns a blank if fullName is null or empty
     *********************************************************************************************/
     ...
     *                              variable names
     *
     * @param substituteUserDefined true to substitute the user-defined variable
     *                              name (if present); false to ignore the
     *                              user-defined name and use the auto-generated one
    ...
    protected String getFullVariableName(String fullName, String varPathSeparator, boolean excludeDataTypes,
        ...
        if (fullName != null && !fullName.isEmpty()) {
            ...
            if (conversionLists == null || convertedVariableName == null) {
                ...
                // names. The lists ensure that no duplicate is returned; instead, a unique name
                // is
     ...
     * variable names result from the conversion; if a duplicate is found append an
     * underscore to the duplicate's name. Once all variable names are processed
     * trim the list to include only those variables that are modified to prevent a
     * duplicate. These lists are used by getFullVariableName() so that it always
    ...
    private void createVariableNameList(String varPathSeparator, boolean excludeDataTypes, String typeNameSeparator) {
        ...
        if (conversionLists == null) {
            ...
            if (!varPathColumnsDb.isEmpty()) {
                ...
                // non-blank) variable paths. This accounts for root tables with user-defined
                // paths
                String[] matches = dbCommand.getList(DatabaseListCommand.SEARCH,
                        new String[][] { { "_search_text_", ".+" }, { "_case_insensitive_", "false" },
                                { "_allow_regex_", "true" }, { "_selected_tables_", SearchType.DATA.toString() },
                ...
                matches = dbCommand.getList(DatabaseListCommand.VAR_PATH,
                        new String[][] { { "_match_column_name_", varPathColumnsUser } }, ccddMain.getMainFrame());
        ...
        // Check if the number of stored conversion lists has reached the maximum
        // allowed
        if (conversionLists.size() == ModifiableSizeInfo.MAX_STORED_CONVERSIONS.getSize()
            ...
            // Remove the second converted variable name list from the conversion lists.
            // This
            // assumes that the first one likely contains the list built using the
            // separators
    ...
    /**********************************************************************************************
     * Return a unique name based on the variable path and the supplied separators.
     * Retain or remove the data types in the supplied variable path + name based on
     * the input flag, replace the commas in the (which separate each structure
     * variable in the path) with the specified separator character, replace any
     * left brackets with underscores and right brackets with blanks (in case there
     * are any array members in the path), and remove the bit length (if one is
     * present). Underscores are appended if the resulting name matches an existing
     * one
     *
     * @param fullName          variable path + name in the normal application
     *                          format
     *
     * @param varPathSeparator  character(s) to place between variables path members
     *
     ...
     *                          variable names
     *
     * @return Variable path + name with the data types retained or removed, commas
     *         replaced by the separator character(s), left brackets replaced by
     *         underscores, right brackets removed, and the bit length removed (if
     *         present). Underscores are appended if the resulting name matches an
     *         existing one so that the returned name is unique
     *********************************************************************************************/
    protected String convertVariableName(String fullName, String varPathSeparator, boolean excludeDataTypes,
        ...
        // Data types are retained
            ...
            // Replace the data type/variable name separator with marker characters. These
            // are used
            // to detect and replace the data type and variable name separator below, and
            // prevents
