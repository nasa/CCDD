...
public class CcddCSVHandler extends CcddImportSupportHandler implements CcddImportExportInterface
{
    ...
    public enum CSVTags
    {
        COLUMN_DATA("_column_data_", null), CELL_DATA("_table_cell_data_", null), NAME_TYPE("_name_type_", null),
        DESCRIPTION("_description_", null), DATA_FIELD("_data_field_", "_data_fields_"), MACRO("_macro_", "_macros_"),
        TABLE_TYPE("_table_type_", null), TABLE_TYPE_DATA_FIELD("_table_type_data_field_", "_table_type_data_fields_"),
        ...
        APP_SCHEDULER_OLD("_app_sched_", null), APP_SCHEDULER_DATA("_app_sched_data_", null),
        APP_SCHEDULER_COMMENTS("_app_sched_comments_", null), TELEM_SCHEDULER_OLD("_telem_sched_", null),
        TELEM_SCHEDULER_COMMENTS("_telem_sched_comments_", null), TELEM_SCHEDULER_DATA("_telem_sched_data_", null),
        RATE_INFO("_rate_info_", null), FILE_DESCRIPTION("File Description", "");
    ...
    private enum Chars
    {
        NEW_LINE("\n"), DOUBLE_QUOTE("\""), EMPTY_STRING(""), COMMA(","), PERIOD("."), COMMA_BETWEEN_QUOTES("\",\""),
     ...
     *
     * @return List of original and new telemetry scheduler messages; null if no new messages have been
     *         added
     *********************************************************************************************/
    @Override
                                     ...
                                     boolean replaceExistingAssociations) throws CCDDException, IOException, Exception
    {
        try
        {
            ...
            if (incomingData.length() != 0)
            {
                ...
                rateHandler.setRateParameters(maxSecPerMsg, maxMsgsPerSec, rateDataStreamNames, maximumMessagesPerCycle,
                                              maximumBytesPerSecond, includeUneven, parent);
            }
            ...
            if (incomingData.length() != 0)
            {
                ...
                for (int i = 0; i < scriptAssocDefns.length; i++)
                {
                    ...
                    // The code appears to expect 4 columns from the above operation. If there are blank
                    // entries, there may be only 3 columns Handle this by creating the fourth column
                    String fourthCol = "";
                    if (Columns.length == 4)
                    {
                        fourthCol = Columns[3];
                    ...
                    ignoreErrors = addImportedScriptAssociation(ignoreErrors, replaceExistingAssociations, associations,
                                                                new String[] {Columns[0], Columns[1], Columns[2],
                                                                              CcddScriptHandler
                                                                                      .convertAssociationMembersFormat(fourthCol,
                                                                                                                       true)},
        ...
        try
        {
            ...
            if (incomingData.length() != 0)
            {
                ...
                for (int i = 1; i < TableTypeDefns.length; i++)
                {
                    ...
                    String TableTypeName = Rows[0].split(Chars.COMMA.getValue())[0]
                            .replace(Chars.DOUBLE_QUOTE.getValue(), Chars.EMPTY_STRING.getValue());
                    String TableTypeDescription = Chars.EMPTY_STRING.getValue()
                                                  + Rows[0].split(Chars.COMMA.getValue())[1]
                                                          .replace(Chars.DOUBLE_QUOTE.getValue(),
                                                                   Chars.EMPTY_STRING.getValue());
                    ...
                    // Check that both the name, description and rather or not the table represents a command is all
                    // defined.
                            ...
                            ignoreErrors = addImportedTableTypeColumnDefinition(ignoreErrors, TableTypeDefn,
                                                                                new String[] {Integer
                                                                                        .toString(ColumnNumber),
                                                                                              Columns[TableTypeEditorColumnInfo.NAME
                                                                                                      .ordinal() - 1],
                                                                                              Columns[TableTypeEditorColumnInfo.DESCRIPTION
                                                                                                      .ordinal() - 1],
                                                                                              Columns[TableTypeEditorColumnInfo.INPUT_TYPE
                                                                                                      .ordinal() - 1],
                                                                                              Columns[TableTypeEditorColumnInfo.UNIQUE
                                                                                                      .ordinal() - 1],
                                                                                              Columns[TableTypeEditorColumnInfo.REQUIRED
                                                                                                      .ordinal() - 1],
                                                                                              Columns[TableTypeEditorColumnInfo.STRUCTURE_ALLOWED
                                                                                                      .ordinal() - 1],
                                                                                              Columns[TableTypeEditorColumnInfo.POINTER_ALLOWED
                                                                                                      .ordinal() - 1]},
                                                                                importFile.getAbsolutePath(),
                                                                                inputTypeHandler, parent);
                        ...
                        for (int x = 1; x < Fields.length; x++)
                        {
                            ...
                            ignoreErrors = addImportedDataFieldDefinition(ignoreErrors, replaceExistingTables,
                                                                          ...
                                                                          new String[] {CcddFieldHandler
                                                                                  .getFieldTypeName(TableTypeDefn
                                                                                          .getTypeName()),
                                                                                        Columns[FieldEditorColumnInfo.NAME
                                                                                                .ordinal()],
                                                                                        Columns[FieldEditorColumnInfo.DESCRIPTION
                                                                                                .ordinal()],
                                                                                        Columns[FieldEditorColumnInfo.CHAR_SIZE
                                                                                                .ordinal()],
                                                                                        Columns[FieldEditorColumnInfo.INPUT_TYPE
                                                                                                .ordinal()],
                                                                                        Columns[FieldEditorColumnInfo.REQUIRED
                                                                                                .ordinal()],
                                                                                        Columns[FieldEditorColumnInfo.APPLICABILITY
                                                                                                .ordinal()],
                                                                                        Columns[FieldEditorColumnInfo.VALUE
                                                                                                .ordinal()],
                                                                                        ""},
                                                                          importFile.getAbsolutePath(),
            ...
            if (incomingData.length() != 0)
            {
                ...
                {
                    // Convert all of the new macro definitions to a set of unique entries based on the
                    // macro name (the first array element) A pair with a boolean indicating if the
                    // input set was unique and the list of unique entries that were extracted
                    ...
                    if (isDupDetected)
                    {
                        ...
                        if (new CcddDialogHandler()
                                .showMessageDialog(ccddMain.getMainFrame(),
                                                   ...
                                                   "Duplicate Macros in Input File", JOptionPane.QUESTION_MESSAGE,
            ...
            if (incomingData.length() != 0)
            {
                ...
                for (int i = 0; i < inputTypeDefns.length; i++)
                {
                    ...
                    if (Columns.length == InputTypesColumn.values().length - 1)
                    {
                        ...
                        String[] inputTypeDefn = checkInputTypeDefinition(new String[] {Columns[InputTypesColumn.NAME
                                .ordinal()], Columns[InputTypesColumn.DESCRIPTION.ordinal()],
                                                                                        Columns[InputTypesColumn.MATCH
                                                                                                .ordinal()],
                                                                                        Columns[InputTypesColumn.ITEMS
                                                                                                .ordinal()],
                                                                                        Columns[InputTypesColumn.FORMAT
                                                                                                .ordinal()],
                                                                                        ""});
            ...
            while (line != null)
            {
                ...
                if (readingGroupData == true && !line.isEmpty())
                {
                    ...
                    if (newFieldInformation.size() > 0)
                    {
                        ...
                        fieldHandler
                                .replaceFieldInformationByOwner(CcddFieldHandler.getFieldGroupName(currentGroupName),
                                                                newFieldInformation);
                    }
                        ...
                        || columnValues.length == GroupDefinitionColumn.values().length - 1)
                    {
                        ...
                        addImportedGroupDefinition(new String[] {groupDefnName,
                                                                 columnValues[GroupDefinitionColumn.DESCRIPTION
                                                                         .ordinal()],
                                                                 columnValues[GroupDefinitionColumn.IS_APPLICATION
                                                                         .ordinal()],
                ...
                else if (readingGroupDataField == true && !line.isEmpty())
                {
                    ...
                    InputType inputType = inputTypeHandler
                            .getInputTypeByName(columnValues[FieldsColumn.FIELD_TYPE.ordinal() - 1]);
                    FieldInformation newField = new FieldInformation(CcddFieldHandler
                            .getFieldGroupName(currentGroupName), columnValues[FieldsColumn.FIELD_NAME.ordinal() - 1],
                                                                     columnValues[FieldsColumn.FIELD_DESC.ordinal()
                                                                                  - 1],
                                                                     inputType,
                                                                     Integer.parseInt(columnValues[FieldsColumn.FIELD_SIZE
                                                                             .ordinal() - 1]),
                                                                     Boolean.parseBoolean(columnValues[FieldsColumn.FIELD_REQUIRED
                                                                             .ordinal() - 1]),
                                                                     ApplicabilityType
                                                                             .getApplicabilityByName(columnValues[FieldsColumn.FIELD_APPLICABILITY
                                                                                     .ordinal() - 1]),
                                                                     columnValues[FieldsColumn.FIELD_VALUE.ordinal()
                                                                                  - 1],
                                                                     Boolean.parseBoolean(columnValues[FieldsColumn.FIELD_INHERITED
                                                                             .ordinal() - 1]),
                                                                     null, 0);
                                        ...
                                        boolean replaceExistingTables, ProjectDefinition projectDefn)
    {
        ...
        try
        {
                ...
                if (!line.isEmpty() && !line.contentEquals(CSVTags.PROJECT_DATA_FIELD.getTag()))
                {
                    ...
                    ignoreErrors = addImportedDataFieldDefinition(ignoreErrors, replaceExistingTables, projectDefn,
                                                                  new String[] {CcddFieldHandler.getFieldProjectName(),
                                                                                columnValues[FieldsColumn.FIELD_NAME
                                                                                        .ordinal() - 1],
                                                                                columnValues[FieldsColumn.FIELD_DESC
                                                                                        .ordinal() - 1],
                                                                                columnValues[FieldsColumn.FIELD_SIZE
                                                                                        .ordinal() - 1],
                                                                                columnValues[FieldsColumn.FIELD_TYPE
                                                                                        .ordinal() - 1],
                                                                                columnValues[FieldsColumn.FIELD_REQUIRED
                                                                                        .ordinal() - 1],
                                                                                columnValues[FieldsColumn.FIELD_APPLICABILITY
                                                                                        .ordinal() - 1],
                                                                                columnValues[FieldsColumn.FIELD_VALUE
                                                                                        .ordinal() - 1],
                                                                                columnValues[FieldsColumn.FIELD_INHERITED
                                                                                        .ordinal() - 1]},
            ...
            fieldHandler.replaceFieldInformationByOwner(CcddFieldHandler.getFieldProjectName(), fieldHandler
                    .getFieldInformationFromDefinitions(projectDefn.getDataFields()));
        }
        catch (
            Exception e
        )
    ...
    protected String getDBUInfo()
    {
        ...
        try
        {
            ...
            if (comment.length > 0)
            {
                ...
                String[][] usersAndAcessLevel = dbTable
                        .retrieveInformationTable(InternalTable.USERS, true, ccddMain.getMainFrame())
                ...
                {
                    usersAndAcessLevelDataToExport += usersAndAcessLevel[index][0] + Chars.COLON.getValue()
                ...
                FinalOutput += Chars.DOUBLE_QUOTE.getValue() + usersAndAcessLevelDataToExport
    ...
    public void importTableDefinitions(BufferedReader br, FileEnvVar importFile, boolean ignoreErrors,
                                       boolean replaceExistingTables, ImportType importType,
                    ...
                    else
                    {
                        ...
                        if (readingNameType == true)
                        {
                            ...
                            if (columnValues.length == 2 || columnValues.length == 3)
                            {
                                ...
                                typeDefn = importType == ImportType.IMPORT_ALL ? tableTypeHandler
                                        .getTypeDefinition(columnValues[1]) : targetTypeDefn;
            ...
            try
            {
                        ...
                        .compareToIgnoreCase(tableDefs.get(counter).getTablePath()) == 0)
                {
                    ...
                    outputData.append(Chars.NEW_LINE.getValue()).append(CSVTags.NAME_TYPE.getTag())
                            ...
                            .append(CcddUtilities
                                    .addEmbeddedQuotesAndCommas(tableDefs.get(counter).getTablePath(),
                                                                tableDefs.get(counter).getType(),
                                                                fieldHandler
                                                                        .getFieldValue(tableDefs.get(counter)
                                                                                .getTablePath(), DefaultInputType.SYSTEM_PATH)));
                    ...
                    if (!tableDefs.get(counter).getDescription().isEmpty())
                    {
                        ...
                        outputData.append(Chars.NEW_LINE.getValue()).append(CSVTags.DESCRIPTION.getTag())
                                .append(Chars.NEW_LINE.getValue())
                    ...
                    outputData.append(Chars.NEW_LINE.getValue()).append(CSVTags.COLUMN_DATA.getTag())
                            .append(Chars.NEW_LINE.getValue())
                    ...
                    {
                        // If this column represents a boolean and it is blank a 'false' will need to be added
                        ...
                        outputData.append(Chars.NEW_LINE.getValue())
                                .append(CcddUtilities.addEmbeddedQuotesAndCommas(Arrays
                                        .copyOfRange(CcddUtilities
                                                .convertObjectToString(tableDefs.get(counter).getData().get(row)),
                                                     NUM_HIDDEN_COLUMNS,
                        ...
                        for (int column = 0; column < columnNames.length; column++)
                        {
                            ...
                            if (includeVariablePaths && typeDefn.isStructure())
                            {
                                ...
                                String variablePath = tableDefs.get(counter).getTablePath() + Chars.COMMA.getValue()
                    ...
                    List<FieldInformation> fieldInformation = fieldHandler
                            .getFieldInformationByOwner(tableDefs.get(counter).getTablePath());
                    ...
                    if (!fieldInformation.isEmpty())
                    {
                        ...
                        outputData.append(Chars.NEW_LINE.getValue()).append(CSVTags.DATA_FIELD.getTag())
                                .append(Chars.NEW_LINE.getValue())
                        ...
                        for (FieldInformation fieldInfo : fieldInformation)
                        {
                            ...
                            outputData.append(Chars.NEW_LINE.getValue()).append(CcddUtilities
                                    .addEmbeddedQuotesAndCommas(fieldInfo.getFieldName(), fieldInfo.getDescription(),
                ...
                if (includeVariablePaths && !variablePaths.isEmpty())
                {
                    ...
                    outputData.append(Chars.NEW_LINE.getValue()).append(CSVTags.VARIABLE_PATHS.getTag())
                            .append(Chars.NEW_LINE.getValue());
                    ...
                    for (String[] variablePath : variablePaths)
                    {
                        ...
                        outputData.append("%s").append(Chars.NEW_LINE.getValue())
                                .append(CcddUtilities.addEmbeddedQuotesAndCommas(variablePath[0], variablePath[1]));
                    }
                }
    ...
    protected String getTlmSchedulerDataAndComments()
    {
        ...
        if (!storedData.isEmpty())
        {
            ...
            for (String[] data : storedData)
            {
                ...
                dataToReturn.append(Chars.DOUBLE_QUOTE.getValue()).append(data[TlmSchedulerColumn.RATE_NAME.ordinal()])
                ...
                dataToReturn.append(Chars.DOUBLE_QUOTE.getValue())
                        ...
                        .append(Chars.DOUBLE_QUOTE_AND_COMMA.getValue());
                dataToReturn.append(Chars.DOUBLE_QUOTE.getValue()).append(data[TlmSchedulerColumn.MESSAGE_ID.ordinal()])
                        .append(Chars.DOUBLE_QUOTE_AND_COMMA.getValue());
                dataToReturn.append(Chars.DOUBLE_QUOTE.getValue()).append(data[TlmSchedulerColumn.MEMBER.ordinal()])
                        .append(Chars.DOUBLE_QUOTE.getValue()).append(Chars.NEW_LINE.getValue());
            }
        }
        ...
        {
            // There are three parameters that will be required by each rate; add them to the output.
            dataToReturn.append(CSVTags.TELEM_SCHEDULER_COMMENTS.getTag()).append(Chars.NEW_LINE.getValue());
            dataToReturn.append(Chars.DOUBLE_QUOTE.getValue()).append(comments[0])
                    .append(Chars.DOUBLE_QUOTE_AND_COMMA.getValue());
            dataToReturn.append(Chars.DOUBLE_QUOTE.getValue()).append(comments[1])
                    .append(Chars.DOUBLE_QUOTE_AND_COMMA.getValue());
            dataToReturn.append(Chars.DOUBLE_QUOTE.getValue()).append(comments[2])
            ...
            if (comments.length > 3)
            {
                ...
                dataToReturn.append(Chars.NEW_LINE.getValue()).append(CSVTags.RATE_INFO.getTag())
                        .append(Chars.NEW_LINE.getValue());
                    ...
                    for (int rateInfo = 0; rateInfo < 4; rateInfo++)
                    {
                        ...
                        switch (rateInfo)
                        {
                            ...
                            case 1:
                                ...
                                break;
                            case 2:
                                dataToReturn.append(Chars.DOUBLE_QUOTE.getValue()).append(comments[count])
                                        .append(Chars.DOUBLE_QUOTE_AND_COMMA.getValue());
                                count++;
                                break;
                            case 3:
    ...
    protected String getAppSchedulerDataAndComments()
    {
        ...
        if (!storedData.isEmpty())
        {
            ...
            {
                // Add the table column values to the output
                dataToReturn.append(Chars.DOUBLE_QUOTE.getValue()).append(data[AppSchedulerColumn.TIME_SLOT.ordinal()])
                        .append(Chars.DOUBLE_QUOTE_AND_COMMA.getValue());
                dataToReturn.append(Chars.DOUBLE_QUOTE.getValue()).append(data[AppSchedulerColumn.APP_INFO.ordinal()])
                        .append(Chars.DOUBLE_QUOTE.getValue()).append(Chars.NEW_LINE.getValue());
            }
        }
        ...
        if (!(comments.length == 0))
        {
            ...
            for (int index = 0; index < comments.length; index++)
            {
                ...
                {
                    dataToReturn.append(Chars.DOUBLE_QUOTE.getValue()).append(comments[index])
                            .append(Chars.DOUBLE_QUOTE_AND_COMMA.getValue());
                }
                else
                {
                        ...
                        case GROUPS:
                            ...
                            if (groupInfoList != null && groupInfoList.size() > 0)
                            {
                                ...
                                for (GroupInformation groupInfo : groupInfoList)
                                {
                                    ...
                                    // Append the group name, description and rather or not it represents an application
                                    groupData.append("\"").append(groupInfo.getName()).append("\",\"")
                                            .append(groupInfo.getDescription()).append("\",\"")
                                            .append(groupInfo.isApplication()).append("\"");
                                    ...
                                    if (tableMembers != null && tableMembers.size() > 0)
                                    {
                                        ...
                                        {
                                            // If this is the first member add a comma rather than a semi-colon
                                            if (firstMember)
                                            {
                                    ...
                                    List<FieldInformation> groupFieldInfo = fieldHandler
                                            .getFieldInformationByOwner("Group:" + groupInfo.getName());
                                    ...
                                    if (groupFieldInfo != null && groupFieldInfo.size() > 0)
                                    {
                                        ...
                                        for (FieldInformation fieldInfo : groupFieldInfo)
                                        {
                                            ...
                                            groupData.append("\"").append(fieldInfo.getFieldName()).append("\",\"")
                                                    .append(fieldInfo.getDescription()).append("\",\"")
                                                    .append(fieldInfo.getSize()).append("\",\"")
                                                    .append(fieldInfo.getInputType().getInputName()).append("\",\"")
                                                    .append(fieldInfo.isRequired()).append("\",\"")
                                                    .append(fieldInfo.getApplicabilityType().getApplicabilityName())
                                                    .append("\",\"").append(fieldInfo.getValue()).append("\"\n");
                                        }
                                    }
                                    else
                                    {
                            ...
                            if (!referencedMacros.isEmpty())
                            {
                                ...
                                for (String[] macro : macroHandler.getMacroData())
                                {
                                    ...
                                    if (referencedMacros.contains(macro[MacrosColumn.MACRO_NAME.ordinal()]))
                                    {
                                        ...
                                        finalOutput.append(CcddUtilities
                                                .addEmbeddedQuotesAndCommas(macro[MacrosColumn.MACRO_NAME.ordinal()],
                                                                            macro[MacrosColumn.VALUE.ordinal()]));
                                        finalOutput = new StringBuilder((finalOutput.toString().replace("\"\"",
                                                                                                        "\"\n\"")));
                                        finalOutput.append("\n");
                                    }
                                }
                            ...
                            if (scriptAssociations != null && scriptAssociations.size() > 0)
                            {
                                ...
                                for (String[] association : scriptAssociations)
                                {
                                    String member = association[AssociationsColumn.MEMBERS.ordinal()].replace(";\n",
                                                                                                              " + ");
                                    associationData.append("\"").append(association[AssociationsColumn.NAME.ordinal()])
                                            .append("\",").append("\"")
                                            .append(association[AssociationsColumn.DESCRIPTION.ordinal()]).append("\",")
                                            .append("\"").append(association[AssociationsColumn.SCRIPT_FILE.ordinal()])
                        ...
                        case RESERVED_MSG_ID:
                            ...
                            if (!rsvMsgIDHandler.getReservedMsgIDData().isEmpty())
                            {
                                ...
                                for (String[] reservedMsgID : rsvMsgIDHandler.getReservedMsgIDData())
                                {
                                    ...
                                    finalOutput.append(CcddUtilities
                                            .addEmbeddedQuotesAndCommas(reservedMsgID[ReservedMsgIDsColumn.MSG_ID
                                                    .ordinal()], reservedMsgID[ReservedMsgIDsColumn.DESCRIPTION.ordinal()]))
                                            .append("\n");
                                }
                            }
                            break;
                        ...
                        case PROJECT_FIELDS:
                            ...
                            if (!fieldInformation.isEmpty())
                            {
                                ...
                                for (FieldInformation fieldInfo : fieldInformation)
                                {
                                    // Output the field information
                                    finalOutput.append(CcddUtilities
                                            .addEmbeddedQuotesAndCommas(fieldInfo.getFieldName(),
        ...
        try
        {
            ...
            if (!referencedInputTypes.isEmpty())
            {
                ...
                for (String inputTypeName : referencedInputTypes)
                {
                    ...
                    pw.printf("%s\n", CcddUtilities
                            .addEmbeddedQuotesAndCommas(inputType.getInputName(), inputType.getInputDescription(),
            ...
            for (String tableType : referencedTableTypes)
            {
                ...
                for (int column = NUM_HIDDEN_COLUMNS; column < tableTypeDefn.getColumnCountDatabase(); column++)
                {
                    ...
                    pw.printf("%s\n",
                              CcddUtilities
                                      .addEmbeddedQuotesAndCommas(tableTypeDefn.getColumnNamesUser()[column],
            ...
            if (!referencedDataTypes.isEmpty())
            {
                ...
                for (String[] dataType : dataTypeHandler.getDataTypeData())
                {
                    ...
                    if (referencedDataTypes.contains(CcddDataTypeHandler.getDataTypeName(dataType)))
                    {
                        ...
                        pw.printf("%s\n",
                                  CcddUtilities
                                          .addEmbeddedQuotesAndCommas(dataType[DataTypesColumn.USER_NAME.ordinal()],
    ...
    public StringBuilder retrieveCSVData(String searchKey, StringBuilder data)
    {
        ...
        else if (searchKey == CSVTags.TELEM_SCHEDULER_COMMENTS.getTag())
        {
            ...
            beginIndex = data.toString().indexOf(CSVTags.TELEM_SCHEDULER_COMMENTS.getTag());
            if (beginIndex == -1)
            {
                beginIndex = data.toString().indexOf(CSVTags.TELEM_SCHEDULER_OLD.getTag());
            }
            endIndex = data.toString().indexOf("\n\n", beginIndex);
        ...
        else if (searchKey == CSVTags.APP_SCHEDULER_COMMENTS.getTag())
        {
            // Extract the data related to the application scheduler
            beginIndex = data.toString().indexOf(CSVTags.APP_SCHEDULER_COMMENTS.getTag());
            if (beginIndex == -1)
            {
                beginIndex = data.toString().indexOf(CSVTags.APP_SCHEDULER_OLD.getTag());
            }
            endIndex = data.toString().indexOf("\n\n", beginIndex);
