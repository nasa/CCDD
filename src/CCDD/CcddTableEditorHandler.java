...
public class CcddTableEditorHandler extends CcddInputFieldPanelHandler {
    ...
    // GUI component over which to center any error dialog. This is the editor
    // dialog to which the
    ...
    // enumeration(s), and
    // rate(s) (in model coordinates). These are only set for structure tables
    // representing
    // structures
    ...
    // List containing the structures that can't be referenced by this table (if it
    // is a
    // structure). This includes the structure itself and all of its ancestor
    // structures
    ...
    protected void updateDataTypeReferences(String oldPrototype, String newPrototype) {
        ...
        for (int row = 0; row < tableModel.getRowCount(); row++) {
            ...
            for (Integer column : varRefColumns) {
                ...
                // Check if the variable reference contains a reference to the old prototype
                // name
    ...
    protected String getTableToolTip() {
        ...
        // Create the indentation text; hide this text by coloring it the same as the
        // background
    ...
    protected void updateVariablePaths() {
        ...
        // the table
        // represents a structure, the table has been created, and the table is open in
        // a table
    ...
    private void getSpecialColumnIndices() {
        ...
        // structure table,
        // but can appear in other table types (if the column is added to a structure
        // type and then
    ...
    protected void updateForTableTypeChange(TableInformation tblInfo) {
        // Update the table type name (in case it changed) in the current table
        // information
    ...
    protected void doTableUpdatesComplete(TableInformation dbTableInfo, boolean applyToInstance) {
        ...
        // Store the table information that represents the currently committed table
        // data
        ...
        if (applyToInstance) {
            ...
            // Check if the data fields don't match those stored in the database. This can
            // occur,
            // for example, if the table has a reference input type (variable. command, or
            // message)
    ...
    private void initialize() {
        ...
        table = new CcddJTableHandler(ModifiableSizeInfo.INIT_VIEWABLE_DATA_TABLE_ROWS.getSize()) {
            ...
            public boolean isCellEditable(int row, int column) {
                ...
                // Check if editing is enabled, the table is displayable (to prevent corruption
                // of
                // the cell editor) or that the table isn't open in an editor (as when a macro
                // change is processed), if the table model exists, and if the table has at
                // least
                // one row
            ...
            protected boolean isDataAlterable(Object[] rowData, int row, int column) {
                ...
                if (rowData != null && rowData.length != 0) {
                    ...
                    (((column == variableNameIndex || column == dataTypeIndex || column == arraySizeIndex
                            || column == bitLengthIndex) && !currentTableInfo.isPrototype())
                    ...
                    // isn't valid for structures
                            || (dataTypeIndex != -1 && !dataTypeHandler.isPrimitive(rowCopy[dataTypeIndex].toString())
                            ...
                            for (int otherRow = 0; otherRow < tableData.size(); otherRow++) {
                                ...
                                if (otherRow != row && newValueS
                                    ...
                                    throw new CCDDException("Invalid input value in table '</b>"
                                            ...
                                            + typeDefn.getColumnNamesUser()[column] + "<b>'; value must be unique");
                                }
                            }
                        }
            ...
            /**************************************************************************************
             * Adjust the starting row index to the next row during a paste (insert)
             * operation. If the insertion point falls within an array, skip to the row
             * immediately following the array's members
             *
             * @param startRow starting row index in view coordinates
             *
             * @return Starting row index, in model coordinates, at which to insert a new
             *         row
             *************************************************************************************/
            protected int adjustPasteStartRow(int startRow) {
                ...
                if (startRow < tableModel.getRowCount() && isCanHaveArrays()
                    ...
                    while (startRow < tableModel.getRowCount()
                            && ArrayVariable.isArrayMember(getExpandedValueAt(startRow, variableNameIndex))) {
            ...
            /**************************************************************************************
             * Determine if a row insertion is required during a paste operation. Array
             * member rows are inserted automatically when an array is defined, so if an
             * array member is being inserted no row needs to be inserted by the paste
             * operation
             *
             * @param index       current index into the cell data array
             *
             * @param cellData    array containing the cell data being inserted
             *
             * @param startColumn data insertion starting column index
             *
             * @param endColumn   data insertion ending column index
             *
            ...
            protected boolean pasteData(Object[] cellData, int numColumns, boolean isInsert, boolean isAddIfNeeded,
                ...
                // Initialize the starting row to the first row, which is the default if no row
                // is
                // selected
                int startRow = 0;
                ...
                // column is selected then default to the first column. Data pasted outside of
                // the
                ...
                // Counters for the number of array member rows added (due to pasting in an
                // array
                // definition) and the number of rows ignored (due to the first pasted row(s)
                // being
                // an array member)
                ...
                for (int index = 0, row = startRow; row < endRow && showMessage != null; row++) {
                    ...
                    if (variableNameView != -1) {
                        ...
                        // is an array member
                        else if (cellData[varIndex] != null && ArrayVariable.isArrayMember(cellData[varIndex])) {
                            ...
                            // No rows were added for this array member
                                ...
                                if (ArrayVariable.isArrayMember(varName) && (adjustedRow == 0 || !ArrayVariable
                                        .removeArrayIndex(tableData.get(adjustedRow - 1)[variableNameIndex].toString())
                                    ...
                                    if (arraySizeView != -1
                                            ...
                                            && ArrayVariable.getNumMembersFromArraySize(
                                                    ArrayVariable.getVariableArrayIndex(varName)) == 0) {
                                        ...
                                        arrayDefn[index + variableNameView - startColumn] = ArrayVariable
                                                .removeArrayIndex(varName);
                                        ...
                                        for (int defnCol = startColumn; defnCol <= endColumn; defnCol++) {
                                            ...
                                            if (!DefaultColumn.isTypeRequiredColumn(TYPE_STRUCTURE, inputTypeHandler,
                                        ...
                                        // pasted
                                        cellData = CcddUtilities.concatenateArrays(
                                                CcddUtilities.concatenateArrays(Arrays.copyOfRange(cellData, 0, index),
                                                        arrayDefn),
                    ...
                    if (!skipRow) {
                        ...
                        if ((isInsert || (isAddIfNeeded && adjustedRow == tableData.size()))
                            ...
                            for (int column = startColumn; column <= endColumn && showMessage != null; column++) {
                                ...
                                if (column < getColumnCount()) {
                                    ...
                                    Object newValue = index < cellData.length
                                            ? (cellData[index] != null
                                                    ? cleanUpCellValue(cellData[index], adjustedRow, columnModel)
                                                    : (isInsert ? "" : null))
                                            : "";
                                    ...
                                    if (newValue != null
                                            ...
                                            && isDataAlterable(tableData.get(adjustedRow), adjustedRow, columnModel)) {
                                        ...
                                        if (!oldValue.equals(newValue)
                                            ...
                                            showMessage = validateCellContent(tableData, adjustedRow, columnModel,
                                                    oldValue, newValue, showMessage, cellData.length > 1);
        ...
        // Get the variable path separators and create the variable path column content,
        // if present
        updateVariablePaths();
    ...
    protected void expandMacros(boolean isExpand, boolean isVisibleOnly) {
        ...
        // Only expand the visible cells
            ...
            // has no
            // rows then set to 0 (the first table row), otherwise set to the first visible
            // row
        ...
        if (isExpand) {
            // Create storage for the original cell values. Note the first two columns are
            // unused
    ...
    private void setArrayExpansionListener() {
        ...
        if (isCanHaveArrays()) {
            // Add a mouse listener to the table to handle mouse clicks on the data type
            // column
    ...
    private boolean isCellValueFound(int row, int column) {
        ...
        // required
            ...
            for (int index = 0; index < comboBox.getItemCount() && !found; index++) {
                // Check if the cell matches the combo box item. When making the comparison for
                // a
    ...
    protected void setUpDataTypeColumns(String[] allStructTbls, CcddTableTreeHandler tblTree) {
        ...
        // Set to null to force the enumeration cell editor and associated combo box to
        // be rebuilt
    ...
    protected void adjustArrayMember(List<Object[]> tableData, int[] arraySizeOld, int[] arraySizeNew,
        ...
        // Initialize the total original and updated array size values to 1 so that the
        // size
        ...
        // Create storage for a copy of the original array values. This copy is used to
        // restore
        ...
        // Copy the array definition row and blank the primary key and row indices to
        // prevent
        ...
        for (int tableRow = definitionRow + 1; tableRow <= lastRow; tableRow++) {
            ...
            // Append the array index value(s) to the variable name for display as the
            // variable
            // name in the array member row
            tableData.get(tableRow)[variableNameIndex] += ArrayVariable.formatArrayIndex(counter);

            // Check if the number of array dimensions is unchanged. For this case the
            // existing
            // values for the array members are restored for those array members that still
            // exist
        ...
        // Check if the number of array dimensions is unchanged and that this is a
        // string array
        if (arraySizeOld.length == arraySizeNew.length
    ...
    private void propagateArrayValues(List<Object[]> tableData, int firstRow, int columnChanged) {
        ...
        if (!typeDefn.isRowValueUnique()[columnChanged]) {
            ...
            // Set to true if the updated row is the array's definition. Set to false to
            // indicate
            // that only the members of the string indicated by the specified row are to be
            // updated
            ...
            if (!isArrayDefn || editorDialog == null || !editorDialog.isArrayOverwriteNone()) {
                ...
                // of
                // strings is having its value changed then include the array index portions
                // other
    ...
    protected boolean isTableChanged() {
        ...
        if (!isEditEnabled) {
            // Display the macros in place of the macro values. If the macros aren't
            // restored then
        ...
        // Check if the table has changes, ignoring the variable path column (if
        // present; this
        // column must be checked for changes separately due to how the values are
        // maintained)
        ...
        if (!isChanged && variablePathIndex != -1 && variableNameIndex != -1 && dataTypeIndex != -1) {
            ...
            for (int row = 0; row < tableModel.getRowCount() && !isChanged; row++) {
                ...
                // Check if the row has a primary key (indicates it exists in the committed
                // data)
