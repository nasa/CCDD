...
public class CcddMacroEditorDialog extends CcddDialogHandler {
    ...
    /**********************************************************************************************
     * Create the macro editor dialog. This is executed in a separate thread since
                                ...
                                if (!tablePaths.isEmpty()) {
                                    ...
                                    new CcddDialogHandler().showMessageDialog(CcddMacroEditorDialog.this,
                                            "<html><b>Cannot delete macro '</b>" + macroName
                                                    ...
                                                    + "<b>'",
                                            "Delete Macro", JOptionPane.ERROR_MESSAGE, DialogOption.OK_OPTION);
                                }
                            }
                        }
    ...
    private JScrollPane createMacroTable() {
        ...
        macroTable = new CcddJTableHandler() {
                        ...
                        // Check if the macro value changed
                            ...
                            if (!macroName.isEmpty() && newMacroHandler.isMacroRecursive(macroName)) {
                                throw new CCDDException(
                                        "Macro '</b>" + macroName + "<b>' contains a recursive reference");
                            }
                            ...
                            for (int commRow = 0; commRow < committedData.length; commRow++) {
                                ...
                                if (index.equals(committedData[commRow][MacroEditorColumnInfo.OID.ordinal()])) {
                                    ...
                                    macroHandler.validateMacroUsage(
                                            committedData[commRow][MacroEditorColumnInfo.NAME.ordinal()],
            ...
            /**************************************************************************************
             * Load the table macro definition values into the table and format the table
             * cells
             *************************************************************************************/
            @Override
     ...
     * macro names (values)
     *
     * @param isExpand true to replace the macro names with the corresponding macro
    ...
    protected void expandMacros(boolean isExpand) {
        ...
        for (int row = 0; row < tableModel.getRowCount(); row++) {
            ...
            tableModel
                    .setValueAt(
                            (isExpand
                                    ? macroHandler.getMacroExpansion(
                ...
                for (int column = 0; column < macroTable.getColumnCount() && !stopCheck; column++) {
                    ...
                    if (MacroEditorColumnInfo.values()[column].isRequired()
                        ...
                        if (new CcddDialogHandler().showMessageDialog(CcddMacroEditorDialog.this,
                                "<html><b>Data must be provided for column '</b>" + macroTable.getColumnName(column)
                                        + "<b>' [row </b>" + (row + 1) + "<b>]",
    ...
    /**********************************************************************************************
     * Compare the current macro table data to the committed data and create lists
