/**
 * CFS Command & Data Dictionary reserved message ID handler. Copyright 2017
...
/******************************************************************************
 * CFS Command & Data Dictionary reserved message ID handler class
 *****************************************************************************/
public class CcddReservedMsgIDHandler
{
    ...
    protected boolean isWithinRange(int[] idA, String otherID)
    {
        ...
        return ((
                ...
                // Check is the new ID is a range and the other is a single
                // value, and if the other ID falls within the new range
                || (idA[1] != -1
                    && idB[1] == -1
                    && idB[0] >= idA[0]
                    && idB[0] <= idA[1])

                // Check if both the new and other are ranges, and if the
                // ranges overlap
                || idA[1] != -1
                   && idB[1] != -1
    ...
    protected boolean isReservedMsgIDExists(String msgID)
    {
        ...
        for (String[] reservedMsgID : reservedMsgIDData)
        {
            ...
            {
                // Set the flag that indicates the supplied ID matches or falls
                // within the range of an existing reserved message ID, and
                // stop searching
                isExists = true;
                break;
            }
