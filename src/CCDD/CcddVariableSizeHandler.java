...
public class CcddVariableSizeHandler
{
    ...
    /**********************************************************************************************
     * Using a variable tree create two lists: one that contains a reference to every structure and
     * variable (keeping the child structures and variables in the order in which they appear
     * relative to their root structure), and another list that has the offset for the variable
     * relative to its root structure. The total structure size in bytes is stored in place of the
    ...
    protected void buildPathAndOffsetLists()
    {
        ...
        // position, variable offsets, and structure sizes
        ...
        for (Enumeration<?> element = allVariableTree.getRootNode().preorderEnumeration(); element.hasMoreElements();)
        {
            ...
            if (nodePath.length > allVariableTree.getHeaderNodeLevel())
            {
                ...
                // Check the list for this variable path. Due to the construction of the table tree
                // a prototype structure reference can occur twice
                int index = structureAndVariablePaths.indexOf(varPath);

                // Check if the variable path (prototype table) is already in the list
                if (index != -1)
                {
                    // The first listing is the prototype table only (no variables); the second
                    // includes the variables and is the one required. Remove the existing
                    // reference from the list and update the index pointer to the structure to
