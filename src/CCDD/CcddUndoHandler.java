...
public class CcddUndoHandler {
    ...
    // Flag that indicates if an edit sequence should be ended when a new value is
    // entered. This is
     ...
     * contents of a data field that is subsequently deleted, then restored via an
     * undo, to retain the undo capability for the content changes
     *
     * @param fieldHandler reference to the data field handler; null to not retain
     *                     data field content changes following deletion and
    ...
    /**********************************************************************************************
     * Set the reference to the table for which node selection changes are to be
     * tracked
     *
     * @param table reference to the table for which node selection changes are to
     *              be tracked
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Set the reference to the table cell selection container for which selection
     * changes are to be tracked
     *
     * @param selectedCells reference to the cell selection container for which
     ...
     * tracked
     *
     * @param tree reference to the tree for which node selection changes are to be
     *             tracked
     *********************************************************************************************/
     ...
     *
     * @param allow true to allow edits to be stored for possible undo action; false
     *              to perform the edit, but without storing it
     *********************************************************************************************/
     ...
     *
     * @return true if edits are stored for possible undo action; false to if edits
     *         are performed without storing it
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Set the flag that automatically ends an edit sequence. This flag is used by
     * the group manager, link manager, and the data field editor panel so that
     * multiple group/link/field changes are stored as a single edit rather than a
     * number of individual edits; if the edit is undone then the changes are
     * restored in a single operation. This flag is not used in the undoable table
     * model and table column model classes
     *
     * @param autoEndEdit true to automatically end an edit sequence when a new edit
     *                    operation occurs
     *********************************************************************************************/
     ...
     * automatically ended. This flag is not used in the undoable table model and
     * table column model classes
     *
     * @return true if the edit sequence is ended automatically; false if automatic
     *         ending is suspended
     *********************************************************************************************/
    ...
    protected class UndoableArrayList<E> extends ArrayList<E> {
        /******************************************************************************************
         * Override the default method with a method that includes a flag to store the
         * list item inserted in the undo stack
         *****************************************************************************************/
        @Override
        ...
        protected void add(int listIndex, E listItem, boolean undoable) {
            ...
            if (isAllowUndo && undoable) {
                // Create the edit event
                new UndoableEditEvent(this,
                        new ListEdit<E>(UndoableArrayList.this, listIndex, listItem, null, ListEditType.ADD_INDEX));
        ...
        /******************************************************************************************
         * Override the default method with a method that includes a flag to store the
         * list items inserted in the undo stack
         *****************************************************************************************/
        @Override
        ...
        /******************************************************************************************
         * Override the default method with a method that includes a flag to store the
         * list item removed in the undo stack
         *****************************************************************************************/
        @Override
        ...
        protected boolean remove(Object listItem, boolean undoable) {
            ...
            if (isAllowUndo && undoable) {
                // Create the edit event
                new UndoableEditEvent(this,
                        new ListEdit<E>(UndoableArrayList.this, -1, (E) listItem, null, ListEditType.REMOVE));
        ...
        protected E remove(int index, boolean undoable) {
            ...
            if (isAllowUndo && undoable) {
                // Create the edit event
                new UndoableEditEvent(this,
                        new ListEdit<E>(UndoableArrayList.this, index, get(index), null, ListEditType.REMOVE_INDEX));
        ...
        /******************************************************************************************
         * Override the default method with a method that includes a flag to store the
         * list items removed in the undo stack
         *****************************************************************************************/
        @Override
    ...
    protected class ListEdit<T> extends AbstractUndoableEdit {
         ...
         *
         * @param listIndex         index at which to add or remove the item (ignored if
         *                          not an INDEX operation)
         *
         * @param listItem          item to add or remove from the list (ignored if not
         *                          an item add or remove operation in which case null
         *                          can be used)
         *
         * @param listItems         list of items to add or remove (ignored if not a
         *                          list item add or remove operation in which case null
         *                          can be used)
         *
         * @param type              ListEditType.ADD if if adding an item to the list;
         *                          ListEditType.ADD_INDEX if if adding an item to the
         *                          list at a specific index; ListEditType.ADD_ALL if if
         *                          adding a list of items to the list;
         *                          ListEditType.REMOVE if removing an item from the
         *                          list; ListEditType.REMOVE_INDEX if removing the item
         *                          at the specified index from the list;
         *                          ListEditType.CLEAR if removing all items from the
         *                          list
         *****************************************************************************************/
    ...
    protected class UndoableCheckBox extends JCheckBox {
                ...
                if (isAllowUndo && undoable && oldValue != isSelected()) {
                    ...
                    if (listeners.length != 0) {
                        ...
                        UndoableEditEvent editEvent = new UndoableEditEvent(this,
                                new CheckBoxEdit(UndoableCheckBox.this, oldValue, isSelected(), fieldOwner, fieldName));
        ...
        /******************************************************************************************
         * Set the data field to which the check box belongs (only used if the check box
         * is a data field)
         *
        ...
        /******************************************************************************************
         * Add a the undo manager to the edit listener list, set the check box model to
         * be undoable, and add a focus change listener to the check box
         *
         * @param select initial selection state of the check box
         *****************************************************************************************/
    ...
    private class CheckBoxEdit extends AbstractUndoableEdit {
         ...
         *
         * @param fieldOwner owner of the data field to which the check box belongs;
         *                   null if the check box is not a data field
         *
         * @param fieldName  name of the data field to which the check box belongs; null
         *                   if the check box is not a data field
         *****************************************************************************************/
        CheckBoxEdit(UndoableCheckBox checkBox, boolean oldValue, boolean newValue, String fieldOwner,
                String fieldName) {
        ...
        /******************************************************************************************
         * Set the selected check box. In order for this to select the check box it must
         * be scheduled to execute after other pending events
         *
         * @param selected check box selection state
         *****************************************************************************************/
        private void setSelectedCheckBox(boolean selected) {
            // Check if a data field handler exists and if the check box's owner and name
            // have been
    ...
    /**********************************************************************************************
     * Combo box value undo/redo class. This is based on the custom padded combo box
     * so that the undoable combo box matches the appearance of other combo boxes in
     * the application
     *********************************************************************************************/
    @SuppressWarnings("serial")
    protected class UndoableComboBox extends PaddedComboBox {
        ...
        /******************************************************************************************
         * Set the data field to which the combo box belongs (only used if the combo box
         * is a data field)
         *
        ...
        /******************************************************************************************
         * Add a the undo manager to the edit listener list, set the combo box model to
         * be undoable, and add a focus change listener to the combo box
         *****************************************************************************************/
        ...
        protected void setSelectedItem(Object selection, boolean undoable) {
            ...
            if (isAllowUndo && undoable && !selection.equals(oldValue)) {
                ...
                if (listeners.length != 0) {
                    // Create the edit event to be passed to the listeners
                    UndoableEditEvent editEvent = new UndoableEditEvent(this,
                            new ComboBoxEdit(UndoableComboBox.this, oldValue, selection, fieldOwner, fieldName));
    ...
    private class ComboBoxEdit extends AbstractUndoableEdit {
         ...
         *
         * @param fieldOwner owner of the data field to which the combo box belongs;
         *                   null if the combo box is not a data field
         *
         * @param fieldName  name of the data field to which the combo box belongs; null
         *                   if the combo box is not a data field
         *****************************************************************************************/
        ...
        private void setSelectedComboBox(Object selection) {
            // Check if a data field handler exists and if the combo box's owner and name
            // have been
            // set (the owner and name are set when the data field is created)
            if (fieldHandler != null && fieldOwner != null && fieldName != null) {
                // Get the reference to the data field based on the owner and name
    ...
    protected class UndoableTextField extends JTextField {
        ...
        /******************************************************************************************
         * Add the undo manager to the edit listeners and add a focus change listener to
         * the text field
         *
         * @param text text to display in the text field
         *****************************************************************************************/
        private void setListeners(String text) {
            ...
            addKeyListener(new KeyAdapter() {
                /**********************************************************************************
                 * Handle a key press event. This allows the text field's owner to be informed
                 * of a change in text, such as to update the text field container's change
                 * indicator
                 *********************************************************************************/
                @Override
        ...
        /******************************************************************************************
         * Set the data field to which the text field belongs (only used if the text
        ...
        protected void setText(String text, boolean undoable) {
            ...
            if (isAllowUndo && undoable && !text.equals(oldValue)) {
                ...
                if (listeners.length != 0) {
                    // Create the edit event to be passed to the listeners
                    UndoableEditEvent editEvent = new UndoableEditEvent(this,
                            new TextFieldEdit(UndoableTextField.this, oldValue, text, fieldOwner, fieldName));
    ...
    /**********************************************************************************************
     * Text field edit event handler class. If the data field handler is set then
     * the undo/redo operation compares the text field's name to the data field
     * owner and field names to determine if these match, and if so the text field
     * specified in the data field information is the one on which the undo/redo
    ...
    private class TextFieldEdit extends AbstractUndoableEdit {
         ...
         *
         * @param fieldOwner owner of the data field to which the text field belongs;
         *                   null if the text field is not a data field
         *
         * @param fieldName  name of the data field to which the text field belongs;
         *                   null if the text field is not a data field
         *****************************************************************************************/
        ...
        /******************************************************************************************
         * Set the selected text field. In order for this to select the text field it
         * must be scheduled to execute after other pending events
         *
         * @param value text to place in the text field
         *****************************************************************************************/
        private void setSelectedTextField(final String value) {
            // Check if a data field handler exists and if the text field's owner and name
            // have
    ...
    protected class UndoableTextArea extends JTextArea {
        ...
        /******************************************************************************************
         * Add the undo manager to the edit listener list and add a focus change
         * listener to the text area
         *
         * @param text text to display in the text area
         *****************************************************************************************/
        private void setListeners(String text) {
            ...
            // Initialize the flag that allows undo/redo operations on this text area to be
            // stored.
            ...
            addKeyListener(new KeyAdapter() {
                /**********************************************************************************
                 * Handle a key press event. This allows the text field's owner to be informed
                 * of a change in text, such as to update the text area container's change
                 * indicator
                 *********************************************************************************/
                @Override
                public void keyPressed(final KeyEvent ke) {
        ...
        /******************************************************************************************
         * Flag the text as being allowed/prevented being stored as an update on the
         * undo/redo stack. The other criteria (the allow undo flag is set and the text
         * has changed) must be met in order for the edit to be undoable
         *
        ...
        /******************************************************************************************
         * Check if an edit to the text area should be stored on the undo/redo stack,
         * and if so add it to the stack
         *****************************************************************************************/
        private void setText() {
            ...
            if (isAllowUndo && undoable && !oldValue.equals(newValue)) {
                ...
                if (listeners.length != 0) {
                    // Create the edit event to be passed to the listeners
                    UndoableEditEvent editEvent = new UndoableEditEvent(this,
                            new TextAreaEdit(UndoableTextArea.this, oldValue, newValue));
            ...
            // Reset the flag so that any further edits must be preceded by another focus
            // gained
    ...
    private void setComponentFocus(JComponent comp) {
        ...
        if (comp.isVisible()) {
            // Disable input verification since this sets the component value, creating an
            // edit
            // event
    ...
    protected class UndoableTableModel extends DefaultTableModel {
        ...
        /******************************************************************************************
         * Placeholder for performing any clean-up of the cell value prior to setting
         * the cell contents. This can include, for example, removal of leading and
        ...
        protected void setValueAt(Object value, int row, int column, boolean undoable) {
            ...
            // Check if the cell value has changed. Not processing duplicate updates
            // prevents the
            ...
            if (!oldValue.equals(value)) {
                ...
                if (isAllowUndo && undoable) {
                    ...
                    if (listeners.length != 0) {
                        // Create the edit event to be passed to the listeners
                        UndoableEditEvent editEvent = new UndoableEditEvent(this,
                                new CellEdit(this, oldValue, value, row, column));
        ...
        /******************************************************************************************
         * Override the default method with a method that includes a flag to store the
         * table data change in the undo stack
         *****************************************************************************************/
        @Override
        ...
        protected void setDataVector(Object[][] dataVector, Object[] columnIdentifiers, boolean undoable) {
            ...
            if (isAllowUndo && undoable) {
                ...
                if (listeners.length != 0) {
                    // Create the edit event to be passed to the listeners
                    UndoableEditEvent editEvent = new UndoableEditEvent(this,
                            new DataVectorEdit(table.getTableData(false), dataVector));
                    // Step through the registered listeners
                    for (UndoableEditListener listener : listeners) {
                        // Inform the listener that an update occurred
                        listener.undoableEditHappened(editEvent);
        ...
        /******************************************************************************************
         * Override the default method with a method that includes a flag to store the
         * row inserted in the undo stack
         *****************************************************************************************/
        @Override
         ...
         *
         * @param values   array containing the column information for the row to insert
         *                 into the table
         *
        ...
        protected void insertRow(int row, Object values[], boolean undoable) {
            ...
            if (isAllowUndo && undoable) {
                ...
                if (listeners.length != 0) {
                    // Create the edit event to be passed to the listeners
                    UndoableEditEvent editEvent = new UndoableEditEvent(this,
                            new RowEdit(this, values, row, 0, 0, TableEditType.INSERT));
            ...
            // Set the table sort capability in case the table had no rows prior to the
            // insert
        ...
        protected void removeRow(int row, boolean undoable) {
            ...
            if (isAllowUndo && undoable) {
                ...
                if (listeners.length != 0) {
                    ...
                    // Create the edit event to be passed to the listeners
                    UndoableEditEvent editEvent = new UndoableEditEvent(this,
                            new RowEdit(this, values, row, 0, 0, TableEditType.DELETE));
            ...
            // Disable the row sorter in case the last table row is removed and a row filter
            // is
            // active
        ...
        protected void moveRow(int start, int end, int to, boolean undoable) {
            ...
            if (isAllowUndo && undoable) {
                ...
                // Check if there is an edit listener registered and that the target row isn't
                // the
                // same as the starting row
                if (listeners.length != 0 && start != to) {
                    // Create the edit event to be passed to the listeners
                    UndoableEditEvent editEvent = new UndoableEditEvent(this,
                            new RowEdit(this, null, to, start, end, TableEditType.MOVE));
            ...
            if (sorter != null && sorter.getRowFilter() != null) {
                ...
                // correctly. If this is done for all cases it can cause the table to scroll to
                // an
                // unexpected position
    ...
    protected class RowEdit extends AbstractUndoableEdit {
         ...
         *
         * @param type       TableEditType.INSERT if inserting a row;
         *                   TableEditType.DELETE if removing a row; TableEditType.MOVE
         *                   if moving a row or rows
         *****************************************************************************************/
    ...
    protected class CellEdit extends AbstractUndoableEdit {
        ...
        /******************************************************************************************
         * Set the selected cell in the table. In order for this to select the cell it
         * must be scheduled to execute after other pending events
         *****************************************************************************************/
    ...
    protected class UndoableTableColumnModel extends DefaultTableColumnModel {
        ...
        /******************************************************************************************
         * Override the default method with a method that includes a flag to store the
         * column moved in the undo stack
         *****************************************************************************************/
        @Override
        ...
        private void moveColumn(int columnIndex, int newIndex, boolean undoable) {
            ...
            if (isAllowUndo && undoable) {
                ...
                // Check if there is an edit listener registered and that the column position
                // has
                // changed
                if (listeners.length != 0 && columnIndex != newIndex) {
                    // Create the edit event to be passed to the listeners
                    UndoableEditEvent editEvent = new UndoableEditEvent(this,
                            new ColumnEdit(this, columnIndex, newIndex, TableEditType.MOVE));
    ...
    protected class ColumnEdit extends AbstractUndoableEdit {
         ...
         *
         * @param type             TableEditType.INSERT if inserting a column;
    ...
    /**********************************************************************************************
     * Cell selection edit event handler class. This handles undo and redo of cell
     * selection events related to removing data field represented by the selected
     * cell
     *********************************************************************************************/
    @SuppressWarnings("serial")
    private class CellSelectEdit extends AbstractUndoableEdit {
         ...
         *
         * @param selectState state to which the cell should be set; true to show the
    ...
    private class TreePathSelectEdit extends AbstractUndoableEdit {
        ...
        public void undo() throws CannotUndoException {
            ...
            // Reset the flag so that changes to the tree are handled by the undo/redo
            // manager
            isAllowUndo = true;
        }
    ...
    protected class UndoableTreeModel extends DefaultTreeModel {
        /******************************************************************************************
         * Tree model undo/redo class constructor. Creates a tree model in which any
        ...
        /******************************************************************************************
         * Tree model undo/redo class constructor. Creates a tree model specifying
         * whether any node can have children, or whether only certain nodes can have
         * children
         *
         * @param root               tree root node
         *
         * @param asksAllowsChildren false if any node can have children, true if each
        ...
        /******************************************************************************************
         * Add the specified child node to the specified parent node at the given
         * position
         *
        ...
        public void valueForPathChanged(TreePath path, Object newValue) {
            ...
            if (isAllowUndo) {
                ...
                if (listeners.length != 0) {
                    // Create the edit event to be passed to the listeners
                    UndoableEditEvent editEvent = new UndoableEditEvent(this,
                            new UndoableNodeChangeEdit(path, oldValue, newValue));
        ...
        public void removeNodeFromParent(MutableTreeNode child) {
            ...
            if (isAllowUndo) {
                ...
                if (listeners.length != 0) {
                    // Create the edit event to be passed to the listeners
                    UndoableEditEvent editEvent = new UndoableEditEvent(this,
                            new UndoableNodeRemoveEdit(parent, child, index));
        ...
        /******************************************************************************************
         * Placeholder for performing any actions needed following an undo or redo
         * operation that adds or removes a node
         *****************************************************************************************/
        ...
        /******************************************************************************************
         * Placeholder for performing any actions needed following an undo or redo
         * operation that affects a node's user object (name)
         *
        ...
        private class UndoableNodeAddEdit extends AbstractUndoableEdit {
            ...
            UndoableNodeAddEdit(MutableTreeNode parent, MutableTreeNode child, int index) {
                ...
                if (tree != null) {
                    ...
                    SwingUtilities.invokeLater(new Runnable() {
                        /**************************************************************************
                         * Since the node restoration involves a GUI update use invokeLater to execute
                         * the call after other pending events. This is necessary for the tree to appear
    ...
    /**********************************************************************************************
     * Data field panel undo/redo class. This handles 'structural' edits to the data
     * field(s) (e.g., field addition or deletion, or changes to the field's name,
     * input type, size, etc., as opposed to the field's value)
     *********************************************************************************************/
    @SuppressWarnings("serial")
    protected class UndoableDataFieldPanel extends JComponent {
        ...
        /******************************************************************************************
         * Set the current state of the data field information. This is the state the
         * data fields revert to in the event of an undo operation on a data field edit
         * (not including edits of the field's value)
         *
         ...
         *
         * @param fieldPnlHandler reference to the data field panel handler in which the
         *                        edit occurred
         *
        ...
        protected void addDataFieldEdit(CcddInputFieldPanelHandler fieldPnlHandler,
            ...
            if (isAllowUndo) {
                ...
                if (listeners.length != 0) {
                    // Create the edit event to be passed to the listeners
                    UndoableEditEvent editEvent = new UndoableEditEvent(this,
                            new DataFieldPanelEdit(fieldPnlHandler, oldFieldInfo, newFieldInfo));
    ...
    private class DataFieldPanelEdit extends AbstractUndoableEdit {
         ...
         *                        edit occurred
         *
         * @param oldFieldInfo    list containing the original state of the data field
         *                        information
         *
         * @param newFieldInfo    list containing the new state of the data field
         *                        information
         *****************************************************************************************/
        ...
        public void undo() throws CannotUndoException {
            ...
            // Rebuild the data fields using the original data field information list. Do
            // not store
        ...
        public void redo() throws CannotUndoException {
            ...
            // Rebuild the data fields using the updated data field information list. Do not
            // store
            // this edit operation on the undo/redo stack
