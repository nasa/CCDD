...
public class CcddApplicationSchedulerInput implements CcddSchedulerInputInterface {
    ...
    private void initialize() {
        ...
        treePnl.add(applicationTree.createTreePanel("Applications", TreeSelectionModel.DISCONTIGUOUS_TREE_SELECTION, true,
                    ccddMain.getMainFrame()), new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.LINE_START,
                    GridBagConstraints.BOTH, new Insets(0, 0, ModifiableSpacingInfo.LABEL_VERTICAL_SPACING.getSpacing()
                    / 2, 0), 0, 0));

        // Initialize the currently selected rate to 1 Hz if present in the list of available rates;
        // otherwise choose the first rate if any rates exist, and if none exist set the rate to a dummy value
        List<String> availableRates = Arrays.asList(getAvailableRates());
        updateVariableTree(
                availableRates.contains("1") ? "1" : (!availableRates.isEmpty() ? availableRates.get(0) : "0"));
    }

    /**********************************************************************************************
     * Get the index at which the specified application should be inserted in the
     * list of applications provided. The application tree is used to determine the
     * target application's position relative to the applications in the list (if
     * any)
     *
     * @param application  application for which to determine the insertion index
     *
     * @param applications list of applications into which the application is to be
     *                     inserted
     *
     ...
     *
     * @param applications list of applications; null to use the currently selected
     *                     application(s)
     *
     ...
     *
     * @return List of variable object(s) representing the application(s) at the
     *         specified rate
     *********************************************************************************************/
    @Override
    public List<Variable> getVariablesAtRate(String rate) {
        ...
        for (GroupInformation grpInfo : applicationTree.getGroupHandler().getGroupInformation()) {
            ...
            if (grpInfo.isApplication() && rate.equals(getDataFieldValue(grpInfo.getName(),
                    DefaultApplicationField.SCHEDULE_RATE)) && !excludedList.contains(grpInfo.getName())) {
    ...
    private List<Variable> getApplicationFields(String[] applications, String rate) {
        ...
        for (String application : applications) {
            ...
            appList.add(new ApplicationData(application, CcddUtilities.convertStringToFloat(rate),
                    Integer.valueOf(getDataFieldValue(application, DefaultApplicationField.EXECUTION_TIME)),
                    Integer.valueOf(getDataFieldValue(application, DefaultApplicationField.PRIORITY)),
                    Integer.valueOf(getDataFieldValue(application, DefaultApplicationField.MESSAGE_RATE)),
                    getDataFieldValue(application, DefaultApplicationField.WAKE_UP_MESSAGE),
                    Integer.valueOf(getDataFieldValue(application, DefaultApplicationField.HK_SEND_RATE)),
    ...
    public String[] getAvailableRates() {
        ...
        for (GroupInformation appInfo : applicationTree.getGroupHandler().getGroupInformation()) {
            ...
            FieldInformation rateInfo = fieldHandler.getFieldInformationByName(
                    CcddFieldHandler.getFieldGroupName(appInfo.getName()), "Schedule Rate");
     ...
     *
     * @param applications list of applications to be removed from the excluded
     *                     application list
     *********************************************************************************************/
    @Override
    ...
    private String getDataFieldValue(String applicationName, DefaultApplicationField appField) {
        ...
        if (!applicationName.isEmpty()) {
            ...
            FieldInformation groupInfo = fieldHandler.getFieldInformationByName(
                    CcddFieldHandler.getFieldGroupName(applicationName), appField.getFieldName());
