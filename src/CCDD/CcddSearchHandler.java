...
public class CcddSearchHandler extends CcddDialogHandler {
     ...
     *
     * @param dataTablesOnly true if only the data tables, and not references in the
     *                       internal tables, are to be searched
     *
     * @param searchColumns  string containing the names of columns, separated by
     *                       commas, to which to constrain a table search
     *
     * @return Search results list containing object arrays providing each match's
     *         location in the database tables or event log, the column within the
     *         location, and an extract for the located match showing its context
     *********************************************************************************************/
    protected List<Object[]> searchTablesOrScripts(String searchText, boolean ignoreCase, boolean dataTablesOnly,
        ...
        String searchType = searchDlgType == SearchDialogType.TABLES
                ? (dataTablesOnly ? SearchType.DATA.toString() : SearchType.ALL.toString())
                : SearchType.SCRIPT.toString();

        // Search the database for the text
        String[] hits = dbCommand.getList(DatabaseListCommand.SEARCH,
                new String[][] { { "_search_text_", searchText }, { "_case_insensitive_", String.valueOf(ignoreCase) },
                        { "_allow_regex_", "true" }, { "_selected_tables_", searchType },
    ...
    /**********************************************************************************************
     * Sort the search results by the first (target) column, and if the same then by
     * second (location) column. Array variable member references in the location
     * column are arranged by array dimension value
     *
     * @param resultsDataList list containing the sorted search results
     *
     * @return Search results sorted by the first (target) column, and if the same
    ...
    /**********************************************************************************************
     * Remove data type or macro search references where the match occurs in an
     * array size column of an array member (the reference in the array's definition
     * is all that's needed)
     *
     * @param matches      list containing the search results for the data type or
     *                     macro reference
     *
    ...
    /**********************************************************************************************
     * Create the match pattern from the search criteria. If the allow regular flag
     * is set then the search string is used as is. If the allow regular expression
     * flag isn't set then a wild card match is enabled. A custom matching system is
     * used: a question mark matches a single character and an asterisk matches one
     * or more characters. This is turned into a regular expression to perform the
     * actual match
     *
     ...
     *
     * @param ignoreCase true if the search is case insensitive and the search
     *                   pattern is not used by the PostgreSQL search_atables()
     *                   function (must be false for this case; the case insensitive
     *                   flag is provided directly to the function instead of being
     *                   included in the matching string)
     *
     ...
     *                   to not display an error dialog if the search pattern is
     *                   invalid
     *
     * @return Compiled regular expression search pattern; null if the regular
    ...
    protected static Pattern createSearchPattern(String searchText, boolean ignoreCase, boolean allowRegEx,
        ...
        try {
            ...
            // replacing these
            // with a marker)
            searchPattern = Pattern.compile("(?" + (ignoreCase ? "i" : "") + ":"
                    + (allowRegEx ? searchText.replaceAll("\\\\", "\\\\\\\\\\\\")
                            : searchText.replaceAll("([\\[\\]\\(\\)\\{\\}\\.\\+\\^\\$\\|\\-\\\\])", "\\\\\\\\\\\\$1")
                                    .replaceAll("\\\\\\?", WILD_CARD_MARKER).replaceAll("\\?", ".")
                                    .replaceAll(WILD_CARD_MARKER, "\\\\?").replaceAll("\\\\\\*", WILD_CARD_MARKER)
                                    .replaceAll("\\*", ".*?").replaceAll(WILD_CARD_MARKER, "\\\\*"))
                    + ")");
            ...
            if (parent != null) {
                ...
                new CcddDialogHandler().showMessageDialog(parent,
                        "<html><b>Invalid regular expression; cause '</b>" + pse.getMessage() + "'<b>", "Invalid Input",
                        JOptionPane.WARNING_MESSAGE, DialogOption.OK_OPTION);
            }
        } catch (Exception e) {
