...
public class CcddCommandLineHandler {
    ...
    private class CommandHandler {
        ...
        /******************************************************************************************
         * Individual command line argument handler class constructor for commands with
         ...
         *
         * @param type        command type: CommandLineType.NAME,
         *                    CommandLineType.MINMAX, CommandLineType.SIZE,
         *                    CommandLineType.COLOR, or CommandLineType.OPTION
         *
         * @param priority    order in which the command line argument should be
         ...
         *
         * @param conditions  array of output parameters associated with each input
         *                    option
         *
        ...
        /******************************************************************************************
         * Individual command line argument handler class constructor for commands
         * having no limit values or fixed set of options
         *
         * @param command     command name
         *
         * @param description command description
         ...
         *
         * @return List of sub-arguments for this command; null if the command has no
         *         sub-arguments
         *****************************************************************************************/
    ...
    CcddCommandLineHandler(final CcddMain ccddMain, String[] args) {
        ...
        argument.add(new CommandHandler("scriptOutPath",
                ...
                "file path", CommandLineType.NAME, CommandLinePriority.DB_DEPENDENT.getStartPriority()) {
            /**************************************************************************************
        ...
        argument.add(new CommandHandler("shutdown",
            ...
            /**************************************************************************************
             * Set the flags to hide the user interface and shutdown the application
             * following completion of the command line commands
             *************************************************************************************/
            @Override
            protected void doCommand(Object parmVal) {
        ...
        argument.add(new CommandHandler("execute", "Execute script(s)",
                "[association name] or\n" + "[\" or ']script file name[" + SCRIPT_MEMBER_SEPARATOR + "table1\n"
            ...
            protected void doCommand(Object parmVal) throws Exception {
                ...
                // command.
                // If the script is executed in the background then when it completes execution
                // the
                // script output path is restored to the program start-up value. Therefore it's
                // set
                // to the value as updated via command line command prior to each execution
                // command
                    ...
                    if (ccddMain.isGUIHidden()) {
                        if ((importFileType == FileExtension.JSON) || (importFileType == FileExtension.CSV) ||
                            ...
                            if (ccddMain.getFileIOHandler().importFiles(dataFile, backupFirst, replaceExistingTables, appendExistingFields,
                                    ...
                                    replaceExistingGroups, replaceExistingDataTypes, importFullDatabase, dialogType, null)) {
                                throw new Exception();
                            }
                        } else {
        ...
        importArgument.add(new CommandHandler("fileName", "Import path+file name(s)",
            ...
            /**************************************************************************************
             * Set the import file path+name(s). Multiple files in the same path can be
             * specified by separating the first path+name from the subsequent names with a
             * plus (+).
             *
             * Example: -fileName "/path/to/fileA+fileB+subPath/fileC" specifies files
             * /path/to/fileA, /path/to/fileB, and /path/to/subPath/fileC.
             *
             * Multiple path groups can be specified by separating each grouping with a
             * semi-colon (;).
             *
        ...
        importArgument
                .add(new CommandHandler("appendExistingFields",
        ...
        importArgument.add(new CommandHandler("deleteAbsentFiles",
                "delete files/data from database that does not exist in import files", "true or false (default: false)",
        ...
        argument.add(new CommandHandler("export", "Export tables, etc. in CSV, EDS, JSON,\n" + "  or XTCE format",
            ...
            /**************************************************************************************
             * Export one or more tables to a file (or files) in CSV, EDS, JSON, or XTCE
             * format
             *************************************************************************************/
            @Override
            protected void doCommand(Object parmVal) throws Exception {
                ...
                if ((filePath.isEmpty() && (fileExtn == FileExtension.EDS || fileExtn == FileExtension.XTCE
        ...
        exportArgument.add(
                new CommandHandler("singleFile", "Store in single file (CSV, JSON)", "true or false (default: false)",
                        CommandLineType.OPTION, 0, new Object[] { true, false }, new String[] { "true", "false" }) {
                    /**************************************************************************************
        ...
        exportArgument.add(new CommandHandler("includeAllTableTypes",
                "Include all table type definitions\n" + " (CSV, JSON)", "true or false (default: false)",
                CommandLineType.OPTION, 0, new Object[] { true, false }, new String[] { "true", "false" }) {
            /**************************************************************************************
        ...
        exportArgument.add(new CommandHandler("includeAllInputTypes",
                "Include all user-defined input type\n" + "  definitions (CSV, JSON)", "true or false (default: false)",
                CommandLineType.OPTION, 0, new Object[] { true, false }, new String[] { "true", "false" }) {
            /**************************************************************************************
        ...
        exportArgument.add(new CommandHandler("includeBuildInformation",
                ...
                "true or false (default: true)", CommandLineType.OPTION, 0, new Object[] { true, false },
                new String[] { "true", "false" }) {
            /**************************************************************************************
        ...
        exportArgument
                .add(new CommandHandler("replaceMacros", "Replace macros with values", "true or false (default: false)",
                        CommandLineType.OPTION, 0, new Object[] { true, false }, new String[] { "true", "false" }) {
                    /**************************************************************************************
        ...
        exportArgument
                .add(new CommandHandler("exportFullDatabase", "Export full database", "true or false (default: false)",
                        CommandLineType.OPTION, 0, new Object[] { true, false }, new String[] { "true", "false" }) {
                    /**************************************************************************************
                     * Set the flag to output the project data field definitions
                     *************************************************************************************/
        ...
        exportArgument.add(new CommandHandler("includeGroups",
                "Include group definitions and data\n" + " fields (CSV, JSON)", "true or false (default: false)",
                CommandLineType.OPTION, 0, new Object[] { true, false }, new String[] { "true", "false" }) {
            /**************************************************************************************
        ...
        exportArgument.add(new CommandHandler("format", "Export file format",
                "csv, eds, json, or xtce\n" + " (default: csv)", CommandLineType.OPTION, 0,
        ...
        exportArgument.add(new CommandHandler("endianess", "Endianess (EDS, XTCE)", "big or little (default: big)",
                CommandLineType.OPTION, 0, new Object[] { EndianType.BIG_ENDIAN, EndianType.LITTLE_ENDIAN },
        ...
        exportArgument.add(new CommandHandler("isHeaderBigEndian",
                "Force telemetry & command header to\n" + "big endian (EDS, XTCE)", "true or false (default: true)",
                CommandLineType.OPTION, 0, new Object[] { true, false }, new String[] { "true", "false" }) {
            /**************************************************************************************
        ...
        exportArgument.add(
                new CommandHandler("version", "Version (EDS, XTCE)", "text (default: 1.0)", CommandLineType.NAME, 0) {
                    /**************************************************************************************
        ...
        argument.add(new CommandHandler("create", "Create a new project database", "'<create sub-commands>'",
                CommandLineType.NAME, CommandLinePriority.SET_UP.getStartPriority() + 5, createArgument) {
            /**************************************************************************************
             * Create a new project database
             *************************************************************************************/
            @Override
        ...
        createArgument.add(
                new CommandHandler("restore", "Backup file to restore", "backup file name", CommandLineType.NAME, 0) {
                    /**************************************************************************************
        ...
        argument.add(new CommandHandler("delete", "Delete an existing project database", "<delete sub-commands>",
                CommandLineType.NAME, CommandLinePriority.SET_UP.getStartPriority() + 4, deleteArgument) {
            /**************************************************************************************
             * Delete an existing project database
             *************************************************************************************/
            @Override
                    ...
                    for (String userDbInfo : dbControl.queryDatabaseList(ccddMain.getMainFrame())) {
                        ...
                        if (isNameMatch) {
                            ...
                            // The user doesn't have administrative access
                                ...
                                throw new Exception(
                                        "Delete disabled; user lacks write access for project '" + deleteName);
                            }
        ...
        deleteArgument.add(
                new CommandHandler("name", "Name of the project to delete", "project name", CommandLineType.NAME, 0) {
                    /**************************************************************************************
                     * Set the project name
                     *************************************************************************************/
                    @Override
        ...
        deleteArgument.add(new CommandHandler("continueIfMissing",
                "Continue to process commands if the\n" + "  project doesn't exist", "", CommandLineType.NONE, 0) {
            /**************************************************************************************
             * Set the flag to indicate that command execution should continue if the
    ...
    /**********************************************************************************************
     * Parse and execute the main command line argument(s). The application exits if
     * the shutdown command is present and all command line commands have completed
     * execution
     *
     * @param priority command priority range - execute only those commands with a
     *                 priority &gt;= to the start value (-1 to start with the
     *                 lowest priority command) and &lt;= to the end value (-1 to
     *                 end with the highest priority command)
     *********************************************************************************************/
     ...
     *
     * @param startPriority   command priority boundary - ignore commands with a
     *                        priority less than this value; -1 starts with the
     *                        lowest priority command
     *
     * @param endPriority     command priority boundary - ignore commands with a
     *                        priority greater than this value; -1 ends with the
    ...
    /**********************************************************************************************
     * Perform any clean-up steps following command line argument execution or
     * failure
     *
     * @param exitStatus 0 if all commands executed successfully; 1 if an error
     *                   occurred
     *********************************************************************************************/
    ...
    private void displayUsageInformation() {
        ...
        String usage = String.format(
                ...
                "Command", "Value", "Description", dash, dash, dash);
     ...
     *
     * @param min  array containing the minimum main window width and minimum main
     *             window height
     *
     * @return Main window dimension value (null if an error is found)
     *
     * @throws Exception If the width or height value is not a number or a value is
     *                   missing
     *********************************************************************************************/
     ...
     *
     * @param limit array containing the lower limit value (inclusive) and the upper
     *              limit value (inclusive)
     *
     ...
     *
     * @throws Exception If the color value is not in the expected hexadecimal
     *                   format or is not a recognized color name
     *********************************************************************************************/
    private Integer handleColor(String arg, String parm) throws Exception {
        ...
        try {
            ...
            throw new Exception(
                    arg + " must be a color name or in the format " + "'0x######' where '#' is a hexadecimal digit");
        }
    ...
    /**********************************************************************************************
     * Get the selected option which must match one of a predefined list of options.
     * The command argument if forced to lower case when comparing to the acceptable
     * options
     *
     ...
     *
     * @throws Exception If the supplied value is not one of those in the predefined
     *                   list
     *********************************************************************************************/
    private Object handleOption(String arg, String parm, String[] inputOptions, Object[] outputValues)
            throws Exception {
        Object val = null;
        int index = 0;
        String valid = "";
