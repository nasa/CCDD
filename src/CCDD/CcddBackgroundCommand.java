...
public class CcddBackgroundCommand
{
     ...
     * background (this method must be supplied), and a command completed
     * method that is run after the execute method ends (this method can be
    ...
    /**************************************************************************
     * Execute a command in the background. The mouse cursor for the main
     * window is set to the 'wait' cursor and the main window's menu items are
     ...
     * component is set to the 'wait' cursor and the main window's menu items
                                                           ...
                                                           final BackgroundCommand backCommand)
    {
        ...
        if (!ccddMain.isGUIHidden())
        {
            ...
            // Since this could take a while, show the "wait" mouse pointer
            // over the main window to alert the user. The pointer is restored
            // to the default when the command completes
            ...
            if (dialog != null)
            {
                ...
                {
                    // Get the dialog's glass pane and disable the dialog's
                    // controls
                ...
                {
                    // Get the frame's glass pane and disable the frame's
                    // controls
                ...
                if (dlgGlassPane != null)
                {
                    ...
                    // pointer over the dialog to alert the user. The pointer
                    // is restored to the default when the command completes
            ...
            protected void done()
            {
                ...
                if (!ccddMain.isGUIHidden())
                {
                    ...
                    if (dlgGlassPane != null)
                    {
                        ...
                        // Restore the dialog's default mouse pointer and hide
                        // the glass pane
                    ...
                    // Hide the glass pane. If this isn't done the resize mouse
                    // pointer doesn't appear in the main window's session
                    // event log header
        ...
        // Initiate the command. The application does not wait for this thread
        // to complete
