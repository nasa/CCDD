...
public class CcddFieldTableEditorDialog extends CcddFrameHandler {
    ...
    /**********************************************************************************************
     * Display a dialog for the user to select the data fields to display in the
     * editor table. This is executed in a separate thread since it can take a
     ...
     * until the telemetry scheduler initialization completes execution
     *********************************************************************************************/
    private void selectDataFields() {
        ...
        CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand() {
            ...
            protected void execute() {
                    ...
                    protected boolean verifySelection() {
                        ...
                        if (selectDlg.getCheckBoxSelected().length == 0) {
                            ...
                            new CcddDialogHandler().showMessageDialog(this,
                                    "<html><b>Must select at least one data field", "No Data Field Selected",
                ...
                // No data field exists to choose
                    ...
                    new CcddDialogHandler().showMessageDialog(
                            (isVisible() ? CcddFieldTableEditorDialog.this : ccddMain.getMainFrame()),
                            "<html><b>No data field exists", "No Data Field", JOptionPane.WARNING_MESSAGE,
            ...
            protected void complete() {
                ...
                if (!fieldHandler.getFieldInformation().isEmpty() && selectDlg.showOptionsDialog(
                        (isVisible() ? CcddFieldTableEditorDialog.this : ccddMain.getMainFrame()), selectPnl,
                        "Select Data Field(s)", DialogOption.OK_CANCEL_OPTION, true) == OK_BUTTON) {
    ...
    /**********************************************************************************************
     * Create the data field table editor dialog. This is executed in a separate
            ...
            protected void execute() {
                ...
                rowFilter = new RowFilter<TableModel, Object>() {
                    ...
                    public boolean include(Entry<? extends TableModel, ? extends Object> owner) {
                        ...
                        String ownerName = highlightFieldOwner(
                                owner.getValue(FieldTableEditorColumnInfo.OWNER.ordinal()).toString(), false);
    ...
    protected void windowCloseButtonAction() {
        ...
        if (dataFieldTable.isLastCellValid()
                && (!isFieldTableChanged() || new CcddDialogHandler().showMessageDialog(CcddFieldTableEditorDialog.this,
                        "<html><b>Discard changes?", "Discard Changes", JOptionPane.QUESTION_MESSAGE,
                        DialogOption.OK_CANCEL_OPTION) == OK_BUTTON)) {
            // Close the dialog
            closeFrame();
    ...
    /**********************************************************************************************
     * Reposition the field table editor dialog relative to the location of the (now
     * invisible) field selection dialog
     *********************************************************************************************/
    ...
    private JScrollPane createDataFieldTableEditorTable() {
        ...
        dataFieldTable = new CcddJTableHandler() {
            ...
            /**************************************************************************************
             * Allow multiple line display in all columns except those displaying check
             * boxes
             *************************************************************************************/
            @Override
            ...
            public boolean isCellEditable(int row, int column) {
                ...
                return column != FieldTableEditorColumnInfo.OWNER.ordinal()
                        && column != FieldTableEditorColumnInfo.PATH.ordinal()
                        && fieldHandler.getFieldInformationByName(getOwnerWithPath(
                                getModel().getValueAt(row, FieldTableEditorColumnInfo.OWNER.ordinal()).toString(),
                                getModel().getValueAt(row, FieldTableEditorColumnInfo.PATH.ordinal()).toString()),
            ...
            protected Boolean validateCellContent(List<Object[]> tableData, int row, int column, Object oldValue,
                ...
                if (!newValueS.isEmpty()) {
                    ...
                    String ownerAndPath = getOwnerWithPath(
                            tableData.get(row)[FieldTableEditorColumnInfo.OWNER.ordinal()].toString(),
                        ...
                        // The value doesn't match the pattern for this data field
                            ...
                            new CcddDialogHandler().showMessageDialog(CcddFieldTableEditorDialog.this,
                                    "<html><b>Invalid characters in field '</b>" + fieldInfo.getFieldName()
                                            ...
                                            + fieldInfo.getInputType().getInputName() + "<b>' expected",
                                    "Invalid " + fieldInfo.getInputType().getInputName(), JOptionPane.WARNING_MESSAGE,
                                    DialogOption.OK_OPTION);
            ...
            /**************************************************************************************
             * Override the CcddJTableHandler method in order to show/hide the data fields
             * based on the selected field filters
             *************************************************************************************/
            @Override
                                ...
                                if (inputType.equals(ccddMain.getInputTypeHandler()
                                    ...
                                    for (int checkRow = 0; checkRow < tableModel.getRowCount(); checkRow++) {
                                        ...
                                        if (rowModel != checkRow) {
                                            // Separate the message name and ID
                                            String[] chkNameID = CcddMessageIDHandler.getMessageNameAndID(
                                                    tableModel.getValueAt(checkRow, columnModel).toString());
        ...
        dataFieldTable.setFixedCharacteristics(scrollPane, false, ListSelectionModel.MULTIPLE_INTERVAL_SELECTION,
                TableSelectionMode.SELECT_BY_CELL, true, ModifiableColorInfo.TABLE_BACK.getColor(), true, true,
    ...
    /**********************************************************************************************
     * Set up combo box cell editors for the columns that display data fields with
     * selection item lists
     *********************************************************************************************/
        ...
        for (int columnModel = 2; columnModel < dataFieldTable.getModel().getColumnCount(); columnModel++) {
            ...
            for (int row = 0; row < dataFieldTable.getRowCount(); row++) {
                ...
                FieldInformation fieldInfo = fieldHandler.getFieldInformationByName(
                        CcddUtilities.removeHTMLTags(dataFieldTable.getValueAt(row, 0).toString()),
     ...
     * of the changed values necessary to update the table data fields in the
     * database to match the current values
     *********************************************************************************************/
    private void buildUpdates() {
        // TODO THIS MAY REQUIRE ALTERATIONS DUE TO INHERITANCE WHEN CHANGING TABLE TYPE
        // FIELD
        // VALUES - THERE'S NO WAY TO SET (OR SEE) THE OVERWRITE BEHAVIOR IN THIS
        // EDITOR. COULD (1)
        // DECIDE THAT CHANGES MADE HERE ARE NEVER PROPAGATED TO THE FIELDS BELONGING TO
        // TABLES OF
        // THE CHANGED TYPE OR (2) NOT ALLOW CHANGES TO TABLE TYPE FIELD VALUES FROM
        // THIS EDITOR.
        //
        // DID IMPLEMENT CHANGE SO THAT INHERITED FIELDS ARE DELETED IF THE DEFAULT IS
        // DELETED.
        ...
        for (int row = 0; row < tableData.length; row++) {
            ...
            for (int column = 0; column < tableData[row].length; column++) {
                ...
                if (column != FieldTableEditorColumnInfo.OWNER.ordinal()
                    ...
                    String ownerName = getOwnerWithPath(
                            tableData[row][FieldTableEditorColumnInfo.OWNER.ordinal()].toString(),
                    ...
                    if (selectedCells.contains(dataFieldTable.convertRowIndexToView(row),
                        ...
                        if (CcddFieldHandler.isTableTypeField(ownerName)) {
                            ...
                            for (String tablePath : dbTable.getAllTablesOfType(
                                    ownerName.replace(TYPE_DATA_FIELD_IDENT, ""), null,
                                    CcddFieldTableEditorDialog.this)) {
                    ...
                    // Check if the current and committed column values differ
                        ...
                        fieldModifications.add(
                                new String[] { ownerName, columnNames[column], tableData[row][column].toString() });
                    }
                }
            }
        }
     ...
     *
     * @return Array containing the unique project data field names in alphabetical
     *         order
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Determine if the data field owner isn't a table. This is the case for
     * project, group, and table type fields
     *
    ...
    /**********************************************************************************************
     * Highlight the data field owner's indicator text (the indicator determines if
     * the field belongs to the project, a group, or a table type)
     *
     * @param ownerName name of the data field's owner
     *
     * @param enable    true to highlight the data field indicator; false to remove
     *                  any highlighting
     *
     * @return The data field owner with the indicator highlighted or not
     *         highlighted
     *********************************************************************************************/
     ...
     *
     * @return true if changes exist that haven't been saved; false if there are no
     *         unsaved changes
     *********************************************************************************************/
