...
public class CcddFieldEditorDialog extends CcddDialogHandler {
     ...
     *
     * @param ownerName            table name, including the path if this is a
     *                             structure type table, or group name
     *
    ...
    private void initialize(final int minimumWidth) {
        ...
        fieldTable = new CcddJTableHandler() {
            /**************************************************************************************
             * Allow resizing of all columns except the Size, Required, and Applicability
             * columns
             *************************************************************************************/
            @Override
            ...
            /**************************************************************************************
             * Allow multiple line display in all but the Required, Size, and Applicability
             * columns
             *************************************************************************************/
            @Override
            ...
            /**************************************************************************************
             * Override isCellEditable so that all columns except the line separators and
             * breaks can be edited
             *************************************************************************************/
            @Override
            public boolean isCellEditable(int row, int column) {
                ...
                if (fieldTable.getRowCount() != 0) {
                    ...
                    (column != FieldEditorColumnInfo.APPLICABILITY.ordinal() && inputTypeHandler
                            .getInputTypeByName(fieldTable.getValueAt(row, inputTypeIndex).toString()).getInputFormat()
                            .equals(InputTypeFormat.PAGE_FORMAT))

                            // ... the row represents a table's inherited field
                            || (CcddFieldHandler.isTableField(ownerName) && Boolean.parseBoolean(fieldTable.getModel()
                                    .getValueAt(convertRowIndexToModel(row), FieldEditorColumnInfo.INHERITED.ordinal())
                                    .toString()))) {
                    ...
                    if (column == FieldEditorColumnInfo.NAME.ordinal() && !newValueS.isEmpty()) {
                        ...
                        for (int otherRow = 0; otherRow < getRowCount(); otherRow++) {
                            ...
                            if (otherRow != row
                                ...
                                throw new CCDDException("Field name '</b>" + newValueS + "<b>' already in use");
                            }
                        }
                    }
            ...
            protected void loadAndFormatData() {
                ...
                setUpdatableCharacteristics(currentData, FieldEditorColumnInfo.getColumnNames(), null,
                        FieldEditorColumnInfo.getToolTips(), true, true, true);
            }
            ...
            public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
                ...
                if (!(isFocusOwner() && isRowSelected(row)
                    ...
                    if (Boolean.parseBoolean(fieldTable.getModel()
                            .getValueAt(row, FieldEditorColumnInfo.INHERITED.ordinal()).toString())) {
                    ...
                    // Check if this is the input type column
                        ...
                        // The type is not a separator or line break
                            ...
                            for (int index = 0; index < inputTypeCbox.getItemCount() && !found; index++) {
                                ...
                                if (inputTypeCbox.getItemAt(index)
                                        .equals(fieldTable.getValueAt(row, column).toString())) {
                                    // Set the flag indicating that the cell value is valid
                                    found = true;
                                }
                            }
        ...
        fieldTable.setFixedCharacteristics(scrollPane, true, ListSelectionModel.MULTIPLE_INTERVAL_SELECTION,
        ...
        // Move Down button
        btnMoveDown = CcddButtonPanelHandler.createButton("Down", DOWN_ICON, KeyEvent.VK_W,
        ...
        // Separator button
        btnSeparator = CcddButtonPanelHandler.createButton("Separator", SEPARATOR_ICON, KeyEvent.VK_S,
        ...
        btnSeparator.addActionListener(new ValidateCellActionListener(fieldTable) {
            ...
            protected void performAction(ActionEvent ae) {
                fieldTable.insertRow(true, TableInsertionPoint.SELECTION,
                        new Object[] { DefaultInputType.SEPARATOR.getInputName(), "Line separator", 0,
                                DefaultInputType.SEPARATOR.getInputName(), false,
        ...
        btnBreak.addActionListener(new ValidateCellActionListener(fieldTable) {
            ...
            protected void performAction(ActionEvent ae) {
                fieldTable.insertRow(true, TableInsertionPoint.SELECTION,
                        new Object[] { DefaultInputType.BREAK.getInputName(), "Line break", 0,
                                DefaultInputType.BREAK.getInputName(), false,
                                ApplicabilityType.ALL.getApplicabilityName(), "", false });
            }
        });
        ...
        btnUpdate = CcddButtonPanelHandler.createButton("Update", STORE_ICON, KeyEvent.VK_U,
        ...
        btnUpdate.addActionListener(new ValidateCellActionListener(fieldTable) {
            ...
            protected void performAction(ActionEvent ae) {
                ...
                if (!checkForMissingColumns() && fieldTable.isLastCellValid()
    ...
    protected void windowCloseButtonAction() {
        ...
        // discarding
        // the changes
        if (fieldTable.isLastCellValid() && (!fieldTable.isTableChanged(currentData)
                || new CcddDialogHandler().showMessageDialog(CcddFieldEditorDialog.this, "<html><b>Discard changes?",
    ...
    /**********************************************************************************************
     * Set up the combo box containing the available field input input types for
    ...
    private void setUpInputTypeColumn() {
        ...
        inputTypeCbox = new PaddedComboBox(inputTypeHandler.getInputTypeNames(), inputTypeHandler.getDescriptions(),
    ...
    /**********************************************************************************************
     * Set up the combo box containing the available field applicability types for
     * display in the table's Applicability cells
     *********************************************************************************************/
                ...
                for (int column = 0; column < fieldTable.getColumnCount() && !stopCheck; column++) {
                    ...
                    if (FieldEditorColumnInfo.values()[column].isRequired()
                        ...
                        if (new CcddDialogHandler().showMessageDialog(CcddFieldEditorDialog.this,
                                "<html><b>Data must be provided for column '</b>" + fieldTable.getColumnName(column)
                                        + "<b>' [row </b>" + (row + 1) + "<b>]",
