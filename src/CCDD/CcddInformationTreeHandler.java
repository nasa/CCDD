...
public abstract class CcddInformationTreeHandler extends CcddCommonTreeHandler {
     ...
     *
     * @param infoDefinitions list of string arrays containing each node name and
     *                        associated tool tip
     *
     * @param filterValue     string value that may be used to modify the tree
     *                        building method; null or blank if not filtering
     *
     * @param filterFlag      flag used to filter the tree content
     *
     * @param treePathOrder   list containing all of the items that potentially can
     *                        appear in the tree in the order in which they appear
     *                        when added to the tree; null if no order is specified
    ...
    CcddInformationTreeHandler(CcddMain ccddMain, CcddUndoHandler undoHandler, InternalTable infoType,
            List<String[]> infoDefinitions, String filterValue, boolean filterFlag, List<String> treePathOrder,
            Component parent) {
        super(ccddMain);
    ...
    /**********************************************************************************************
     * Placeholder for performing any actions needed following an undo or redo
     * operation that affects a node's user object (name) filter nodes
     *
     * @param wasValue node user object value prior to the undo/redo operation
     *
     * @param isValue  node user object value after the undo/redo operation
    ...
    /**********************************************************************************************
     * Get the node level that skips any active filter nodes and other header nodes.
     * The default number of header nodes is 2; this method can be overridden to
    ...
    /**********************************************************************************************
     * Placeholder for method to perform any initialization steps necessary prior to
    ...
    /**********************************************************************************************
     * Set the list of all tree paths in the table tree in the order to be used when
     * placing a path in the information tree
     *
     * @param treePathOrder list of all paths in the table tree in the order to be
     ...
     *
     * @param order       order in which the added node should be placed relative to
    ...
    /**********************************************************************************************
     * Add a new node to the tree's root node, or the application node is filtering
     * by application is active
     *
     * @param nodeName    name of the node to add
     *
     * @param toolTipText tool tip text for the new node
    ...
    /**********************************************************************************************
     * Add the specified nodes(s) from the source tree to the specified node(s) in
     * the information tree
     *
     * @param sourcePaths     list containing the node paths for the selected nodes
     *                        in the source tree
     *
     * @param startIndex      tree level at which the desired node names first
     *                        appear in the source tree
     *
     * @param onlyIfPrimitive true to only include nodes that end with a reference
     *                        to primitive data type; false to include all nodes
     *********************************************************************************************/
                ...
                if (!infoNodes.contains(infoNode)) {
                    ...
                    for (Object[] sourcePath : sourcePaths) {
                        ...
                        if (onlyIfPrimitive) {
                            ...
                            if (lastPeriod != -1 && dataTypeHandler
                                    .isPrimitive(removeExtraText(lastNode.substring(0, lastPeriod)))) {
     ...
     *
     * @param startIndex tree level at which the variable names first appear in the
     *                   array
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Remove the specified child node, and its ancestors until a node with other
     * children or the top level node is reached
     *
    ...
    /**********************************************************************************************
     * Remove the currently selected top level node(s) and all of its child nodes
     ...
     *
     * @return Array containing the selected node path(s); an empty array if no node
     *         is selected
     *********************************************************************************************/
     ...
     *
     * @return Reference to the renamed node; null if no node with the name oldName
     *         exists
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Placeholder for method to add a copy of the specified information object to
    ...
    /**********************************************************************************************
     * Create the information definitions from the tree for storage in the database
     * or to use as a comparison to a previously committed version of the tree.
     *
     * @return List containing the path array for each child node belonging to an
     *         information node
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Remove any unwanted text from the node names. The object array is converted
     * to a string array
     *
    ...
    /**********************************************************************************************
     * Recursively step through the information tree and append the path to each
     * leaf node to the definition list
     *
    ...
    /**********************************************************************************************
     * Add the specified leaf definition to the tree definition list. Override this
     * method to insert other information into the leaf definition prior to adding
     * it to the tree definition list
     *
     * @param treeDefns   list to which to add the leaf definition
     *
     * @param leafDefn    leaf definition to add to the list
     *
     * @param filterValue string value that may be used to modify the tree; null or
     *                    blank if not used
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Convert the array describing the leaf node path into a single string, with
     * the nodes separated by commas
     *
