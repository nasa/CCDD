...
public class CcddScriptManagerDialog extends CcddFrameHandler {
    ...
    /**********************************************************************************************
     * Perform the steps needed following execution of script association updates to
     * the database
     *
    ...
    /**********************************************************************************************
     * Create the script association manager dialog. This is executed in a separate
            ...
            CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand() {
                ...
                protected void execute() {
                    ...
                    GridBagConstraints gbc = new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.LINE_START,
                            ...
                            new Insets(0, 0, ModifiableSpacingInfo.LABEL_VERTICAL_SPACING.getSpacing(),
                    ...
                    assnsTable.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
                        /**************************************************************************
                         * Handle a table row selection change. Populate the script description field,
                         * file field, and table tree based on the first selected associations table row
                         *************************************************************************/
                        @Override
                                ...
                                if (row != -1) {
                                    ...
                                    for (String tableName : tableNames) {
                                        ...
                                        if (tableName.startsWith(GROUP_DATA_FIELD_IDENT)) {
                                            ...
                                            node = tableTree.getNodeByNodeName(
                                                    tableName.substring(GROUP_DATA_FIELD_IDENT.length()));
                                        }
                    ...
                    JButton btnAddAssn = CcddButtonPanelHandler.createButton("Add", INSERT_ICON, KeyEvent.VK_A,
                        ...
                        public void actionPerformed(ActionEvent ae) {
                            ...
                            // The script file field is blank
                                ...
                                new CcddDialogHandler().showMessageDialog(ccddMain.getMainFrame(),
                                        "<html><b>Must enter or select a script", "Script Missing",
                    ...
                    JButton btnRemoveAssn = CcddButtonPanelHandler.createButton("Remove", DELETE_ICON, KeyEvent.VK_R,
                    ...
                    JButton btnReplaceAssn = CcddButtonPanelHandler.createButton("Replace", REPLACE_ICON, KeyEvent.VK_P,
                    ...
                    btnStoreAssns = CcddButtonPanelHandler.createButton("Store", STORE_ICON, KeyEvent.VK_S,
                        ...
                        public void actionPerformed(ActionEvent ae) {
                            ...
                            if (assnsTable.isTableChanged(committedAssnsData,
                                ...
                                dbTable.storeInformationTableInBackground(InternalTable.ASSOCIATIONS,
                                        createAssociationsFromTable(), null, CcddScriptManagerDialog.this);
                            }
                        }
                    });
    ...
    private JPanel createScriptSelectionPanel() {
        ...
        btnSelectScript.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                FileEnvVar[] scriptFile = new CcddDialogHandler().choosePathFile(ccddMain, CcddScriptManagerDialog.this,
                        null, "script", scriptHandler.getExtensions(), false, "Select Script",
    ...
    /**********************************************************************************************
     * Add an association to the script associations list based on the script and
     * table selections
     *
     * @param insertPoint insertion point for the added row into the associations
     *                    table: TableInsertionPoint.START to insert as the first
     *                    row in the table, TableInsertionPoint.SELECTION to insert
     *                    below the currently selected row, or
     *                    TableInsertionPoint.END to insert as the last row in the
     *                    table
     *
     * @param ignoreRow   row to ignore when checking for an identical, existing
     *                    association (as is possible when replacing an association,
     *                    if no changes are made); -1 to prevent a duplicate
     *                    association (as when adding an association)
     *
        ...
        try {
            ...
            if (!nameFld.getText().isEmpty()) {
                ...
                for (int row = 0; row < assnsTable.getRowCount(); row++) {
                    ...
                    if (row != ignoreRow && nameFld.getText().equalsIgnoreCase(
                            assnsTable.getValueAt(row, AssociationsTableColumnInfo.NAME.ordinal()).toString())) {
            ...
            if (CcddScriptHandler.getMatchingAssociation(
                    Arrays.asList(CcddUtilities.convertObjectToString(assnsTable.getTableData(false))),
                    scriptFile.getAbsolutePathWithEnvVars(), members.toArray(new String[0]), ignoreRow) != -1) {
    ...
    private List<String[]> createAssociationsFromTable() {
        ...
        for (Object[] assn : assnsTable.getTableData(true)) {
            ...
            currentAssociations.add(new String[] { assn[AssociationsTableColumnInfo.NAME.ordinal()].toString(),
                    ...
                    assn[AssociationsTableColumnInfo.SCRIPT_FILE.ordinal()].toString(), CcddUtilities.removeHTMLTags(
                            assn[AssociationsTableColumnInfo.MEMBERS.ordinal()].toString().replaceAll("<br>", "\n")) });
        }
    ...
    /**********************************************************************************************
     * Update the table tree and the associations table with the latest table and
     * script associations information
     *********************************************************************************************/
