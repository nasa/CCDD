...
public class CcddDbTableCommandHandler {
    ...
    // Character(s) separating table references in the script associations and telemetry scheduler tables,
    ...
    /**********************************************************************************************
     * Enclose database text string objects with a delimiter. The default delimiter
     * is single quotes. If the text contains single quotes or a backslash then $$
     * is used instead, unless the text already contains $$ or ends with $, in which
     * case $_$ is used as the delimiter
     *
     ...
     *
     * @param structuresOnly true to only include structure tables; false to include
     *                       tables of all types
     *
     * @param parent         GUI component over which to center any error dialog
     *
    ...
    /**********************************************************************************************
     * Build the specified data table's comment command. The table's visible table
     * name (preserving case) and its type are stored as a comment on the table
     *
     * @param tableName table name
     *
     ...
     *                    dataType1.variableName1[,...[,dataTypeN,variableNameN]]];
     *                    the first data type/variable name pair is from the root
     *                    table, with each succeeding pair coming from the next
     *                    level down in the structure's hierarchy. The path is the
     ...
     *
     * @return Command to update the specified table's column order for the current
     *         user
     *********************************************************************************************/
    private String buildColumnOrder(String tablePath, String columnOrder) {
        return "DELETE FROM " + InternalTable.ORDERS.getTableName() + " WHERE " + OrdersColumn.USER_NAME.getColumnName()
                ...
                + "'; INSERT INTO " + InternalTable.ORDERS.getTableName() + " ("
                + OrdersColumn.USER_NAME.getColumnName() + ", " + OrdersColumn.TABLE_PATH.getColumnName() + ", "
                + OrdersColumn.COLUMN_ORDER.getColumnName() + ") VALUES ('" + dbControl.getUser() + "', '" + tablePath
                + "', '" + columnOrder + "'); ";
    }
     ...
     *         returned by the database query; returns null if the query produces an
     ...
     *
     * @param primaryKey value of the table's primary key column from which to
    ...
    protected String queryTableCellValue(String tableName, String primaryKey, String columnName, Component parent) {
        ...
        List<String[]> results = queryDatabase(new StringBuilder("SELECT ").append(columnName).append(" FROM ").append(
                tableName).append(" WHERE ").append(DefaultColumn.PRIMARY_KEY.getDbName()).append(" = ").append(
                        primaryKey), parent);
    ...
    /**********************************************************************************************
     * Retrieve a list of all data tables in the database with the names in the
     * user-viewable form. Any non-data tables are ignored
     *
     * @param parent GUI component over which to center any error dialog
     *
    ...
    /**********************************************************************************************
     * Retrieve a list of all data tables in the database along with the table's
     * type. Any non-data tables are ignored
     *
     * @param parent GUI component over which to center any error dialog
     *
     * @return List array containing the table names and table types. The table
     *         names are provided in both the user-viewable form, which preserves
     *         capitalization, and the database form. An empty array is returned if
     *         no tables exist
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Retrieve a list of all table types in the database. Any non-data tables are
     * ignored
     *
     * @param parent GUI component over which to center any error dialog
     *
     ...
     *
     * @param tableType table type by which to filter the table names. The table
    ...
    /**********************************************************************************************
     * Retrieve an array containing the table name, data type, and variable names
     * for the specified structure table types in the format
     * tableName&lt;,dataType.variableName&gt;
     *
     * @param tableTypes comma-separated string of table types
     *
     * @param parent     GUI component over which to center any error dialog
     *
     * @return Array containing the table name, data type, and variable names for
     *         the specified table
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Retrieve a list of comments for all data tables in the database. Any non-data
     * tables are ignored
     *
     * @param parent GUI component over which to center any error dialog
     *
     * @return String array containing the table comment parameters and the table
    ...
    protected String[] queryTableComment(String tableName, int numParts, Component parent) {
        ...
        try {
            ...
            ResultSet comment = dbCommand.executeDbQuery(new StringBuilder("SELECT obj_description('public." + 
                                tableName.toLowerCase() + "'::regclass, 'pg_class');"), parent);
            ...
            eventLog.logFailEvent(parent,
                    "Cannot obtain comment for table '" + tableName + "'; cause '" + se.getMessage() + "'",
    ...
    /**********************************************************************************************
     * Retrieve the comment for the specified data table. The comment contains the
     * table's visible name (preserves case) and type
     *
    ...
    /**********************************************************************************************
     * Retrieve a list of all data table descriptions in the database. Any non-data
     * tables are ignored
     *
     * @param parent GUI component over which to center any error dialog
     *
     * @return String array containing the table names and their variable paths, if
    ...
    protected String queryTableDescription(String tablePath, Component parent) {
        ...
        try {
            command.append("SELECT ").append(ValuesColumn.VALUE.getColumnName()).append(" FROM ")
                   .append(InternalTable.VALUES.getTableName()).append(" WHERE ").append(ValuesColumn.TABLE_PATH.getColumnName())
                   .append(" = '").append(tablePath).append("' AND ").append(ValuesColumn.COLUMN_NAME.getColumnName()).append(" = '';");
     ...
     *
     * @param tablePath comma-separated variable names describing the path to this
     *                  table, beginning with the root table; the names are in the
     *                  order from highest level to lowest level, where the lowest
     *                  level name is the &lt;variable name&gt; portion of the table
    ...
    private String queryColumnOrder(String tablePath, String tableType, Component parent) {
        ...
        try {
            ...
            command.append("SELECT ").append(OrdersColumn.COLUMN_ORDER.getColumnName()).append(" FROM ")
                   .append(InternalTable.ORDERS.getTableName()).append(" WHERE ").append(OrdersColumn.USER_NAME.getColumnName())
                   .append(" = '").append(dbControl.getUser()).append("' AND ").append(OrdersColumn.TABLE_PATH.getColumnName())
                   .append(" = '").append(tablePath).append("';");
     ...
     *
     * @param comments    array of table comments, with each comment divided into
     *                    its component parts
     *
    ...
    protected void setTableComment(String tableName, String comment, Component parent) {
        ...
        try {
            ...
            eventLog.logFailEvent(parent,
                    "Cannot update comment for table '" + tableName + "'; cause '" + se.getMessage() + "'",
     ...
     *
     * @param fieldTableName name of the table in the data field table from which to
     *                       copy
     *
    ...
    /**********************************************************************************************
     * Update the root structure table, variable path and offset, and command lists,
     * and the variable, command, and message ID references. This is needed once one
     * or more tables are created, copied, renamed, or deleted
     *
     * @param parent GUI component over which to center any error dialog
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Create one or more tables of the specified type. This command is executed in
     * a separate thread since it can take a noticeable amount time to complete, and
     * by using a separate thread the GUI is allowed to continue to update. The GUI
     * menu commands, however, are disabled until the database command completes
     * execution
     *
    ...
    /**********************************************************************************************
     * Change the name of a table. Only prototype tables can be renamed using this
     * method (instances are renamed by changing the variable name in the
     * prototype). This command is executed in a separate thread since it can take a
            ...
            protected void execute() {
                try {
                        ...
                        for (InputType inputType : typeDefn.getInputTypesList()) {
                            ...
                            if (inputType.getInputFormat().equals(InputTypeFormat.DATA_TYPE)) {
                                ...
                                for (Integer column : typeDefn.getColumnIndicesByInputType(inputType)) {
                                    ...
                                    command.append("UPDATE ").append(rootTable).append(" SET ").append(dataTypeColName).append(" = '")
                                           .append(newName).append("' WHERE ").append(dataTypeColName).append(" = '").append(tableName)
                                           .append("'; ");
                                }
                            }
                            ...
                            if (inputType.getInputFormat().equals(InputTypeFormat.VARIABLE_REF)) {
                                ...
                                for (Integer column : typeDefn.getColumnIndicesByInputType(inputType)) {
                                    ...
                                    command.append("UPDATE ").append(rootTable).append(" SET ").append(varRefColName).append(" = regexp_replace(")
                                           .append(varRefColName).append(", E'(^|,)").append(tableName).append("(\\\\.|,|$)', E'\\\\1")
                                           .append(newName).append("\\\\2'); ");
                                }
                            }
                        }
                    }
                    ...
                    if (!dbTableName.equals(dbNewName)) {
                        ...
                        command.append("ALTER TABLE ").append(dbTableName).append(" RENAME TO ").append(dbNewName).append("; ");
                    }
                    ...
                    command.append(buildTableDescription(newName, newDescription))
                           .append(renameInfoTableCommand(InternalTable.VALUES, ValuesColumn.TABLE_PATH.getColumnName(), tableName, newName))
                           .append(renameInfoTableCommand(InternalTable.LINKS, LinksColumn.MEMBER.getColumnName(), tableName, newName))
                           .append(renameInfoTableCommand(InternalTable.TLM_SCHEDULER, TlmSchedulerColumn.MEMBER.getColumnName(), tableName, newName))
                           .append(renameInfoTableCommand(InternalTable.GROUPS, GroupsColumn.MEMBERS.getColumnName(), tableName, newName))
                           .append(renameInfoTableCommand(InternalTable.FIELDS, FieldsColumn.OWNER_NAME.getColumnName(), tableName, newName))
                           .append(renameInfoTableCommand(InternalTable.ORDERS, OrdersColumn.TABLE_PATH.getColumnName(), tableName, newName))
                           .append(renameInfoTableCommand(InternalTable.ASSOCIATIONS, AssociationsColumn.MEMBERS.getColumnName(), tableName, newName));
                    ...
                    eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Table '" + tableName + "' renamed to '" + newName + "'"));
                } catch (SQLException se) {
    ...
    /**********************************************************************************************
     * Make a copy of an existing table. Only prototype tables can be copied using
     * this method (instances are copied by adding a new instance and copying the
     * values to it in the table editor). This command is executed in a separate
            ...
            protected void execute() {
                try {
                    ...
                    command.append("CREATE TABLE ").append(dbNewName).append(" (LIKE ").append(dbTableName)
                           .append(" INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING INDEXES); ALTER TABLE ").append(dbNewName)
                           .append(" ALTER ").append(DefaultColumn.PRIMARY_KEY.getDbName()).append(" DROP DEFAULT; CREATE SEQUENCE ")
                           .append(sequenceName).append("; INSERT INTO ").append(dbNewName).append(" SELECT * FROM ").append(dbTableName)
                           .append("; ALTER SEQUENCE ").append(sequenceName).append(" OWNED BY ").append(dbNewName).append(".")
                           .append(DefaultColumn.PRIMARY_KEY.getDbName()).append("; SELECT setval('").append(sequenceName)
                           .append("', (SELECT max(").append(DefaultColumn.PRIMARY_KEY.getDbName()).append(") FROM ").append(dbTableName)
                           .append("), true); ALTER TABLE ").append(dbNewName).append(" ALTER ").append(DefaultColumn.PRIMARY_KEY.getDbName())
                    ...
                    command.append(buildDataTableComment(newName, comment[TableCommentIndex.TYPE.ordinal()]))
                           .append(buildTableDescription(newName, newDescription)).append(buildColumnOrder(newName, columnOrder));
                    ...
                    // Log that renaming the table succeeded
                    eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Table '" + tableName + "' copied to '" + newName + "'"));
                } catch (SQLException se) {
    ...
    /**********************************************************************************************
     * Delete one or more prototype or script tables. The selected tables(s) are
     * deleted from the database, all references to the table are deleted from the
     * custom values table, and any open editors for tables of this prototype are
     * closed. This command is executed in a separate thread since it can take a
     * noticeable amount time to complete, and by using a separate thread the GUI is
     * allowed to continue to update. The GUI menu commands, however, are disabled
     * until the database command completes execution
     *
     ...
     *
     * @param dialog     reference to the table manager dialog calling this method;
     ...
     * @param tableNames  array of names of the tables to delete
     *
     * @param isDataTable true if the table(s) to be deleted are data tables; false
     *                    if the tables are scripts or internal tables
     *
    ...
    private String deleteTableCommand(String[] tableNames, boolean isDataTable) {
        ...
        if (isDataTable) {
            StringBuilder valuesDelCmd = new StringBuilder("; DELETE FROM ").append(InternalTable.VALUES.getTableName()).append(" WHERE ");
            StringBuilder linksDelCmd = new StringBuilder("; DELETE FROM ").append(InternalTable.LINKS.getTableName()).append(" WHERE ");
            StringBuilder tlmSchDelCmd = new StringBuilder("; DELETE FROM ").append(InternalTable.TLM_SCHEDULER.getTableName()).append(" WHERE ");
            StringBuilder groupsDelCmd = new StringBuilder("; DELETE FROM ").append(InternalTable.GROUPS.getTableName()).append(" WHERE ");
            StringBuilder fieldsDelCmd = new StringBuilder("; DELETE FROM ").append(InternalTable.FIELDS.getTableName()).append(" WHERE ");
            StringBuilder ordersDelCmd = new StringBuilder("; DELETE FROM ").append(InternalTable.ORDERS.getTableName()).append(" WHERE ");
            ...
            for (String name : tableNames) {
                // Add the table to the commands
                infoCmd.append("col1 ~ E'^").append(name).append("$' OR col1 ~ E'^").append(name).append(",' OR col1 ~ E',").append(name).append("\\\\.' OR ");
                ...
                assnsUpdCmd.append("; UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                           ...
                           .append(assnsSeparator).append("|").append(assnsSeparator).append("(?:^| \\+ |").append(PATH_IDENT)
                           .append(",)?").append(name).append("(?:(?:,|\\\\.)").append(PATH_IDENT).append(")?)', E'', 'g'); UPDATE ")
                           .append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ").append(AssociationsColumn.MEMBERS.getColumnName())
                           .append(" = '' WHERE ").append(AssociationsColumn.MEMBERS.getColumnName()).append(" ~ E'^(?:").append(PATH_IDENT)
                           .append(",)?").append(name).append("(?:(?:,|\\\\.)").append(PATH_IDENT).append(")?$'");
            }
            ...
            tlmSchDelCmd.append(infoCmd.toString().replaceAll("col1", TlmSchedulerColumn.MEMBER.getColumnName()).replaceAll("\\^",
                    "^\\\\\\\\d+\\\\\\\\.\\\\\\\\d+" + "\\\\\\\\\\\\\\" + TLM_SCH_SEPARATOR));
    ...
    /**********************************************************************************************
     * Load the contents of a single database table and display the data in a table
     * editor. Do not load the table if it is already open for editing. If the table
     * is already open, then the editor for this table is brought to the foreground.
     * This command is executed in a separate thread since it can take a noticeable
     ...
     *
     * @param currentEditorDlg editor dialog to open the table in; null to open the
    ...
    /**********************************************************************************************
     * Load the contents of an array of database tables and display the data in a
     * table editor. Do not load tables that are already open for editing. If no
     * table is loaded, but one in the list is already open, then the editor for
     * this table is brought to the foreground. This command is executed in a
     * separate thread since it can take a noticeable amount time to complete, and
     * by using a separate thread the GUI is allowed to continue to update. The GUI
     * menu commands, however, are disabled until the database command completes
     * execution
     *
     ...
     *                         level down in the structure's hierarchy
     *
     * @param callingEditorDlg table editor dialog in which to open the table; null
     *                         to open the table in a new editor dialog
     *
     * @return SwingWorker reference for this thread
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Perform the database query to load the contents of a database table. The data
     * is sorted in ascending numerical order based on the index (primary key)
     * column
     *
     * @param tablePath       table path in the format
     *                        rootTable[,dataType1.variable1[,dataType2
     ...
     *                        type/variable name pair is from the root table, with
     *                        each succeeding pair coming from the next level down
     ...
     * @param parent          GUI component over which to center any error dialog
     *
     * @return TableInformation class containing the table data from the database.
    ...
    /**********************************************************************************************
     * Perform the database query to load the rows from the custom values table that
     * match the specified column name and column value
     *
     * @param columnName  name to match in the custom values table 'column name'
     *                    column
     *
     * @param columnValue value to match in the custom values table 'value' column;
     *                    null or blank to match any value
     *
     * @param parent      GUI component over which to center any error dialog
     *
     * @return List containing arrays with the row data (table path, column name,
     *         and value) from the custom values table for those rows that match the
    ...
    protected List<String[]> getCustomValues(String columnName, String columnValue, Component parent) {
        ...
        try {
            ...
            command.append("SELECT * FROM ").append(InternalTable.VALUES.getTableName()).append(" WHERE ")
                   ...
                   .append((columnValue == null || columnValue.isEmpty() ? "" : " AND " + ValuesColumn.VALUE.getColumnName()
                   + " = '" + columnValue + "'")).append(";");
            ResultSet rowData = dbCommand.executeDbQuery(command, parent);
    ...
    /**********************************************************************************************
     * Store the rate parameters in the project database and update the database
    ...
    /**********************************************************************************************
     * Create a list of all prototype tables with their child tables (prototypes and
     * instances), and primitive variables (if specified). The table must contain
     * all of the protected columns defined for a structure in order for its members
     * to be determined. Non-structure tables are included in the returned list, but
     * by definition have no members
     *
     * @param memberType Type of table members to load: TABLES_ONLY to exclude
     *                   primitive variables or INCLUDE_PRIMITIVES to include tables
     *                   and primitive variables
     *
     * @param sortByName true to return the table members in alphabetical order
     *                   (e.g., for use in a tree); false to return the members
     *                   sorted by row index (e.g., for use in determining the
     *                   variable offsets in the structure)
     *
     * @param parent     GUI component over which to center any error dialog
     *
     * @return List containing the table member information. For structure tables
     *         the member tables are included, along with primitive variables (if
     *         specified), sorted by variable name or row index as specified
     *********************************************************************************************/
    protected List<TableMembers> loadTableMembers(TableMemberType memberType, boolean sortByName,
        ...
        try {
            ...
            ResultSet rowData = dbCommand.executeDbQuery(new StringBuilder("SELECT * FROM ").append(sortByName ?
                    "get_table_members_by_name();" : "get_table_members_by_index();"), parent);
            ...
            while (doLoop) {
                    ...
                    if (dataType != null && !dataType.isEmpty() && variableName != null && !variableName.isEmpty()
                        ...
                        if (addIndex != 0 && sortByName) {
                            ...
                            if (ArrayVariable.isArrayMember(variableName) && ArrayVariable.isArrayMember(lastVarName)
                                    && ArrayVariable.removeArrayIndex(variableName) .equals(ArrayVariable.removeArrayIndex(lastVarName))) {
                ...
                tableMembers.add(new TableMembers(comment[TableCommentIndex.NAME.ordinal()],
                        comment[TableCommentIndex.TYPE.ordinal()], dataTypes, variableNames, bitLengths, rates));
            }
            ...
            for (String tableName : tableNames) {
                ...
                if (!isFound) {
                    // Grab the table info
                    TableInformation tableInfo = ccddMain.getDbTableCommandHandler().loadTableData(tableName.toLowerCase(), false, true, parent);
    ...
    /**********************************************************************************************
     * Add, modify, and/or delete data in a table. This command is executed in a
     * separate thread since it can take a noticeable amount time to complete, and
     * by using a separate thread the GUI is allowed to continue to update. The GUI
     * menu commands, however, are disabled until the database command completes
     * execution
     ...
     *
     * @param forceUpdate        true to make the changes to other tables; false to
     *                           only make changes to tables other than the one in
     *                           which the changes originally took place
     *
     * @param skipInternalTables true to not build and execute the commands to
     ...
     *
     * @param updateDescription  true to update the table description from the table
     *                           information; false to not change the table
     *                           description
     *
     * @param updateColumnOrder  true to update the table column order from the
     *                           table information; false to not change the table
     *                           column order
     *
     * @param updateFieldInfo    true to update the table data fields from the table
     *                           information; false to not change the table data
     *                           fields
     *
     * @param newDataTypeHandler data type handler with data type modifications.
    ...
    /**********************************************************************************************
     * Add, modify, and/or delete data in a table. If the table is a prototype then
     * its database table is altered; if the table is an instance then the changes
     * are made to the custom values table
     *
     * @param tableInfo          table information
     *
     * @param additions          list of row addition information
     ...
     * @param deletions          list of row deletion information
     *
     * @param forceUpdate        true to make the changes to other data tables;
     *                           false to only make changes to tables other than the
     *                           one in which the changes originally took place
     *
     * @param skipInternalTables true to not build and execute the commands to
     *                           update the internal tables. This is used during a
     *                           data type update where only the data type name has
    ...
    protected boolean modifyTableData(TableInformation tableInfo, List<TableModification> additions,
        ...
        try {
            ...
            if (updateMessageNameAndIDReference(tableInfo, typeDefinition, modifications, deletions, msgIDRefChk, parent)) {
                commandExecuted = true;
            }
            ...
            if (commandExecuted) {
                ...
                eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Table '" + tableInfo.getProtoVariableName() + "' data modified"));
            }
        } catch (SQLException se) {
            // Inform the user that updating the table failed
            eventLog.logFailEvent(parent,
                    "Cannot modify data in table '" + tableInfo.getProtoVariableName() + "'; cause '" + se.getMessage()
                            + "'",
                    "<html><b>Cannot modify data in table '</b>" + tableInfo.getProtoVariableName() + "<b>'");
            errorFlag = true;
        } catch (Exception e) {
            // Display a dialog providing details on the unanticipated error
            CcddUtilities.displayException(e, parent);
    ...
    private boolean buildAndExecuteAdditionCommand(TableInformation tableInfo, List<TableModification> additions,
        ...
        addCmd.append("INSERT INTO ").append(dbTableName).append(" (")
              .append(CcddUtilities.convertArrayToString(typeDefn.getColumnNamesDatabaseQuoted())).append(") VALUES ");
     ...
     *
     * @param dbTableName        name of the table's prototype to which to modify
     *                           rows
     *
     * @param typeDefn           table type definition
     *
     ...
     *                           data type editor
     *
     * @param tableTree          CcddTableTreeHandler reference describing the table
     *                           tree
     *
     * @param skipInternalTables true to not build and execute the commands to
     *                           update the internal tables. This is used during a
            ...
            if (tableInfo.isPrototype()) {
                    ...
                    if (variableChanged || dataTypeChanged || arraySizeChanged || bitLengthChanged || rateChanged) {
                        ...
                        if (dataTypeChanged && !newDataTypeHandler.isPrimitive(newDataType)
                            ...
                            valuesModCmd.append("UPDATE ").append(InternalTable.VALUES.getTableName()).append(" SET ")
                                        .append(ValuesColumn.TABLE_PATH.getColumnName()).append(" = regexp_replace(")
                                        .append(ValuesColumn.TABLE_PATH.getColumnName()).append(", E'^" + newDataType + ",', E'")
                            ...
                            ordersModCmd.append("UPDATE ").append(InternalTable.ORDERS.getTableName()).append(" SET ")
                                        ...
                                        .append(OrdersColumn.TABLE_PATH.getColumnName()).append(", E'^").append(newDataType)
                                        .append("', E'").append(newVariablePath).append("'), ").append(OrdersColumn.COLUMN_ORDER.getColumnName())
                                        .append(" FROM ").append(InternalTable.ORDERS.getTableName()).append(" WHERE ")
                                        .append(OrdersColumn.TABLE_PATH.getColumnName()).append(" = '").append(newDataType).append("'; ");
                        ...
                        if (tablePathList == null) {
                            ...
                            if (dataTypeChanged && !dataTypeHandler.isPrimitive(oldDataType)) {
                                ...
                                tablePathList.addAll(tableTree.getTableTreePathArray(tableInfo.getPrototypeName(),
                                        tableTree.getNodeByNodeName(DEFAULT_PROTOTYPE_NODE_NAME), -1));
                            }
                        }
                        ...
                        try {
                            ...
                            if (!wasShifted) {
                                for (Object[] path : tablePathList) {
                                    ...
                                    if ((variableChanged || (dataTypeChanged && dataTypeHandler.isPrimitive(oldDataType) && 
                                            newDataTypeHandler.isPrimitive(newDataType))) && !(arraySizeChanged && 
                                            (oldArraySize.isEmpty() || newArraySize.isEmpty()))) {
                                        // Create the commands to update the internal tables for instances of non-array member variables of the prototype
                                        // table
                                        ...
                                        for (int index = 0; index < assnsMemberTables.length; index++) {
                                            if (assnsMemberTables[index].contains((String)path[path.length-1])) {
                                                ...
                                                .append(")").append(orgPathWithChildren).append(")', E'\\\\2").append(newVariablePath)
                                                .append("\\\\1\\\\3', 'ng'); ");
                                    ...
                                    if (dataTypeChanged && variableStillExists && !dataTypeHandler.isPrimitive(oldDataType)) {
                                        ...
                                        valuesModCmd.append("DELETE FROM ").append(InternalTable.VALUES.getTableName()).append(" WHERE ")
                                                    .append(ValuesColumn.TABLE_PATH.getColumnName()).append(" ~ E'^").append(orgVarPathEsc)
                                                    .append(",'; ").append(updateVarNameAndDataType(orgVarPathEsc, newVariablePath,
                                                        InternalTable.VALUES.getTableName(),
                                        ...
                                        groupsModCmd.append("DELETE FROM ").append(InternalTable.GROUPS.getTableName()).append(" WHERE ")
                                                    .append(GroupsColumn.MEMBERS.getColumnName()).append(" ~ E'^").append(pathMatch)
                                                    .append("'; ");
                                        ...
                                        assnsModCmd.append("UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                                                   .append(AssociationsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                                                   .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'^" + orgPathWithChildren)
                                    ...
                                    if (arraySizeChanged && !variableStillExists && 
                                        ...
                                        groupsModCmd.append("DELETE FROM ").append(InternalTable.GROUPS.getTableName()).append(" WHERE ")
                                                    .append(GroupsColumn.MEMBERS.getColumnName()).append(" ~ E'^").append(pathMatch).append("'; ");
                                        
                                        fieldsModCmd.append("DELETE FROM ").append(InternalTable.FIELDS.getTableName()).append(" WHERE ")
                                                    .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" ~ E'^").append(pathMatch).append("'; ");
                                        
                                        ordersModCmd.append("DELETE FROM ").append(InternalTable.ORDERS.getTableName()).append(" WHERE ")
                                                    .append(OrdersColumn.TABLE_PATH.getColumnName()).append(" ~ E'^").append(pathMatch).append("'; ");
                                        ...
                                        if (!newArraySize.isEmpty()) {
                                            ...
                                            assnsModCmd.append("UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                                                       .append(AssociationsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                                                       .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'^").append(orgVarPathEsc)
                                                       .append("(?:").append(assnsSeparator).append("|,").append(PATH_IDENT).append("|$)', E'', 'ng'); ");
                                        }
                            ...
                            e.printStackTrace();
                            eventLog.logFailEvent(parent,
                                    "Building command failed cause " + e.getMessage() + ".",
                                    "<html><b>SQL command failed</b>");
                        }
                ...
                // Check if this is a command table
                    ...
                    if (!oldCommandName.equals(newCommandName) || !oldCommandCode.equals(newCommandCode)
                        ...
                        // arguments change
                        modifyCommandReference(tableInfo.getTablePath(), oldCommandName, newCommandName,
                                oldCommandCode, newCommandCode, oldCommandArg, newCommandArg, cmdRefChkResults,
                ...
                modCmd = CcddUtilities.removeTrailer(modCmd, ", ").append(" WHERE ").append(typeDefn.getColumnNamesDatabase()[
                          DefaultColumn.PRIMARY_KEY.ordinal()]).append(" = ").append(mod.getRowData()[DefaultColumn.PRIMARY_KEY.ordinal()]).append("; ");
            }
                    ...
                    if (!mod.getOriginalRowData()[column].equals(mod.getRowData()[column])) {
                        ...
                        if (!mod.getRowData()[column].toString().startsWith(REPLACE_INDICATOR)) {
                            modCmd.append(" INSERT INTO ").append(InternalTable.VALUES.getTableName()).append(" (")
                                  .append(ValuesColumn.TABLE_PATH.getColumnName()).append(", ").append(ValuesColumn.COLUMN_NAME.getColumnName())
                                  .append(", ").append(ValuesColumn.VALUE.getColumnName()).append(") VALUES ('").append(variablePath).append("', '")
                                  .append(typeDefn.getColumnNamesUser()[column]).append("', ").append(delimitText(mod.getRowData()[column])).append("); ");
                        }
                    }
                }
                ...
                for (int rateIndex : mod.getRateColumn()) {
                    ...
                    if (!oldRate.equals(newRate)) {
                        ...
                        deleteLinkAndTlmPathRef(mod.getRowData()[mod.getArraySizeColumn()].toString(),
                                mod.getRowData()[mod.getVariableColumn()].toString(), variablePath, variablePathEsc,
                                linksDelCmd, tlmDelCmd);
                        break;
                    }
                }
            }
    ...
    /**********************************************************************************************
     * Build the command to delete a table row. Only prototype tables can have a row
     * deleted
     *
     * @param tableInfo          table information
     *
    ...
    private boolean buildAndExecuteDeletionCommand(TableInformation tableInfo, List<TableModification> deletions,
        ...
        for (TableModification del : deletions) {
            ...
            delCmd.append("DELETE FROM ").append(dbTableName).append(" WHERE ").append(typeDefn.getColumnNamesDatabase()[DefaultColumn.PRIMARY_KEY.ordinal()])
                  .append(" = ").append(del.getRowData()[DefaultColumn.PRIMARY_KEY.ordinal()]).append("; ");
            ...
            if (!skipInternalTables && typeDefn.isStructure()) {
                ...
                if (!dataTypeHandler.isPrimitive(dataType)) {
                    ...
                    assnsDelCmd.append("UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                               .append(AssociationsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                               .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'^").append(protoPathWithChildren)
                               .append("', E'', 'ng'); UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                               .append(AssociationsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                               .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'^").append(instancePathWithChildren)
                               .append("', E'', 'ng'); ");
                }
            ...
            // Check if this is a command table
                ...
                deleteCommandReference(del.getRowData()[typeDefn.getColumnIndexByInputType(DefaultInputType.COMMAND_NAME)].toString(),
                        cmdRefChkResults, valuesDelCmd, fieldsDelCmd);
            }
        }
        ...
        delCmd.append(valuesDelCmd).append(groupsDelCmd).append(fieldsDelCmd).append(ordersDelCmd)
              .append(assnsDelCmd).append(linksDelCmd).append(tlmDelCmd);
    ...
    /**********************************************************************************************
     * Build the command to update the variable name and/or the data type. Combine
     * updates to array members into a single command by using a regular expression
     * to match the array indices
     *
     ...
     *
     * @param columnName      name of the column in the internal table that contains
     *                        the variable path
     *
     * @param captureIn       pattern match to capture any character(s) that
     *                        precede(s) the variable path; blank if none
     *
     * @param captureOut      pattern for replacing the captured character(s)
     *                        specified in captureIn; blank if none
     *
    ...
    private String updateVarNameAndDataType(String orgVariablePath, String newVariablePath, String tableName,
            String columnName, String captureIn, String captureOut, boolean includeChildren) {
        StringBuilder command = new StringBuilder();
        ...
        if (orgVariablePath.contains("[")) {
            ...
            if (!orgVariablePath.matches(".+\\[[1-9].*")) {
                ...
                command.append("UPDATE ").append(tableName).append(" SET ").append(columnName).append(" = regexp_replace(")
                       .append(columnName).append(", E'^").append(captureIn).append(orgVariablePath).append((includeChildren ? "(,.*|$)" : "$"))
                       .append("', E'").append(captureOut).append(newVariablePath).append((includeChildren ? "\\\\" + captureGrp : ""))
                       .append("'); ");
            }
        }
        // The path doesn't contain an array member
            ...
            command.append("UPDATE ").append(tableName).append(" SET ").append(columnName).append(" = regexp_replace(")
                   .append(columnName).append(", E'^").append(captureIn).append(orgVariablePath).append((includeChildren ?
                           "(,.*|:\\\\d+|$)" : "$")).append("', E'").append(captureOut).append(newVariablePath)
                   .append((includeChildren ? "\\\\" + captureGrp : "")).append("'); ");
        }
    ...
    /**********************************************************************************************
     * Create a copy of the specified table's table tree node and immediate child
     * nodes
     *
     * @param tableName table name
     *
     ...
     *
     * @return Copy of the specified table's table tree node and immediate child
     *         nodes (if any)
     *********************************************************************************************/
    private ToolTipTreeNode copyPrototypeTableTreeNode(String tableName, CcddTableTreeHandler tableTree) {
        ...
        if (tableNode != null) {
            ...
            for (int childIndex = 0; childIndex < tableNode.getChildCount(); childIndex++) {
                ...
                orgTableNode.add(new ToolTipTreeNode(
                        ((ToolTipTreeNode) tableNode.getChildAt(childIndex)).getUserObject().toString(), ""));
            }
        }
    ...
    /**********************************************************************************************
     * Build the commands to delete variable references in the links and telemetry
     * scheduler tables that are no longer valid due to changes in bit-packing
     *
     * @param orgTableNode reference to the node prior to the update
     *
     * @param parent       GUI component over which to center any error dialog
     *
     * @return Commands to delete variable references in the links and telemetry
     *         scheduler tables that are no longer valid due to changes in
     *         bit-packing
     *********************************************************************************************/
        ...
        if (updTableNode != null) {
            ...
            for (int childIndex = 0; childIndex < updTableNode.getChildCount(); childIndex++) {
                ...
                if (nodeIndex.getFirstIndex() != nodeIndex.getLastIndex()) {
                    ...
                    for (int packIndex = nodeIndex.getFirstIndex(); packIndex <= nodeIndex
                            .getLastIndex(); packIndex++) {
    ...
    /**********************************************************************************************
     * Update the link definitions when a string variable's size is increased (i.e.,
     * a new member is added to the array)
     *
    ...
    /**********************************************************************************************
     * Build the command to delete the specified table/variable references from the
     * links table
     *
     * @param linksPath table/variable path to remove from the links table. Leading
     *                  or trailing regular expression constructs must surround the
     *                  path, and any reserved PostgreSQL characters in the path
     *                  must be escaped
     *
     * @param linksCmd  StringBuilder containing the existing links table deletion
    ...
    /**********************************************************************************************
     * Build the commands to delete the specified table/variable references from the
     * telemetry scheduler table
     *
     * @param tlmPath table/variable path to remove from the telemetry scheduler
     ...
     *
     * @param tlmCmd  StringBuilder containing the existing telemetry scheduler
     ...
     * telemetry scheduler table
     *
     * @param variablePaths list of variable paths to remove from the telemetry
     *                      scheduler table
     *
     ...
     * telemetry scheduler tables. If the variable is an array definition then amend
     * the regular expression to match all members of the array; skip any subsequent
     * members of the array passed to this method
     *
     ...
     *
     * @param variablePathEsc variable path with any PostgreSQL reserved characters
     *                        escaped so that the path can be used in a regular
     *                        expression
     *
     * @param linksDelCmd     StringBuilder containing the existing links table
     *                        deletion command
     *
     * @param tlmDelCmd       StringBuilder containing the existing telemetry
     *                        scheduler table deletion command
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Build the command to remove all references for the specified rate column name
     * in tables of the changed table type in the links and telemetry scheduler
     * tables
     *
     * @param rateColName rate column name
     *
     * @param linksCmd    sub-command for references in the links table to tables of
     *                    the target type
     *
     * @param tlmSchCmd   sub-command for references in the telemetry scheduler
     *                    table to tables of the target type
     *
     * @return Command to remove all references for the specified rate column name
     *         in tables of the changed table type in the links and telemetry
     *         scheduler tables
     *********************************************************************************************/
    private String deleteLinkAndTlmRateRef(String rateColName, StringBuilder linksCmd, StringBuilder tlmSchCmd) {
        ...
        command.append("DELETE FROM ").append(InternalTable.LINKS.getTableName()).append(" WHERE ")
               .append(LinksColumn.RATE_NAME.getColumnName()).append(" = E'").append(rateColName)
               .append("'").append(linksCmd).append("; DELETE FROM ").append(InternalTable.TLM_SCHEDULER.getTableName())
               .append(" WHERE ").append(TlmSchedulerColumn.RATE_NAME.getColumnName()).append(" = '").append(rateColName)
               .append("'").append(tlmSchCmd).append("; ");
        
        return command.toString();
    }

    /**********************************************************************************************
     * Build the command to remove all references to the specified rate column name
     * in the links and telemetry scheduler tables
     *
     * @param rateColName rate column name
     *
     * @return Command to remove all references to the specified rate column name in
     *         the links and telemetry scheduler tables
     *********************************************************************************************/
    private String deleteAllLinkAndTlmRateRefs(String rateColName) {
        ...
        command.append("DELETE FROM ").append(InternalTable.LINKS.getTableName()).append(" WHERE ")
               ...
               .append("'; DELETE FROM ").append(InternalTable.TLM_SCHEDULER.getTableName())
               .append(" WHERE ").append(TlmSchedulerColumn.RATE_NAME.getColumnName()).append(" = '")
               .append(rateColName).append("'; ");
        
        return command.toString();
    }

    /**********************************************************************************************
     * Update any references to a variable that had its name changed in every table
     * cell and data field with a variable reference input type
     *
     ...
     * @param varRefChkResults results of the variable reference input type search
     *
     * @param valuesModCmd     StringBuilder containing the existing values table
    ...
    private void modifyVariableReference(TableInformation tableInfo, String oldVariableName, String newVariableName,
        ...
        String match = "(^|.+," + protoTypeName + ".[^,]+,)" + oldDataType + "\\."
                + escapedOldVariableName + "(,.+|$)";
        ...
        if (rootStructures.contains(tableInfo.getProtoVariableName())) {
            targetVars.add(new String[] {
                    CcddUtilities.escapePostgreSQLReservedChars(tableInfo.getPrototypeName() + "," + oldVariableName),
                    ...
                    if (entry.contains(escapedOldVariableName)) {
                        if (entry.matches(match)) {
                            ...
                            if (!targetVars.contains((Object)targetVar)) {
                                ...
                                targetVars.add(new String[] { targetVar, variableHandler.removeDataTypeFromVariablePath(
                                        entry.replaceFirst(match, "$1" + newDataType + "." + newVariableName + "$2")) });
                            }
                        }
                    }
                }
                ...
                for (String[] targetVar : targetVars) {
                    ...
                    if (!rootStructures.contains(table)) {
                        ...
                        valuesModCmd.append("UPDATE ").append(InternalTable.VALUES.getTableName()).append(" SET ")
                                    ...
                                    .append(ValuesColumn.VALUE.getColumnName()).append(" = E'").append(targetVar[0]).append("'; ");
                    }
                }
            }
        }
    ...
    /**********************************************************************************************
     * Blank any references to a variable that has been deleted in every table cell
     * and data field with a variable reference input type
     *
     ...
     * @param varRefChkResults results of the variable reference input type search
     *
     * @param valuesDelCmd     StringBuilder containing the existing values table
     *                         deletion command
     *
     * @param fieldsDelCmd     StringBuilder containing the existing fields table
     *                         deletion command
     *********************************************************************************************/
    private void deleteVariableReference(String protoTableName, String variableName,
            ReferenceCheckResults varRefChkResults, StringBuilder valuesDelCmd, StringBuilder fieldsDelCmd) {
            ...
            for (String table : varRef.getTables()) {
                ...
                if (!rootStructures.contains(table)) {
                    ...
                    valuesDelCmd.append("UPDATE ").append(InternalTable.VALUES.getTableName()).append(" SET ")
                                ...
                                .append(ValuesColumn.TABLE_PATH.getColumnName()).append(" ~ E'^.+,")
                                .append(table).append("\\\\..*$' AND ").append(ValuesColumn.COLUMN_NAME.getColumnName())
                                .append(" = '").append(varRef.getColumnVisible()).append("' AND ")
                                .append(ValuesColumn.VALUE.getColumnName()).append(" ~ E'")
                                .append(instanceVarPathEscNoDt).append("'; ");
                }
            }
        }
    ...
    /**********************************************************************************************
     * Update any references to a command that had its name, code, or argument path
     * changed in every table cell and data field with a command reference input
     * type
     *
    ...
    private void modifyCommandReference(String tablePath, String oldCommandName, String newCommandName,
            String oldCommandCode, String newCommandCode, String oldCommandArg, String newCommandArg,
            ReferenceCheckResults cmdRefChkResults, StringBuilder valuesModCmd, StringBuilder fieldsModCmd) {
        ...
        if (cmdRefChkResults.isFieldUsesType()) {
            ...
            fieldsModCmd.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                        ...
                        .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = E'").append(oldCmdRef)
                        .append("'; ");
        }
    }

    /**********************************************************************************************
     * Blank any references to a command that has been deleted in every table cell
     * and data field with a command reference input type
     *
    ...
    private void deleteCommandReference(String commandName, ReferenceCheckResults cmdRefChkResults,
        ...
        if (cmdRefChkResults.isFieldUsesType()) {
            ...
            fieldsDelCmd.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                        ...
                        .append(FieldsColumn.FIELD_TYPE.getColumnName()).append(" = E'")
                        .append(DefaultInputType.COMMAND_REFERENCE.getInputName()).append("' AND ")
                        .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" ~ E'")
                        .append(commandName).append("'; ");
        }
    }

    /**********************************************************************************************
     * Build the commands to update any references to a message name or ID that has
     * been modified or deleted in every table cell and data field with a message
     * name and ID reference input type
     *
     * @param tableInfo          table information
     *
     ...
     * @param deletions          list of row deletion information
     *
     * @param msgIDRefChkResults results of the message name and ID reference input
     *                           type search
     *
            ...
            for (int row = 0; row < tableInfo.getData().length; row++) {
                ...
                if (!isModDel) {
                    ...
                    for (TableModification del : deletions) {
                        ...
                        if (row == Integer.valueOf(del.getRowData()[DefaultColumn.ROW_INDEX.ordinal()].toString())
                                - 1) {
                            // Step through each message name and ID column
                            for (Integer nameCol : msgColumns) {
            ...
            for (int index = 0; index < msgNameIDs.size(); index++) {
                ...
                if (!msgNameIDs.get(index).isEmpty()) {
                    ...
                    String newMsgNameIDOwner = newMsgNameIDs.get(index).isEmpty() ? ""
                            : newNameAndID[0] + " (ID: " + newNameAndID[1] + ", owner: " + tableInfo.getTablePath()
                                    + ")";
                    ...
                    if (msgIDRefChkResults.isFieldUsesType()) {
                        ...
                        msgIDCmd.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                                .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = '").append(newMsgNameIDOwner)
                                .append("' WHERE ").append(FieldsColumn.FIELD_TYPE.getColumnName()).append(" = E'")
                                .append(DefaultInputType.MESSAGE_REFERENCE.getInputName()).append("' AND ")
                                .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = E'").append(msgNameIDOwner).append("'; ");
                    }
                }
            }
    ...
    /**********************************************************************************************
     * Retrieve a list of internal table data from the database, ignoring script
     * file information tables
     *
     * @param intTable   type of internal table to retrieve
     *
     * @param includeOID true to read in the OID column in addition to the
     *                   information table columns
     *
     * @param parent     GUI component over which to center any error dialog
     *
     * @return List of the items in the internal table. An empty list is returned if
     *         the specified table is empty or doesn't exist
     *********************************************************************************************/
     ...
     *                   information table columns
     *
     * @param scriptName script file name; ignored for non-script file information
     *                   tables
     *
     * @param parent     GUI component over which to center any error dialog
     *
    ...
    protected List<String[]> retrieveInformationTable(InternalTable intTable, boolean includeOID, String scriptName,
        ...
        try {
            ...
            if (isTableExists(intTableName, parent)) {
                ...
                ResultSet infoData = dbCommand.executeDbQuery(new StringBuilder("SELECT *").append((includeOID ? ", OID" : ""))
    ...
    /**********************************************************************************************
     * Store the internal table into the database. This command is executed in a
     * separate thread since it can take a noticeable amount time to complete, and
     * by using a separate thread the GUI is allowed to continue to update. The GUI
     * menu commands, however, are disabled until the database command completes
     * execution
     ...
     *
     * @param fieldInformationList list containing the list of data field
     *                             information for each group with a data field
     *                             update (only applicable to the groups table);
     *                             null if none
     *
     * @param deletedGroups        list containing the names of groups that have
     *                             been deleted
     *
     * @param invalidLinkVars      list containing the link member variables that
     *                             should be removed from the telemetry scheduler
     *                             due to the addition of one or more variables;
     *                             null or an empty list if there are no invalid
     *                             link members
     *
     * @param tableComment         table comment; null if unchanged
     *
     ...
     *********************************************************************************************/
    protected void storeInformationTableInBackground(final InternalTable intTable, final List<String[]> tableData,
            final List<List<FieldInformation>> fieldInformationList, final List<String> deletedGroups,
            final List<String> invalidLinkVars, final String tableComment, final Component parent) {
        // Execute the command in the background
     ...
     * @param intTable     type of internal table to store
     *
     * @param tableData    array containing the table data to store; unused for the
     *                     table types internal table
     *
     * @param tableComment table comment; null if unchanged
     *
     * @param parent       GUI component over which to center any error dialog
    ...
    protected void updateGroupsTable(List<List<FieldInformation>> fieldInformationList, List<String> deletedGroups,
        ...
        try {
            ...
            // Inform the user that the database command failed
            eventLog.logFailEvent(parent,
                    "Cannot store internal table '" + InternalTable.GROUPS.getTableName() + "'; cause '" + se.getMessage() + "'",
                    "<html><b>Cannot store internal table '</b>" + InternalTable.GROUPS.getTableName() + "<b>'");
        }
    }
     ...
     * @param intTable             type of internal table to store
     *
     * @param tableData            list containing the table data to store; unused
     *                             for the table types internal table
     *
     * @param fieldInformationList list containing the list of data field
     *                             information for each group with a data field
     *                             update (only applicable to the groups table);
    ...
    protected void storeInformationTable(InternalTable intTable, List<String[]> tableData,
            List<List<FieldInformation>> fieldInformationList, List<String> deletedGroups, List<String> invalidLinkVars,
    ...
    /**********************************************************************************************
     * Build the command for storing the groups, script associations, links table,
     * data fields, or script
     *
     * @param intTable     type of internal table to store
     *
     * @param tableData    array containing the table data to store
        ...
        try {
            ...
            if (tableComment == null) {
                ...
                ResultSet comment = dbCommand.executeDbQuery(new StringBuilder("SELECT obj_description('public.").append(tableName)
                          .append("'::regclass, 'pg_class');"), parent);
            ...
            if (tableComment != null) {
                ...
                // recreated
                command.append("COMMENT ON TABLE ").append(tableName).append(" IS ").append(delimitText(tableComment)).append("; ");
            }
        } catch (SQLException se) {
            // Inform the user that loading the internal table comment failed
            eventLog.logFailEvent(parent,
                    "Cannot obtain comment for internal table '" + tableName + "'; cause '" + se.getMessage() + "'",
    ...
    protected String storeTableTypesInfoTableCommand() {
        ...
        for (TypeDefinition typeDefn : tableTypeHandler.getTypeDefinitions()) {
            ...
            for (int index = 0; index < typeDefn.getColumnNamesDatabase().length; index++) {
                ...
                command.append("(").append(delimitText(typeDefn.getName())).append(", ").append(index)
                       .append(", '").append(typeDefn.getColumnNamesDatabase()[index]).append("', '")
                       .append(typeDefn.getColumnNamesUser()[index]).append("', ").append(delimitText(typeDefn.getColumnToolTips()[index]))
                       .append(", '").append(typeDefn.getInputTypes()[index].getInputName()).append("', ")
     ...
     *
     * @param fieldInformation project data field information; null to delete the
     ...
     * specified data table, table type, group, or project
     *
     * @param ownerName        name of the table, including the path if this table
     *                         represents a structure, table type, group, or project
     *                         to which the field(s) belong
     *
     * @param fieldInformation field information; null to delete the fields for the
     *                         specified table/table type/group
     *
     * @return Command for updating the table/table type/group fields in the data
     *         field table
     *********************************************************************************************/
    private String modifyFieldsCommand(String ownerName, List<FieldInformation> fieldInformation) {
        ...
        StringBuilder command = new StringBuilder("DELETE FROM ").append(InternalTable.FIELDS.getTableName()).append(" WHERE ")
                .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = '").append(ownerName).append("'; ");
        ...
        if (fieldInformation != null && !fieldInformation.isEmpty()) {
            ...
            for (FieldInformation fieldInfo : fieldInformation) {
                ...
                command.append("('").append(ownerName).append("', ").append(delimitText(fieldInfo.getFieldName())).append(", ")
                       .append(delimitText(fieldInfo.getDescription())).append(", ").append(fieldInfo.getSize()).append(", ")
                       .append(delimitText(fieldInfo.getInputType().getInputName())).append(", ").append(String.valueOf(fieldInfo.isRequired()))
                       .append(", ").append(delimitText(fieldInfo.getApplicabilityType().getApplicabilityName())).append(", ")
                       .append(delimitText(fieldInfo.getValue())).append(", ")
                       .append(String.valueOf(fieldInfo.isInherited() || (CcddFieldHandler.isTableTypeField(fieldInfo.getOwnerName())
                                        && !CcddFieldHandler.isTableTypeField(ownerName)))).append("), ");
            }
     ...
     * specified type. This command is executed in a separate thread since it can
     * take a noticeable amount time to complete, and by using a separate thread the
     * GUI is allowed to continue to update. The GUI menu commands, however, are
        ...
        CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand() {
            ...
            protected void execute() {
                try {
                    ...
                    // Log that renaming the table succeeded
                    eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Table '").append(tableName).append("' type renamed to '")
                            .append(newName).append("'"));
                } catch (SQLException se) {
                    // Inform the user that renaming the table type failed
                    eventLog.logFailEvent(typeDialog,
                            "Cannot rename table type for table '" + tableName + "'; cause '" + se.getMessage() + "'",
        ...
        CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand() {
            ...
            protected void execute() {
                try {
                    ...
                    StringBuilder command = new StringBuilder(storeTableTypesInfoTableCommand()).append(storeNonTableTypesInfoTableCommand(
                            InternalTable.FIELDS, fieldDefinitions, null, typeDialog));

                    // Execute the command to change the table's type name
                    dbCommand.executeDbCommand(command, typeDialog);

                    // Log that renaming the table succeeded
                    eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Table type '").append(typeName).append("' copied to '")
                            .append(copyName).append("'"));
                } catch (SQLException se) {
            ...
            protected void execute() {
                try {
                    ...
                    if (tableNames.length != 0) {
                        ...
                        if (new CcddDialogHandler().showMessageDialog(parent,
                                "<html><b>Delete table(s) '</b>" + names + "<b>' of type '</b>" + typeDefn.getName()
                    ...
                    if (!errorFlag) {
                        ...
                        command.append(modifyFieldsCommand(CcddFieldHandler.getFieldTypeName(typeDefn.getName()), null));
                        ...
                        eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Table type '").append(typeDefn.getName()).append("'")
                                .append(CcddUtilities.removeHTMLTags(names)).append(" deleted"));
                    }
                } catch (SQLException se) {
                    ...
                    eventLog.logFailEvent(parent,
                            ...
                            "<html><b>Cannot delete table type '</b>" + typeDefn.getName() + "<b>'</b>" + names
                                    + "<b>'");
                    errorFlag = true;
                } catch (Exception e) {
                    // Display a dialog providing details on the unanticipated error
    ...
    /**********************************************************************************************
     * Add, rename, or delete table type column names in the table definitions table
     * and update the existing tables of the specified type. This command is
     * executed in a separate thread since it can take a noticeable amount time to
     * complete, and by using a separate thread the GUI is allowed to continue to
     * update. The GUI menu commands, however, are disabled until the database
     * command completes execution
     ...
     *
     * @param overwriteFields    OverwriteFieldValueType: ALL to overwrite all field
     *                           values, SAME to overwrite only those fields with a
     *                           matching value, EMPTY to overwrite only fields with
     *                           blank values, or NONE to not overwrite any field
     *                           values
     *
     * @param typeAdditions      list of new columns to add to the tables. Each list
     *                           item is an array containing: [0] column name
     *                           (user), [1] column input type
     *
     * @param typeModifications  list of name changes of existing columns in the
     *                           tables. Each list item is an array containing: [0]
     *                           original column name (user), [1] new column name
     *                           (user), [2] column input type
     *
     * @param typeDeletions      list of columns to remove from the tables. Each
     *                           list item is an array containing: [0] column name
     *                           (user), [1] column input type
     *
     * @param columnOrderChange  true if the table type's column order changed
     *
     * @param originalDefn       reference to the table type definition prior to
     *                           making any changes
     *
     * @param fieldAdditions     list of new fields to add to tables of this type
     *
     * @param fieldModifications list of changes to existing fields in tables of
     *                           this type
     *
     ...
     *
     * @param editor             reference to the table type editor where the
     *                           change(s) occurred
     *********************************************************************************************/
    protected void modifyTableTypeInBackground(final String tableType, final List<FieldInformation> fieldInformation,
            final OverwriteFieldValueType overwriteFields, final List<String[]> typeAdditions,
            final List<String[]> typeModifications, final List<String[]> typeDeletions, final boolean columnOrderChange,
            final TypeDefinition originalDefn, final List<TableModification> fieldAdditions,
            final List<TableModification> fieldModifications, final List<TableModification> fieldDeletions,
            final CcddTableTypeEditorDialog editorDialog, final CcddTableTypeEditorHandler editor) {
        // Execute the command in the background
        CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand() {
            /**************************************************************************************
    ...
    /**********************************************************************************************
     * Add, rename, or delete table type column names in the table definitions table
     * and update the existing tables of the specified type
     *
     ...
     *
     * @param originalDefn       reference to the table type definition prior to
     *                           making any changes; null if this is a new table
     *                           type
     *            
     * @param newDataFields      Reference to the new data fields. This is only needed
     *                           when no editor is provided
     *
     * @param editorDialog       reference to the table type editor dialog
     *
     * @param editor             reference to the table type editor where the
    ...
    protected void modifyTableType(String typeName, List<FieldInformation> fieldInformation,
            OverwriteFieldValueType overwriteFields, List<String[]> typeAdditions, List<String[]> typeModifications,
            List<String[]> typeDeletions, boolean columnOrderChange, TypeDefinition originalDefn, List<String[]> newDataFields,
        ...
        try {
            ...
            if (originalDefn != null) {
                ...
                for (String protoName : protoTableNames) {
                    ...
                    for (String add[] : typeAdditions) {
                        ...
                        command.append("ALTER TABLE ").append(dbTable).append(" ADD COLUMN ").append(dbColumn).append(" text DEFAULT ''; ");
                    }
                    ...
                    for (String[] mod : typeModifications) {
                        ...
                        if (!mod[2].equals(mod[3]) && inputTypeHandler.getInputTypeByName(mod[3])
                                .getInputFormat() != InputTypeFormat.TEXT) {
                            ...
                            command.append("UPDATE ").append(dbTable).append(" SET ").append(newDbColumn).append(" = '' WHERE ")
                                   .append(newDbColumn).append(" != '' AND ").append(newDbColumn).append(" !~ E'^")
                    ...
                    for (String[] del : typeDeletions) {
                        ...
                        command.append("ALTER TABLE ").append(dbTable).append(" DROP COLUMN ").append(dbColumn).append("; ");
                    }
                ...
                if ((!typeModifications.isEmpty() || !typeDeletions.isEmpty()) && protoTableNames.length != 0) {
                    ...
                    for (String protoName : protoTableNames) {
                        ...
                        valuesCmd.append(ValuesColumn.TABLE_PATH.getColumnName()).append(" ~ E'[^,]+,").append(protoName)
                                 .append("\\.[^,]+,[^,]+$' OR ");
                    }
                    ...
                    if (isStructure && wasStructure) {
                        ...
                        for (String protoName : protoTableNames) {
                            ...
                            tlmSchCmd.append(TlmSchedulerColumn.MEMBER.getColumnName()).append(" ~ E'(?:").append(tlmSchSeparator)
                                     .append(protoName).append(",|[^,]+,").append(protoName).append("\\.[^,]+,[^,]+$)' OR ");
                        }
                            ...
                            // Check if the rate column name changed
                                ...
                                // type (i.e., the rate name is unique to this table type)
                                    ...
                                    command.append("UPDATE ").append(InternalTable.LINKS.getTableName()).append(" SET ")
                                           ...
                                           .append("' WHERE ").append(LinksColumn.RATE_NAME.getColumnName()).append(" = '")
                                           .append(mod[0]).append("'; UPDATE ").append(InternalTable.TLM_SCHEDULER.getTableName())
                                           .append(" SET ").append(TlmSchedulerColumn.RATE_NAME.getColumnName()).append(" = '")
                                           .append(mod[1]).append("' WHERE ").append(TlmSchedulerColumn.RATE_NAME.getColumnName())
                                           .append(" = '").append(mod[0]).append("'; ");
                                }
            ...
            try {
                ...
                eventLog.logFailEvent(editorDialog, "Cannot update table type '" + typeName + "'" + CcddUtilities.removeHTMLTags(names)
                                + "; cause '" + se.getMessage() + "'", "<html><b>Cannot update table type '</b>" + typeName + "<b>'" + names);
                errorFlag = true;
            } catch (Exception e) {
                // Display a dialog providing details on the unanticipated error
     ...
     *
     * @param tableNames list containing arrays of information on the table name(s)
     *                   to close. For a prototype table each array in the list
     *                   contains the prototype name and a null. If the deletion is
     *                   due to a change in a variable's structure data type then
     *                   the array contains the immediate parent table name and the
     *                   original data type (i.e., structure table name) of the
     *                   variable
     *
     * @param parent     GUI component over which to center any error dialog
     *********************************************************************************************/
     ...
     *                       changes; null if none of the input type names changed
     *
     * @param parent         GUI component over which to center any error dialog
     *********************************************************************************************/
            ...
            protected void execute() {
                try {
                    ...
                    for (String[] mod : modifications) {
                        ...
                        // table
                        command.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                               .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = ").append(delimitText(mod[2]))
                               .append(" WHERE ").append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = '")
                               .append(mod[0]).append("' AND ").append(FieldsColumn.FIELD_NAME.getColumnName())
                               .append(" = '").append(mod[1]).append("'; ");
                    }
    ...
    /**********************************************************************************************
     * Modify all tables affected by changes to the user-defined data type names, or
     * macro names and/or macro values. This command is executed in a separate
     * thread since it can take a noticeable amount time to complete, and by using a
     * separate thread the GUI is allowed to continue to update. The GUI menu
     * commands, however, are disabled until the database command completes
     * execution
     ...
     *
     * @param updates       list of string arrays reflecting the content of the data
     *                      types (macros) after being changed in the data type
     *                      (macro) editor
     *
    ...
    protected void modifyTablesPerDataTypeOrMacroChangesInBackground(final List<TableModification> modifications,
            final List<String[]> updates, final CcddDialogHandler dialog) {
        // Execute the command in the background
    ...
    /**********************************************************************************************
     * Modify all tables affected by changes to the user-defined data type names, or
     * macro names and/or macro values
     *
     * @param modifications list of data type (macro) definition modifications
     *
     * @param updates       list of string arrays reflecting the content of the data
    ...
    protected boolean modifyTablesPerDataTypeOrMacroChanges(final List<TableModification> modifications,
        ...
        /******************************************************************************************
         * Class for table information for those tables modified due to changes in a
        ...
        class ModifiedTable {
            ...
            /**************************************************************************************
             * Class constructor for table information for those tables modified due to
             * changes in a data type (macro) definition
             *
        ...
        for (TableModification mod : modifications) {
            ...
            if (isDataType) {
                ...
                oldName = CcddDataTypeHandler.getDataTypeName(
                        mod.getOriginalRowData()[DataTypesColumn.USER_NAME.ordinal()].toString(),
                        mod.getOriginalRowData()[DataTypesColumn.C_NAME.ordinal()].toString());
                newName = CcddDataTypeHandler.getDataTypeName(
                        mod.getRowData()[DataTypesColumn.USER_NAME.ordinal()].toString(),
            ...
            for (String ref : references) {
                ...
                boolean isValueChange = (isDataType && (dataTypeHandler.getSizeInBytes(oldName) != newDataTypeHandler.getSizeInBytes(newName)
                                || !dataTypeHandler.getBaseDataType(oldName) .equals(newDataTypeHandler.getBaseDataType(newName))))
                                || (!isDataType && macroValue != null && !macroValue.equals(newMacroHandler.getMacroValue(newName)));
        ...
        try {
            ...
            if (nameChangeOnly) {
                ...
                for (TableModification mod : modifications) {
                    ...
                    if (isDataType) {
                        ...
                        String oldName = CcddDataTypeHandler.getDataTypeName(
                                mod.getOriginalRowData()[DataTypesColumn.USER_NAME.ordinal()].toString(),
                                mod.getOriginalRowData()[DataTypesColumn.C_NAME.ordinal()].toString());
                        String newName = CcddDataTypeHandler.getDataTypeName(
                                mod.getRowData()[DataTypesColumn.USER_NAME.ordinal()].toString(),
                                mod.getRowData()[DataTypesColumn.C_NAME.ordinal()].toString());
                        ...
                        vals.add(new ImmutableTriple<String, String, String>(InternalTable.LINKS.getTableName(), LinksColumn.MEMBER.getColumnName() ,LinksColumn.MEMBER.getColumnName()));
                        vals.add(new ImmutableTriple<String, String, String>(InternalTable.TLM_SCHEDULER.getTableName(), TlmSchedulerColumn.MEMBER.getColumnName() ,TlmSchedulerColumn.MEMBER.getColumnName()));
                        vals.add(new ImmutableTriple<String, String, String>(InternalTable.VALUES.getTableName(), ValuesColumn.TABLE_PATH.getColumnName() ,ValuesColumn.TABLE_PATH.getColumnName()));

                        // Generate the SQL command to replace the name
                        String replaceCommand = ", E'(.*,)" + oldName + "(\\..*)'" + ", E'\\\\1" + newName + "\\\\2'); ";
                    ...
                    // This is a macro change
                        ...
                        command.append("UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                               ...
                               .append(InternalTable.FIELDS.getTableName()).append(" SET ").append(FieldsColumn.OWNER_NAME.getColumnName())
                               .append(" = regexp_replace(").append(FieldsColumn.OWNER_NAME.getColumnName()).append(", E'(.*)")
                               .append(oldName).append("(.*)'").append(", E'\\\\1").append(newName).append("\\\\2'); UPDATE ")
                               .append(InternalTable.GROUPS.getTableName()).append(" SET ").append(GroupsColumn.MEMBERS.getColumnName())
                               .append(" = regexp_replace(").append(GroupsColumn.MEMBERS.getColumnName()).append(", E'(.*)")
                               .append(oldName).append("(.*)'").append(", E'\\\\1").append(newName).append("\\\\2'); UPDATE ")
                               .append(InternalTable.LINKS.getTableName()).append(" SET ").append(LinksColumn.MEMBER.getColumnName())
                               .append(" = regexp_replace(").append(LinksColumn.MEMBER.getColumnName()).append(", E'(.*)").append(oldName)
                               .append("(.*)'").append(", E'\\\\1").append(newName).append("\\\\2'); UPDATE ")
                               .append(InternalTable.TLM_SCHEDULER.getTableName()).append(" SET ").append(TlmSchedulerColumn.MEMBER.getColumnName())
                               .append(" = regexp_replace(").append(TlmSchedulerColumn.MEMBER.getColumnName()).append(", E'(.*)")
                               .append(oldName).append("(.*)'").append(", E'\\\\1").append(newName).append("\\\\2'); UPDATE ")
                               .append(InternalTable.VALUES.getTableName()).append(" SET ").append(ValuesColumn.TABLE_PATH.getColumnName())
                               .append(" = regexp_replace(").append(ValuesColumn.TABLE_PATH.getColumnName()).append(", E'(.*)").append(oldName)
            ...
            if (isDataType) {
                ...
                dbCommand.executeDbUpdate(new StringBuilder(storeNonTableTypesInfoTableCommand(InternalTable.DATA_TYPES,
                        CcddUtilities.removeArrayListColumn(updates, DataTypesColumn.OID.ordinal()), null, dialog)),
                        dialog);
                dbCommand.executeDbUpdate(new StringBuilder(storeNonTableTypesInfoTableCommand(InternalTable.MACROS,
                        CcddUtilities.removeArrayListColumn(newMacroHandler.getMacroData(), MacrosColumn.OID.ordinal()),
                        null, dialog)), dialog);
            }
            // This is a macro change
            else {
                ...
                dbCommand.executeDbUpdate(new StringBuilder(storeNonTableTypesInfoTableCommand(InternalTable.MACROS,
                        CcddUtilities.removeArrayListColumn(updates, MacrosColumn.OID.ordinal()), null, dialog)),
                        dialog);
            }
    ...
    /**********************************************************************************************
     * Modify all tables affected by changes to the user-defined input types. This
     ...
     *
     * @param updates       array of string arrays reflecting the content of the
     *                      input types after being changed in the input type editor
     *
    ...
    protected void modifyTablesPerInputTypeChanges(final List<TableModification> modifications,
        ...
        /******************************************************************************************
         * Class for table information for those tables modified due to changes in an
        ...
        class ModifiedTable {
            ...
            /**************************************************************************************
             * Class constructor for table information for those tables modified due to
             * changes in an input type definition
             *
             * @param tablePath table path (if applicable) and name
             *************************************************************************************/
                ...
                try {
                    ...
                    for (TableModification mod : modifications) {
                        ...
                        if (!oldName.equals(newName)) {
                            ...
                            tableTypeCommand.append("UPDATE ").append(InternalTable.TABLE_TYPES.getTableName()).append(" SET ")
                                            .append(TableTypesColumn.INPUT_TYPE.getColumnName()).append(" = ").append(delimitText(newName))
                                            .append(" WHERE ").append(TableTypesColumn.INPUT_TYPE.getColumnName()).append(" = ")
                                            .append(delimitText(oldName)).append("; ");
                            
                            fieldCommand.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                                        .append(FieldsColumn.FIELD_TYPE.getColumnName()).append(" = ").append(delimitText(newName))
                                        .append(" WHERE ").append(FieldsColumn.FIELD_TYPE.getColumnName()).append(" = ")
                                        .append(delimitText(oldName)).append("; ");
                        }
                        ...
                        if (!mod.getOriginalRowData()[InputTypesColumn.MATCH.ordinal()]
                            ...
                            for (String inputTypeRef : inputTypeHandler.searchInputTypeReferences(oldName, dialog)) {
                                ...
                                String[] refColumns = CcddUtilities.splitAndRemoveQuotes(
                                        tblColCmtAndCntxt[SearchResultsQueryColumn.CONTEXT.ordinal()]);
                                        ...
                                        for (int row = 0; row < tableData.size(); row++) {
                                            ...
                                            for (Integer column : columns) {
                                                ...
                                                showMessage = table.validateCellContent(tableData, row, column, "",
                                                        tableData.get(row)[column], showMessage, true);
                            ...
                            for (String[] field : fields) {
                                ...
                                if (!field[FieldsColumn.FIELD_VALUE.ordinal()]
                                    ...
                                    fieldCommand.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                                                .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = '' WHERE ")
                                                .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = ")
                                                .append(delimitText(field[FieldsColumn.OWNER_NAME.ordinal()])).append(" AND ")
                                                .append(FieldsColumn.FIELD_NAME.getColumnName()).append(" = ")
                                                .append(delimitText(field[FieldsColumn.FIELD_NAME.ordinal()])).append("; ");
                                }
                            }
                        }
                    }
                    ...
                    dbCommand.executeDbUpdate(new StringBuilder(storeNonTableTypesInfoTableCommand(InternalTable.INPUT_TYPES,
                            CcddUtilities.removeArrayListColumn(Arrays.asList(dialog.getUpdatedData()), InputTypesColumn.OID.ordinal()),
                            null, dialog)), dialog);
                    ...
                    // Inform the user that the update succeeded
                    eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Input types and all affected tables and fields updated"));
                } catch (SQLException se) {
    ...
    /**********************************************************************************************
     * Get an array containing all prototype tables that are of the specified table
     * type (combining all structure and all command tables)
     *
     * @param tableType TYPE_STRUCTURE to get all tables for any type that
     *                  represents a structure, TYPE_COMMAND to get all tables for
     *                  any type that represents a command, TYPE_OTHER to get all
     *                  tables that are neither a structure or command table, or the
     *                  table type name to get all tables for the specified type
     *
     * @return Array containing all prototype tables that represent the specified
     *         type. Returns an empty array if no table of the specified type exists
     ...
     *
     * @param protoTableNames names of the prototype tables of the specified table
     *                        type; null to load the list
     *
     * @param parent          GUI component over which to center any error dialog
     *
     * @return List containing all tables that are of the specified table type.
     *         Returns an empty list if no table of the specified type exists in the
     *         project database
     *********************************************************************************************/
