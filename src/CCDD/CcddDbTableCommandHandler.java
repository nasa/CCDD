...
public class CcddDbTableCommandHandler
{
    ...
    // Flag that indicates a variable has been added to a link definition and the
    ...
    // tables,
    // with any special characters escaped so as to be used in a PostgrSQL command
    ...
    CcddDbTableCommandHandler(CcddMain ccddMain)
    {
        ...
        // Escape any special characters in the script associations and telemetry
    ...
    /**********************************************************************************************
     * Enclose database text string objects with a delimiter. The default delimiter is single quotes. If
     * the text contains single quotes or a backslash then $$ is used instead, unless the text already
     * contains $$ or ends with $, in which case $_$ is used as the delimiter
     *
     ...
     *
     * @param structuresOnly true to only include structure tables; false to include tables of all types
     *
     * @param parent         GUI component over which to center any error dialog
     *
     ...
     *                    coming from the next level down in the structure's hierarchy. The path is the
     *                    same as the root table name for non-structure tables
     *
    ...
    {
        String command = "DELETE FROM " + InternalTable.VALUES.getTableName() + " WHERE "
                         + ValuesColumn.TABLE_PATH.getColumnName() + " = '" + tablePath + "' AND "
                         + ValuesColumn.COLUMN_NAME.getColumnName() + " = ''; ";

        // Check if the description isn't blank
        if (!description.isEmpty())
        {
            command += "INSERT INTO " + InternalTable.VALUES.getTableName() + " ("
                       + ValuesColumn.TABLE_PATH.getColumnName() + ", " + ValuesColumn.COLUMN_NAME.getColumnName()
                       + ", " + ValuesColumn.VALUE.getColumnName() + ") VALUES ('" + tablePath + "', '', "
    ...
    {
        return "DELETE FROM " + InternalTable.ORDERS.getTableName() + " WHERE " + OrdersColumn.USER_NAME.getColumnName()
               + " = '" + dbControl.getUser() + "' AND " + OrdersColumn.TABLE_PATH.getColumnName() + " = '" + tablePath
               + "'; INSERT INTO " + InternalTable.ORDERS.getTableName() + " (" + OrdersColumn.USER_NAME.getColumnName()
               + ", " + OrdersColumn.TABLE_PATH.getColumnName() + ", " + OrdersColumn.COLUMN_ORDER.getColumnName()
               + ") VALUES ('" + dbControl.getUser() + "', '" + tablePath + "', '" + columnOrder + "'); ";
    }
    ...
    protected List<String[]> queryDatabase(StringBuilder sqlCommand, Component parent)
    {
        ...
        try
        {
            ...
            ResultSet results = dbCommand.executeDbQuery(new StringBuilder(sqlCommand), parent);
        ...
        )
        {
            ...
            eventLog.logFailEvent(parent, "Database query failed; cause '" + se.getMessage() + "'",
     ...
     *
     * @param primaryKey value of the table's primary key column from which to extract the cell value
     *
     ...
     *********************************************************************************************/
    protected String queryTableCellValue(String tableName, String primaryKey, String columnName, Component parent)
    {
        String cellValue = "";

        // Get the cell value from the table
        List<String[]> results = queryDatabase(new StringBuilder("SELECT ").append(columnName).append(" FROM ")
                .append(tableName).append(" WHERE ").append(DefaultColumn.PRIMARY_KEY.getDbName()).append(" = ")
                .append(primaryKey), parent);
    ...
    /**********************************************************************************************
     * Retrieve a list of all data tables in the database with the names in the user-viewable form. Any
     ...
     * @param parent GUI component over which to center any error dialog
     *
     * @return List array containing the table names and table types. The table names are provided in
     *         both the user-viewable form, which preserves capitalization, and the database form. An
     *         empty array is returned if no tables exist
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Retrieve a list of all tables of a specific type in the database. Any non-data tables are ignored
     *
    ...
    protected String[] queryTablesOfTypeList(String tableType, Component parent)
    {
        return dbCommand.getList(DatabaseListCommand.TABLES_OF_TYPE,
                                 new String[][] {{"_type_", tableType.toLowerCase()}}, parent);
    }
     ...
     * @param parent     GUI component over which to center any error dialog
     *
     * @return Array containing the table name, data type, and variable names for the specified table
     *********************************************************************************************/
    protected String[] queryDataTypeAndVariablesTypeList(String tableTypes, Component parent)
    {
        return dbCommand.getList(DatabaseListCommand.TABLE_DATA_VAR_NAMES,
                                 new String[][] {{"_table_types_", tableTypes}}, parent);
    }
    ...
    /**********************************************************************************************
     * Retrieve a list of comments for all data tables in the database. Any non-data tables are ignored
     *
     * @param parent GUI component over which to center any error dialog
     *
     * @return String array containing the table comment parameters and the table name as used in the
     *         database
     *********************************************************************************************/
    ...
    protected String[] queryTableComment(String tableName, int numParts, Component parent)
    {
        ...
        try
        {
            ...
            ResultSet comment = dbCommand
                    .executeDbQuery(new StringBuilder("SELECT obj_description('public." + tableName.toLowerCase()
                                                      + "'::regclass, 'pg_class');"),
                                    parent);
        ...
        )
        {
            ...
            eventLog.logFailEvent(parent, "Cannot obtain comment for table '" + tableName + "'; cause '"
                                          + se.getMessage() + "'",
                                  "<html><b>Cannot obtain comment for table '</b>" + tableName + "<b>'");
        }
    ...
    /**********************************************************************************************
     * Retrieve the comment for the specified data table. The comment contains the table's visible name
     * (preserves case) and type
     *
    ...
    protected String[] queryDataTableComment(String tableName, Component parent)
    {
        ...
        // Create an array to contain the data table comment elements and initialize
        // each to a
        // blank
    ...
    /**********************************************************************************************
     * Retrieve a list of all data table descriptions in the database. Any non-data tables are ignored
     *
     * @param parent GUI component over which to center any error dialog
     *
    ...
    protected String[][] queryTableDescriptions(Component parent)
    {
        ...
        String[] descriptions = dbCommand.getList(DatabaseListCommand.TABLE_DESCRIPTIONS, null, parent);
     ...
     *                  from the root table, with each succeeding pair coming from the next level down
     *                  in the structure's hierarchy
     *
     * @param parent    GUI component over which to center any error dialog
     *
    ...
    protected String queryTableDescription(String tablePath, Component parent)
    {
        ...
        try
        {
            command.append("SELECT ").append(ValuesColumn.VALUE.getColumnName()).append(" FROM ")
                    .append(InternalTable.VALUES.getTableName()).append(" WHERE ")
                    .append(ValuesColumn.TABLE_PATH.getColumnName()).append(" = '").append(tablePath).append("' AND ")
                    .append(ValuesColumn.COLUMN_NAME.getColumnName()).append(" = '';");
            ...
            if (descData.next())
            {
                // Split the description data into its constituent parts and copy these to the description
        ...
        )
        {
            ...
            eventLog.logFailEvent(parent,
                                  "Cannot obtain description for table '" + tablePath + "'; cause '" + se.getMessage()
                                          + "'",
                                  "<html><b>Cannot obtain description for table '</b>" + tablePath + "<b>'");
        }
     ...
     *
     * @param tablePath comma-separated variable names describing the path to this table, beginning with
    ...
    private String queryColumnOrder(String tablePath, String tableType, Component parent)
    {
        ...
        try
        {
            ...
            command.append("SELECT ").append(OrdersColumn.COLUMN_ORDER.getColumnName()).append(" FROM ")
                    .append(InternalTable.ORDERS.getTableName()).append(" WHERE ")
                    .append(OrdersColumn.USER_NAME.getColumnName()).append(" = '").append(dbControl.getUser())
                    .append("' AND ").append(OrdersColumn.TABLE_PATH.getColumnName()).append(" = '").append(tablePath)
                    .append("';");
        ...
        )
        {
            ...
            eventLog.logFailEvent(parent,
                                  "Cannot obtain column order for table '" + tablePath + "'; cause '" + se.getMessage()
                                          + "'",
                                  "<html><b>Cannot obtain column order for table '</b>" + tablePath + "<b>'");
        }
     ...
     *
     * @param comments    array of table comments, with each comment divided into its component parts
     *
    ...
    protected void setTableComment(String tableName, String comment, Component parent)
    {
        ...
        )
        {
            ...
            eventLog.logFailEvent(parent, "Cannot update comment for table '" + tableName + "'; cause '"
                                          + se.getMessage() + "'",
                                  "<html><b>Cannot update comment for table '</b>" + tableName + "<b>'");
        }
    }
    ...
    /**********************************************************************************************
     * Update the root structure table, variable path and offset, and command lists, and the variable,
     * command, and message ID references. This is needed once one or more tables are created, copied,
     * renamed, or deleted
     *
     * @param parent GUI component over which to center any error dialog
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Create one or more tables of the specified type. This command is executed in a separate thread
     * since it can take a noticeable amount time to complete, and by using a separate thread the GUI is
     * allowed to continue to update. The GUI menu commands, however, are disabled until the database
     ...
     *********************************************************************************************/
    protected void createTableInBackground(final String[] tableNames, final String description, final String tableType,
                                           final CcddTableManagerDialog tableDialog)
    {
        ...
        CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand()
        {
            ...
            {
                // Create the table(s)
                errorFlag = createTable(tableNames, description, tableType, true, true, tableDialog);
            }
     ...
     *
     * @param updatePreloadedInfo Should the preloaded table tree information be updated?
     *
     * @param parent              GUI component over which to center any error dialog
     *
    ...
    protected boolean createTable(String[] tableNames, String description, String tableType, boolean defaultFields,
                                  boolean updatePreloadedInfo, Component parent)
    {
        boolean errorFlag = false;
        ...
        try
        {
            ...
            for (String tableName : tableNames)
            {
                ...
                command.append(createTableCommand(tableName, description, tableType, defaultFields, parent));
            }
            ...
            // Inform the user that the update succeeded
            eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Table(s) '" + allNames + "' created"));
        }
        catch (
            SQLException se
        )
        {
            // Inform the user that the database command failed
            eventLog.logFailEvent(parent, "Cannot create table(s) '" + allNames + "'; cause '" + se.getMessage() + "'",
                                      ...
                                      Component parent)
    {
        ...
        for (int column = 0; column < typeDefn.getColumnNamesDatabase().length; column++)
        {
            ...
            command.append("\"" + typeDefn.getColumnNamesDatabase()[column] + "\" "
        ...
        command.append("); " + buildDataTableComment(tableName, tableType)
    ...
    /**********************************************************************************************
     * Change the name of a table. Only prototype tables can be renamed using this method (instances are
     * renamed by changing the variable name in the prototype). This command is executed in a separate
     * thread since it can take a noticeable amount time to complete, and by using a separate thread the
     ...
     * @param tableDialog    reference to the table manager dialog
     *********************************************************************************************/
    protected void renameTable(final String tableName, final String newName, final String newDescription,
                               final CcddTableManagerDialog tableDialog)
    {
        // Execute the command in the background
        CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand()
            ...
            protected void execute()
            {
                try
                {
                    ...
                    for (String rootTable : getRootTables(false, tableDialog))
                    {
                        ...
                        TypeDefinition typeDefn = tableTypeHandler
                                .getTypeDefinition(rootComment[TableCommentIndex.TYPE.ordinal()]);
                        ...
                        for (InputType inputType : typeDefn.getInputTypesList())
                        {
                            ...
                            if (inputType.getInputFormat().equals(InputTypeFormat.DATA_TYPE))
                            {
                                ...
                                for (Integer column : typeDefn.getColumnIndicesByInputType(inputType))
                                {
                                    ...
                                    command.append("UPDATE ").append(rootTable).append(" SET ").append(dataTypeColName)
                                            .append(" = '").append(newName).append("' WHERE ").append(dataTypeColName)
                            ...
                            if (inputType.getInputFormat().equals(InputTypeFormat.VARIABLE_REF))
                            {
                                ...
                                for (Integer column : typeDefn.getColumnIndicesByInputType(inputType))
                                {
                                    ...
                                    command.append("UPDATE ").append(rootTable).append(" SET ").append(varRefColName)
                                            .append(" = regexp_replace(").append(varRefColName).append(", E'(^|,)")
                                            .append(tableName).append("(\\\\.|,|$)', E'\\\\1").append(newName)
                                            .append("\\\\2'); ");
                                }
                            }
                        }
                    ...
                    if (!dbTableName.equals(dbNewName))
                    {
                        ...
                        command.append("ALTER TABLE ").append(dbTableName).append(" RENAME TO ").append(dbNewName)
                                .append("; ");
                    }
                    ...
                    command.append(buildTableDescription(newName, newDescription))
                            .append(renameInfoTableCommand(InternalTable.VALUES,
                                                           ValuesColumn.TABLE_PATH.getColumnName(), tableName, newName))
                            .append(renameInfoTableCommand(InternalTable.LINKS, LinksColumn.MEMBER.getColumnName(),
                                                           tableName, newName))
                            .append(renameInfoTableCommand(InternalTable.TLM_SCHEDULER,
                                                           TlmSchedulerColumn.MEMBER.getColumnName(), tableName,
                                                           newName))
                            .append(renameInfoTableCommand(InternalTable.GROUPS, GroupsColumn.MEMBERS.getColumnName(),
                                                           tableName, newName))
                            .append(renameInfoTableCommand(InternalTable.FIELDS,
                                                           FieldsColumn.OWNER_NAME.getColumnName(), tableName, newName))
                            .append(renameInfoTableCommand(InternalTable.ORDERS,
                                                           OrdersColumn.TABLE_PATH.getColumnName(), tableName, newName))
                            .append(renameInfoTableCommand(InternalTable.ASSOCIATIONS,
                                                           AssociationsColumn.MEMBERS.getColumnName(), tableName,
                                                           newName));
                    ...
                    eventLog.logEvent(SUCCESS_MSG,
                                      new StringBuilder("Table '" + tableName + "' renamed to '" + newName + "'"));
                }
                catch (
                    SQLException se
                )
                {
                    ...
                    eventLog.logFailEvent(tableDialog,
                                          "Cannot rename table '" + tableName + "'; cause '" + se.getMessage() + "'",
     ...
     *********************************************************************************************/
    private String renameInfoTableCommand(InternalTable intType, String idColumnName, String oldName, String newName)
    {
        ...
        return "UPDATE " + intType.getTableName() + " SET " + idColumnName + " = regexp_replace(" + idColumnName
               + ", E'(^|," + (intType == InternalTable.TLM_SCHEDULER ? "|" + tlmSchSeparator : separator) + ")"
               + oldName + "(\\\\.|," + separator + "|$)', E'\\\\1" + newName + "\\\\2'); ";
    }

    /**********************************************************************************************
     * Make a copy of an existing table. Only prototype tables can be copied using this method
     * (instances are copied by adding a new instance and copying the values to it in the table editor).
     * This command is executed in a separate thread since it can take a noticeable amount time to
     * complete, and by using a separate thread the GUI is allowed to continue to update. The GUI menu
     * commands, however, are disabled until the database command completes execution
     *
     ...
     * @param tableDialog    reference to the table manager dialog
     *********************************************************************************************/
    protected void copyTable(final String tableName, final String newName, final String newDescription,
                             final CcddTableManagerDialog tableDialog)
    {
        // Execute the command in the background
        CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand()
        {
            ...
            protected void execute()
            {
                try
                {
                    ...
                    String sequenceName = newName.toLowerCase() + "_" + DefaultColumn.PRIMARY_KEY.getDbName() + "_seq";

                    // Create the command to copy the original table
                    StringBuilder command = new StringBuilder();

                    command.append("CREATE TABLE ").append(dbNewName).append(" (LIKE ").append(dbTableName)
                            .append(" INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING INDEXES); ALTER TABLE ")
                            .append(dbNewName).append(" ALTER ").append(DefaultColumn.PRIMARY_KEY.getDbName())
                            .append(" DROP DEFAULT; CREATE SEQUENCE ").append(sequenceName).append("; INSERT INTO ")
                            .append(dbNewName).append(" SELECT * FROM ").append(dbTableName).append("; ALTER SEQUENCE ")
                            .append(sequenceName).append(" OWNED BY ").append(dbNewName).append(".")
                            .append(DefaultColumn.PRIMARY_KEY.getDbName()).append("; SELECT setval('")
                            .append(sequenceName).append("', (SELECT max(")
                            .append(DefaultColumn.PRIMARY_KEY.getDbName()).append(") FROM ").append(dbTableName)
                            .append("), true); ALTER TABLE ").append(dbNewName).append(" ALTER ")
                            .append(DefaultColumn.PRIMARY_KEY.getDbName()).append(" SET DEFAULT nextval('")
                    ...
                    // Log that renaming the table succeeded
                    eventLog.logEvent(SUCCESS_MSG,
                                      new StringBuilder("Table '" + tableName + "' copied to '" + newName + "'"));
                }
                catch (
                    SQLException se
                )
                {
                    ...
                    eventLog.logFailEvent(tableDialog,
                                          "Cannot copy table '" + tableName + "'; cause '" + se.getMessage() + "'",
     ...
     * editors for tables of this prototype are closed. This command is executed in a separate thread
     * since it can take a noticeable amount time to complete, and by using a separate thread the GUI is
     * allowed to continue to update. The GUI menu commands, however, are disabled until the database
     * command completes execution
     *
     ...
     *
     * @param dialog     reference to the table manager dialog calling this method; null if not called
     *                   by the table manager
     *
     * @param parent     GUI component over which to center any error dialog
     *********************************************************************************************/
    protected void deleteTableInBackground(final String[] tableNames, final CcddTableManagerDialog dialog,
                                           final Component parent)
    {
        ...
        if (new CcddDialogHandler()
                .showMessageDialog(parent,
                                   "<html><b>Delete table(s) '</b>" + names
                                           + "<b>'?<br><br><i>Warning: This action cannot be undone!",
                                   "Delete Table(s)", JOptionPane.QUESTION_MESSAGE,
     ...
     *********************************************************************************************/
    protected boolean deleteTable(String[] tableNames, CcddTableManagerDialog dialog, Component parent)
    {
        boolean errorFlag = false;
     ...
     * @param tableNames  array of names of the tables to delete
     *
     * @param isDataTable true if the table(s) to be deleted are data tables; false if the tables are
    ...
    private String deleteTableCommand(String[] tableNames, boolean isDataTable)
    {
        ...
        for (String name : tableNames)
        {
            ...
            command.append(dbControl.getQuotedName(name)).append(", ");
        }
        ...
        if (isDataTable)
        {
            ...
            StringBuilder linksDelCmd = new StringBuilder("; DELETE FROM ").append(InternalTable.LINKS.getTableName())
                    .append(" WHERE ");
            StringBuilder tlmSchDelCmd = new StringBuilder("; DELETE FROM ")
                    .append(InternalTable.TLM_SCHEDULER.getTableName()).append(" WHERE ");
            ...
            {
                // Add the table to the commands
                infoCmd.append("col1 ~ E'^").append(name).append("$' OR col1 ~ E'^").append(name)
                        .append(",' OR col1 ~ E',").append(name).append("\\\\.' OR ");

                // Build the command to update the script associations table by removing references to this table
                assnsUpdCmd.append("; UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                        .append(AssociationsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                        .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'(?:(?:^|(?:").append(PATH_IDENT)
                        .append(",))").append(name).append("(?:(?:,|\\\\.)").append(PATH_IDENT).append(")?")
                        .append(assnsSeparator).append("|").append(assnsSeparator).append("(?:^| \\+ |")
                        .append(PATH_IDENT).append(",)?").append(name).append("(?:(?:,|\\\\.)").append(PATH_IDENT)
                        .append(")?)', E'', 'g'); UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName())
                        .append(" SET ").append(AssociationsColumn.MEMBERS.getColumnName()).append(" = '' WHERE ")
                        .append(AssociationsColumn.MEMBERS.getColumnName()).append(" ~ E'^(?:").append(PATH_IDENT)
                        .append(",)?").append(name).append("(?:(?:,|\\\\.)").append(PATH_IDENT).append(")?$'");
            }

            // Replace the column 1 name placeholder with the table's column 1 name, then
            // add the
            // update string to the command. In the telemetry scheduler table the table
            // names are
            ...
            tlmSchDelCmd.append(infoCmd.toString().replaceAll("col1", TlmSchedulerColumn.MEMBER.getColumnName())
                    .replaceAll("\\^", "^\\\\\\\\d+\\\\\\\\.\\\\\\\\d+" + "\\\\\\\\\\\\\\" + TLM_SCH_SEPARATOR));
            ...
            command.append(CcddUtilities.removeTrailer(valuesDelCmd, " OR "))
                    ...
                    .append(CcddUtilities.removeTrailer(ordersDelCmd, " OR ")).append(assnsUpdCmd);
        }
    ...
    /**********************************************************************************************
     * Load the contents of a single database table and display the data in a table editor. Do not load
     * the table if it is already open for editing. If the table is already open, then the editor for
     * this table is brought to the foreground. This command is executed in a separate thread since it
     * can take a noticeable amount time to complete, and by using a separate thread the GUI is allowed
     * to continue to update. The GUI menu commands, however, are disabled until the database command
     * completes execution
     *
     * @param tablePath        table path in the format rootTable[,dataType1.variable1[,dataType2
     ...
     *********************************************************************************************/
    protected SwingWorker<?, ?> loadTableDataInBackground(String tablePath, CcddTableEditorDialog currentEditorDlg)
    {
        return loadTableDataInBackground(new String[] {tablePath}, currentEditorDlg);
    }

    /**********************************************************************************************
     * Load the contents of an array of database tables and display the data in a table editor. Do not
     * load tables that are already open for editing. If no table is loaded, but one in the list is
     * already open, then the editor for this table is brought to the foreground. This command is
     ...
     *                         rootTable[,dataType1.variable1[,dataType2 .variable2[,...]]]. The table
     *                         path for a non-structure table is simply the root table name. For a
     *                         structure table the root table is the top level structure table from
     *                         which this table descends. The first data type/variable name pair is from
     *                         the root table, with each succeeding pair coming from the next level down
     *                         in the structure's hierarchy
     *
     * @param callingEditorDlg table editor dialog in which to open the table; null to open the table in
     *                         a new editor dialog
     *
     * @return SwingWorker reference for this thread
     *********************************************************************************************/
            ...
            protected void complete()
            {
                ...
                if (!tableInformation.isEmpty())
                {
                    ...
                    if (callingEditorDlg == null)
                    {
                        ...
                        ccddMain.getTableEditorDialogs().add(new CcddTableEditorDialog(ccddMain, tableInformation));
                    }
                ...
                // Check if no table data was loaded due to the table(s) already being open in
                // an
                // editor
                ...
                for (String tableName : tablePaths)
                {
                    ...
                    // preferences
                    CcddUtilities.updateRememberedItemList(tableName, ccddMain.getRecentTableNames(),
     ...
     * @param parent          GUI component over which to center any error dialog
     *
     * @return TableInformation class containing the table data from the database. If the error flag is
     *         set the an error occurred and the data is invalid
     *********************************************************************************************/
                                      ...
                                      boolean ignoreErrors, Component parent)
    {
        ...
        try
        {
            ...
            if (varNameIndex != -1 && dataTypeIndex != -1 && tablePath.contains(","))
            {
                ...
                // Place double back slashes before each square brace character in an array index so that
                // the brackets are interpreted correctly in the query's regular expression comparisons
                ...
                command.append("SELECT * FROM ").append(InternalTable.VALUES.getTableName()).append(" WHERE ")
                        .append(ValuesColumn.TABLE_PATH.getColumnName()).append(" ~ E'^").append(tablePath)
                        .append(",[^,]+$' AND ").append(ValuesColumn.COLUMN_NAME.getColumnName()).append(" != '';");
                ...
                while (rowData.next())
                {
                    ...
                    // Get the index of the last data type/variable name separator character (if present)
                    ...
                    if (varIndex != -1)
                    {
                        ...
                        int row = typeDefn.getRowIndexByColumnValue(tableInfo.getData(),
                                                                    variableName.substring(varIndex + 1), varNameIndex);
                                ...
                                .equals(variableName.subSequence(variableName.lastIndexOf(",") + 1, varIndex)))
                        {
                            ...
                            int column = typeDefn.getColumnIndexByUserName(rowData
                                    .getString(ValuesColumn.COLUMN_NAME.getColumnName()));
                            ...
                            if (column != -1)
                            {
                                ...
                                if (typeDefn.getInputTypes()[column].getInputFormat() == InputTypeFormat.BOOLEAN)
                                {
                                    // Store the column value as a boolean
                                    tableInfo.getData()
                                            .get(row)[column] = rowData.getString(ValuesColumn.VALUE.getColumnName())
    ...
    /**********************************************************************************************
     * Perform the database query to load the rows from the custom values table that match the specified
     * column name and column value
     *
     * @param columnName  name to match in the custom values table 'column name' column
     *
     * @param columnValue value to match in the custom values table 'value' column; null or blank to
     *                    match any value
     *
     * @param parent      GUI component over which to center any error dialog
     *
     * @return List containing arrays with the row data (table path, column name, and value) from the
     *         custom values table for those rows that match the specified column name and column value
     *********************************************************************************************/
    protected List<String[]> getCustomValues(String columnName, String columnValue, Component parent)
    {
        ...
        try
        {
            ...
            StringBuilder command = new StringBuilder();
            command.append("SELECT * FROM ").append(InternalTable.VALUES.getTableName()).append(" WHERE ")
                    .append(ValuesColumn.COLUMN_NAME.getColumnName()).append(" = '").append(columnName).append("'")
                    .append((columnValue == null || columnValue.isEmpty() ? ""
                                                                          : " AND " + ValuesColumn.VALUE.getColumnName()
                                                                            + " = '" + columnValue + "'"))
                    .append(";");
            ResultSet rowData = dbCommand.executeDbQuery(command, parent);
        ...
        )
        {
            ...
            eventLog.logFailEvent(parent,
                                  "Cannot load data from the custom values table; cause '" + se.getMessage() + "'",
    ...
    protected void storeRateParameters(Component parent)
    {
        ...
        String comment = rateHandler.getMaxSecondsPerMsg() + "," + rateHandler.getMaxMsgsPerSecond() + ","
                         + rateHandler.isIncludeUneven();

        // Step through each stream
        for (RateInformation rateInfo : rateHandler.getRateInformation())
        {
            // Build the string containing the rate name and parameters. The name is placed
            // within double quotes
            comment += ",\"" + rateInfo.getRateName() + "\",\"" + rateInfo.getStreamName() + "\","
                       + rateInfo.getMaxMsgsPerCycle() + "," + rateInfo.getMaxBytesPerSec();
        }
    ...
    {
        if (!updatingDefaultTableMembers.contentEquals("Updating"))
        {
            updatingDefaultTableMembers = "Updating";
            // Update the pre-loaded table members in the background
            CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand()
            {
                /**************************************************************************************
                ...
                protected void execute()
                {
                    ...
                    updatingDefaultTableMembers = "Updated";
                }
            });
        }
    }
    ...
    /**********************************************************************************************
     * Are the pre-loaded table members currently being updated?
     *
     * @return String representing if the table members are being updated
     *********************************************************************************************/
    protected String defaultTableMembersUpdating()
    {
        return updatingDefaultTableMembers;
    }
    ...
    /**********************************************************************************************
     * Create a list of all prototype tables with their child tables (prototypes and instances), and
     * primitive variables (if specified). The table must contain all of the protected columns defined
     * for a structure in order for its members to be determined. Non-structure tables are included in
     * the returned list, but by definition have no members
     *
     * @param memberType Type of table members to load: TABLES_ONLY to exclude primitive variables or
     ...
     * @param parent     GUI component over which to center any error dialog
     *
     * @return List containing the table member information. For structure tables the member tables are
     *         included, along with primitive variables (if specified), sorted by variable name or row
     *         index as specified
     *********************************************************************************************/
    protected List<TableMembers> loadTableMembers(TableMemberType memberType, boolean sortByName,
                                                  final Component parent)
    {
        ...
        try
        {
            ...
            // information returned by the PostgreSQL function also returns the bit length, rate(s),
            // and enumeration(s) for each member; the enumeration information currently isn't used
            ResultSet rowData = dbCommand.executeDbQuery(new StringBuilder("SELECT * FROM ")
                    .append(sortByName ? "get_table_members_by_name();" : "get_table_members_by_index();"), parent);
            ...
            while (doLoop)
            {
                // Initialize the data type, variable, bit length, rate(s), and enumeration(s) lists
                ...
                do
                {
                    // Get the data type, variable name, bit length, and rate(s) from this query row
                    ...
                    // Check if a data type and variable name exist, and that the data type is not a primitive type
                    // (i.e., this is a structure) or if primitive types are to be included
                    if (dataType != null && !dataType.isEmpty() && variableName != null && !variableName.isEmpty()
                        && (!dataTypeHandler.isPrimitive(dataType) || memberType == TableMemberType.INCLUDE_PRIMITIVES))
                    {
                        ...
                        // Check if a variable name is already in the list and that the data is sorted by variable name
                        if (addIndex != 0 && sortByName)
                        {
                            ...
                            // Check if the current and previous variables are members of the same array
                            if (ArrayVariable.isArrayMember(variableName) && ArrayVariable.isArrayMember(lastVarName)
                                && ArrayVariable.removeArrayIndex(variableName)
                                        .equals(ArrayVariable.removeArrayIndex(lastVarName)))
                            {
                                // The PostgreSQL function that obtains the table members sorted by variable name
                                // treats array indices as strings, so array member [10] appears immediately after
                                // member [1] instead of after [9]. This code section determines the position in
                                // the list where the current array member should be placed relative to those
                                // members already in the list, sorted numerically by array dimension value(s)

                                boolean notFound = true;

                                do
                                {
                                    // Compare the two array members numerically, by array dimension value(s)
                                    ...
                                    {
                                        // Set the flag indicating that the position of the current variable
                                        // in the list is located and stop checking the dimension values
                                        notFound = false;
                                    }
                                } while (notFound && addIndex > 0);
                                // Continue to adjust the insertion index as long as the current variable's
                                // array indices places it prior to the that of the variable at the current
                                // insertion index. The test for addIndex > 0 accounts for the possibility
                    ...
                    // Go to the next row in the query results; set the flag to true if the row exists
                    doLoop = rowData.next();
                ...
                // Add the table name, table type, and the table's member information to the members list
                tableMembers.add(new TableMembers(comment[TableCommentIndex.NAME.ordinal()],
                                                  comment[TableCommentIndex.TYPE.ordinal()], dataTypes, variableNames,
                                                  bitLengths, rates));
            }

            rowData.close();

            // Create storage for the tables not added above; i.e., for non-structure tables
            // and for structure tables containing no rows
            ...
            for (String tableName : tableNames)
            {
                ...
                if (!isFound)
                {
                    ...
                    if (tableInfo.getType().equals(TYPE_ENUM) && memberType == TableMemberType.INCLUDE_PRIMITIVES)
                    {
                        ...
                        {
                            EnumVarNames.add(tableInfo.getData().get(index)[typeDefn
                                    .getColumnIndexByUserName(DefaultColumn.ENUM_NAME.getName())].toString());
                            EnumDataTypes.add("");
                            EnumBitLenths.add("");
                            String[] newRate = {""};
            ...
            // Add the new members to the member list. The list now contains a reference to all tables
        ...
        )
        {
            ...
            eventLog.logFailEvent(parent, "Cannot load table members; cause '" + se.getMessage() + "'",
    ...
    /**********************************************************************************************
     * Add, modify, and/or delete data in a table. This command is executed in a separate thread since
     ...
     *
     * @param forceUpdate        true to make the changes to other tables; false to only make changes to
     ...
     *                           tables. This is used during a data type update where only the data type
     *                           name has changed in order to speed up the operation
     *
     * @param updateDescription  true to update the table description from the table information; false
     *                           to not change the table description
     *
     * @param updateColumnOrder  true to update the table column order from the table information; false
     *                           to not change the table column order
     *
     * @param updateFieldInfo    true to update the table data fields from the table information; false
     *                           to not change the table data fields
     *
     * @param newDataTypeHandler data type handler with data type modifications. null (or a reference to
     *                           the current data type handler) if the change does not originate from
     *                           the data type editor
     *
     * @param parent             GUI component over which to center any error dialog
     *********************************************************************************************/
    protected void modifyTableDataInBackground(final TableInfo tableInfo, final List<TableModification> additions,
                                               final List<TableModification> modifications,
                                               final List<TableModification> deletions, final boolean forceUpdate,
                                               final boolean skipInternalTables, final boolean updateDescription,
                                               final boolean updateColumnOrder, final boolean updateFieldInfo,
                                               final CcddDataTypeHandler newDataTypeHandler, final Component parent)
    {
        // Execute the command in the background
        CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand()
        {
            ...
            protected void execute()
            {
                modifyTableData(tableInfo, additions, modifications, deletions, forceUpdate, skipInternalTables,
                                updateDescription, updateColumnOrder, updateFieldInfo, newDataTypeHandler, true, true,
                                false, parent);
    ...
    /**********************************************************************************************
     * Add, modify, and/or delete data in a table. If the table is a prototype then its database table
     * is altered; if the table is an instance then the changes are made to the custom values table
     *
     * @param tableInfo          table information
     *
     * @param additions          list of row addition information
     ...
     *                           changes to tables other than the one in which the changes originally
     *                           took place
     *
     * @param skipInternalTables true to not build and execute the commands to update the internal
     *                           tables. This is used during a data type update where only the data type
     ...
     *
     * @param isRootTable        Does the tableInfo belong to a root table? If 'updateListsAndRefs' is
     *                           true then it does not matter what value this is set to.
     *
     * @param ignoreErrors       Should errors be ignored
     *
     * @param parent             GUI component over which to center any error dialog
    ...
    protected boolean modifyTableData(TableInfo tableInfo, List<TableModification> additions,
                                      ...
                                      boolean forceUpdate, boolean skipInternalTables, boolean updateDescription,
                                      boolean updateColumnOrder, boolean updateFieldInfo,
                                      CcddDataTypeHandler newDataTypeHandler, boolean updateListsAndRefs,
                                      boolean isRootTable, boolean ignoreErrors, Component parent)
    {
        ...
        try
        {
            ...
            // Get the name of the table to modify and convert the table name to lower case. PostgreSQL
            // automatically
            // does this, so it's done here just to differentiate the table name from the database commands in
            // the event log
            ...
            if (!skipInternalTables && typeDefinition.isStructure())
            {
                ...
                {
                    // Create the table tree. Suppress any warning messages when creating this tree
                    tableTree = new CcddTableTreeHandler(ccddMain, TableTreeType.STRUCTURES_WITH_PRIMITIVES, true,
                                                         parent);
                }
                ...
                if (!modifications.isEmpty() || !deletions.isEmpty())
                {
                    ...
                    varRefChk = inputTypeHandler.getInputTypeReferences(DefaultInputType.VARIABLE_REFERENCE, parent);
                    isRefFieldChange |= varRefChk.isFieldUsesType();
                }
            }
            // Check if the table represents a command and if there are any modifications or deletions
            // (additions won't
            ...
            {
                // Get the references in the table type and data field internal tables that use the command
                // reference
                // input type. If a command name, code, or argument is changed or deleted then the tables and fields
                // may
                // require updating
            ...
            // Get the references in the table type and data field internal tables that use the message name &
            // ID input
            // type. If a message name or ID is added, changed, or deleted then the tables and fields may
            // require updating
            ...
            // Check if this table is an instance and that the description is the same as the one for the
            // prototype of this table
            if (!tableInfo.isPrototype()
            ...
            // Combine the table, data fields table, table description, and column order update commands. If the
            // flag is set
            // to update the data fields then the entire fields table is rewritten. This must precede applying
            // the table
            // updates since these makes further changes to the fields table
            boolean commandExecuted = false;

            StringBuilder command = new StringBuilder((updateFieldInfo ? modifyFieldsCommand(tableInfo
                    .getTablePath(), tableInfo.getFieldInformation()) : "")
                                                      + (updateDescription ? buildTableDescription(tableInfo
                                                              .getTablePath(), description) : "")
            ...
            // Build the commands to add, modify, and delete table rows, and to update any table cells or data
            // fields that
            // have the message name & ID input type if a message name or ID value is changed
            if (!deletions.isEmpty())
            {
                if (buildAndExecuteDeletionCommand(tableInfo, deletions, dbTableName, typeDefinition,
                                                   skipInternalTables, varRefChk, cmdRefChk, parent))
                {
                    commandExecuted = true;
                }
            }
            ...
            {
                if (buildAndExecuteModificationCommand(tableInfo, modifications, dbTableName, typeDefinition,
                                                       newDataTypeHandler, tableTree, skipInternalTables, varRefChk,
                                                       cmdRefChk, parent))
                {
                    commandExecuted = true;
                }
            ...
            {
                if (buildAndExecuteAdditionCommand(tableInfo, additions, dbTableName, typeDefinition,
                                                   skipInternalTables, ignoreErrors, parent))
                {
                    commandExecuted = true;
                }
            }

            if (updateMessageNameAndIDReference(tableInfo, typeDefinition, modifications, deletions, msgIDRefChk,
                                                parent))
            {
                commandExecuted = true;
            }

            // Check if a command was generated (e.g., the additions, modifications, and deletions lists aren't
            // empty)
            if (commandExecuted)
            {
                ...
                if (!skipInternalTables && typeDefinition.isStructure())
                {
                    ...
                    if (tableInfo.isPrototype() && tableInfo.getData().size() > 0)
                    {
                        ...
                        if (updateLinks)
                        {
                            ...
                            storeInformationTable(InternalTable.LINKS, addLinkHandler.getLinkDefinitions(), null,
                                                  parent);
                        }
                    }
                ...
                eventLog.logEvent(SUCCESS_MSG,
                                  new StringBuilder("Table '" + tableInfo.getProtoVariableName() + "' data modified"));
            }
        }
        catch (
            SQLException se
        )
        {
            ...
            eventLog.logFailEvent(parent, "Cannot modify data in table '" + tableInfo.getProtoVariableName()
                                          + "'; cause '" + se.getMessage() + "'",
                                  "<html><b>Cannot modify data in table '</b>" + tableInfo
                                          .getProtoVariableName() + "<b>'");
            errorFlag = true;
        }
        catch (
            Exception e
                        ...
                        for (int index = 0; index < subTableData.size(); index++)
                        {
                                    ...
                                    && subTableData.get(index)[tableMod.getVariableColumn()].toString().endsWith("]")))
                            {
                                ...
                                if (!dataTypeHandler.isPrimitive(subDataType))
                                {
                                    ...
                                    String subVariableName = subTableData.get(index)[tableMod.getVariableColumn()]
                                            .toString();
                                        ...
                                        || (variablePath.endsWith("]")))
                                    {
                                        ...
                                        command.append("INSERT INTO ").append(InternalTable.FIELDS.getTableName())
                                                .append(" SELECT regexp_replace(")
                                                .append(FieldsColumn.OWNER_NAME.getColumnName()).append(", E'^")
                                                .append(dataType).append("$', E'").append(subVariablePath)
                                                .append("'), ").append(FieldsColumn.FIELD_NAME.getColumnName())
                                                .append(", ").append(FieldsColumn.FIELD_DESC.getColumnName())
                                                .append(", ").append(FieldsColumn.FIELD_SIZE.getColumnName())
                                                .append(", ").append(FieldsColumn.FIELD_TYPE.getColumnName())
                                                .append(", ").append(FieldsColumn.FIELD_REQUIRED.getColumnName())
                                                .append(", ").append(FieldsColumn.FIELD_APPLICABILITY.getColumnName())
                                                .append(", ").append(FieldsColumn.FIELD_VALUE.getColumnName())
                                                .append(", ").append(FieldsColumn.FIELD_INHERITED.getColumnName())
                                                .append(" FROM ").append(InternalTable.FIELDS.getTableName())
                                                .append(" WHERE ").append(FieldsColumn.OWNER_NAME.getColumnName())
                                                .append(" = '").append(dataType).append("' AND ")
            ...
            if ((found == false) || (dataTypeChanged == true))
            {
                ...
                if (((mod.getRowData()[mod.getArraySizeColumn()].equals("")) || (variablePath.endsWith("]"))))
                {
                    ...
                    if (tableInfo.isPrototype())
                    {
                        ...
                        for (int index = 0; index < newVariablePaths.size(); index++)
                        {
                            newVariablePaths.set(index,
                                                 newVariablePaths.get(index) + "," + dataType + "." + variableName);
                        }
                    }
                    ...
                    for (int index = 0; index < newVariablePaths.size(); index++)
                    {
                        ...
                        command.append("INSERT INTO ").append(InternalTable.FIELDS.getTableName())
                                .append(" SELECT regexp_replace(").append(FieldsColumn.OWNER_NAME.getColumnName())
                                .append(", E'^").append(dataType).append("$', E'").append(newVariablePaths.get(index))
                                .append("'), ").append(FieldsColumn.FIELD_NAME.getColumnName()).append(", ")
                                .append(FieldsColumn.FIELD_DESC.getColumnName()).append(", ")
                                .append(FieldsColumn.FIELD_SIZE.getColumnName()).append(", ")
                                .append(FieldsColumn.FIELD_TYPE.getColumnName()).append(", ")
                                .append(FieldsColumn.FIELD_REQUIRED.getColumnName()).append(", ")
                                .append(FieldsColumn.FIELD_APPLICABILITY.getColumnName()).append(", ")
                                .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(", ")
                                .append(FieldsColumn.FIELD_INHERITED.getColumnName()).append(" FROM ")
                                .append(InternalTable.FIELDS.getTableName()).append(" WHERE ")
                                .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = '").append(dataType)
                                .append("' AND ").append(FieldsColumn.FIELD_APPLICABILITY.getColumnName())
                                .append(" != '").append(ApplicabilityType.ROOT_ONLY.getApplicabilityName())
                                .append("'; ");
                    }
                }
            }
    ...
    private boolean buildAndExecuteAdditionCommand(TableInfo tableInfo, List<TableModification> additions,
                                                   String dbTableName, TypeDefinition typeDefn,
                                                   boolean skipInternalTables, boolean ignoreErrors,
                                                   Component parent) throws CCDDException
    {
        ...
        addCmd.append("INSERT INTO ").append(dbTableName).append(" (")
            ...
            if (!skipInternalTables && typeDefn.isStructure())
            {
                    ...
                    if (rootStructures.contains(dataType))
                    {
                        ...
                        if (rootStructures.contains(tableInfo.getRootTable()))
                        {
                                    ...
                                    && add.getRowData()[add.getVariableColumn()].toString().endsWith("]")))
                            {
                                ...
                                valuesAddCmd.append("UPDATE ").append(InternalTable.VALUES.getTableName())
                                        .append(" SET ").append(ValuesColumn.TABLE_PATH.getColumnName())
                                        .append(" = regexp_replace(").append(ValuesColumn.TABLE_PATH.getColumnName())
                                        .append(", E'^").append(dataType).append(",', E'").append(newVariablePath)
                                        .append(",'); ");

                                groupsAddCmd.append("UPDATE ").append(InternalTable.GROUPS.getTableName())
                                        .append(" SET ").append(GroupsColumn.MEMBERS.getColumnName())
                                        .append(" = regexp_replace(").append(GroupsColumn.MEMBERS.getColumnName())
                                        .append(", E'^").append(dataType).append("(,|$)', E'").append(newVariablePath)
                                        .append("\\\\1'); ");

                                fieldsAddCmd.append("UPDATE ").append(InternalTable.FIELDS.getTableName())
                                        .append(" SET ").append(FieldsColumn.OWNER_NAME.getColumnName())
                                        .append(" = regexp_replace(").append(FieldsColumn.OWNER_NAME.getColumnName())
                                        .append(", E'^").append(dataType).append(",', E'").append(newVariablePath)
                                        .append(",'); ");

                                ordersAddCmd.append("UPDATE ").append(InternalTable.ORDERS.getTableName())
                                        .append(" SET ").append(OrdersColumn.TABLE_PATH.getColumnName())
                                        .append(" = regexp_replace(").append(OrdersColumn.TABLE_PATH.getColumnName())
                                        .append(", E'^").append(dataType).append(",', E'").append(newVariablePath)
                                        .append(",'); INSERT INTO ").append(InternalTable.ORDERS.getTableName())
                                        .append(" SELECT ").append(OrdersColumn.USER_NAME.getColumnName())
                                        .append(", regexp_replace(").append(OrdersColumn.TABLE_PATH.getColumnName())
                                        .append(", E'^").append(dataType).append("', E'").append(newVariablePath)
                                        .append("'), ").append(OrdersColumn.COLUMN_ORDER.getColumnName())
                                        .append(" FROM ").append(InternalTable.ORDERS.getTableName()).append(" WHERE ")
                                        .append(OrdersColumn.TABLE_PATH.getColumnName()).append(" = '").append(dataType)
                                        .append("'; ");

                                String orgPathWithChildren = dataType + "(," + PATH_IDENT + ")?";
                                assnsAddCmd.append("UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName())
                                        ...
                                        .append(", E'(?:^").append(orgPathWithChildren).append("|(")
                                        .append(assnsSeparator).append(")").append(orgPathWithChildren)
                                        .append(")', E'\\\\2").append(newVariablePath).append("\\\\1\\\\3', 'g'); ");
                            }
                        }
                ...
                if (dataTypeHandler.isString(dataType) && ArrayVariable.isArrayMember(variableName))
                {
                    ...
                    if (!stringArrays.contains(stringArrayDefn))
                    {
                        ...
                        // Remove all references to the string array from the telemetry scheduler table
        ...
        // Remove the ending comma and space, and append the command's closing
        // semi-colon
        addCmd = CcddUtilities.removeTrailer(addCmd, ", ");
        addCmd.append("; ").append(valuesAddCmd).append(groupsAddCmd).append(fieldsAddCmd).append(ordersAddCmd)
                .append(assnsAddCmd).append(linksDelCmd).append(tlmDelCmd);
        ...
        {
            // Inform the user that the database command failed
            eventLog.logFailEvent(parent, "SQL command failed cause " + e.getMessage() + ".",
    ...
    private boolean buildAndExecuteModificationCommand(TableInfo tableInfo, List<TableModification> modifications,
                                                       ...
                                                       CcddTableTreeHandler tableTree, boolean skipInternalTables,
                                                       ...
                                                       ReferenceCheckResults cmdRefChkResults, Component parent)
    {
            ...
            if (tableInfo.isPrototype())
            {
                ...
                for (int column = 0; column < mod.getRowData().length; column++)
                {
                        ...
                        || !mod.getOriginalRowData()[column].equals(mod.getRowData()[column]))
                    {
                        ...
                        modCmd.append(typeDefn.getColumnNamesDatabaseQuoted()[column]).append(" = ")
                                .append(delimitText(mod.getRowData()[column])).append(", ");
                    }
                }
                    ...
                    if (variableChanged || dataTypeChanged || arraySizeChanged || bitLengthChanged || rateChanged)
                    {
                            ...
                            && rootStructures.contains(newDataType))
                        {
                            ...
                            valuesModCmd.append("UPDATE ").append(InternalTable.VALUES.getTableName()).append(" SET ")
                                    ...
                                    .append(", E'^" + newDataType + ",', E'").append(newVariablePath + ",'); ");

                            groupsModCmd.append("UPDATE ").append(InternalTable.GROUPS.getTableName()).append(" SET ")
                                    .append(GroupsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                                    .append(GroupsColumn.MEMBERS.getColumnName()).append(", E'^").append(newDataType)
                                    .append("(,|$)', E'").append(newVariablePath).append("\\\\1'); ");

                            fieldsModCmd.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                                    .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = regexp_replace(")
                                    .append(FieldsColumn.OWNER_NAME.getColumnName()).append(", E'^").append(newDataType)
                                    ...
                                    .append(FieldsColumn.FIELD_NAME.getColumnName()).append(", ")
                                    .append(FieldsColumn.FIELD_DESC.getColumnName()).append(", ")
                                    .append(FieldsColumn.FIELD_SIZE.getColumnName()).append(", ")
                                    .append(FieldsColumn.FIELD_TYPE.getColumnName()).append(", ")
                                    .append(FieldsColumn.FIELD_REQUIRED.getColumnName()).append(", ")
                                    .append(FieldsColumn.FIELD_APPLICABILITY.getColumnName()).append(", ")
                                    .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(", ")
                                    .append(FieldsColumn.FIELD_INHERITED).append(" FROM ")
                                    .append(InternalTable.FIELDS.getTableName()).append(" WHERE ")
                                    .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = '").append(newDataType)
                                    .append("' AND ").append(FieldsColumn.FIELD_APPLICABILITY.getColumnName())
                                    .append(" != '").append(ApplicabilityType.ROOT_ONLY.getApplicabilityName())
                                    .append("'; ");
                            ...
                            ordersModCmd.append("UPDATE ").append(InternalTable.ORDERS.getTableName()).append(" SET ")
                                    ...
                                    .append(OrdersColumn.TABLE_PATH.getColumnName()).append(", E'^").append(newDataType)
                                    .append(",', E'").append(newVariablePath).append(",'); INSERT INTO ")
                                    .append(InternalTable.ORDERS.getTableName()).append(" SELECT ")
                                    .append(OrdersColumn.USER_NAME.getColumnName()).append(", regexp_replace(")
                                    .append(OrdersColumn.TABLE_PATH.getColumnName()).append(", E'^").append(newDataType)
                                    .append("', E'").append(newVariablePath).append("'), ")
                                    .append(OrdersColumn.COLUMN_ORDER.getColumnName()).append(" FROM ")
                                    .append(InternalTable.ORDERS.getTableName()).append(" WHERE ")
                                    .append(OrdersColumn.TABLE_PATH.getColumnName()).append(" = '").append(newDataType)
                                    .append("'; ");
                            ...
                            assnsModCmd.append("UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName())
                                    .append(" SET ").append(AssociationsColumn.MEMBERS.getColumnName())
                                    .append(" = regexp_replace(").append(AssociationsColumn.MEMBERS.getColumnName())
                                    .append(", E'(?:^").append(orgPathWithChildren).append("|(").append(assnsSeparator)
                                    .append(")").append(orgPathWithChildren).append(")', E'\\\\2")
                                    .append(newVariablePath).append("\\\\1\\\\3', 'ng'); ");
                        ...
                        if (tablePathList == null)
                        {
                            ...
                            // prototype table
                            tablePathList = tableTree
                                    .getTableTreePathArray(tableInfo.getPrototypeName(),
                                                           tableTree.getNodeByNodeName(DEFAULT_INSTANCE_NODE_NAME), -1);
                            ...
                            if (dataTypeChanged && !dataTypeHandler.isPrimitive(oldDataType))
                            {
                                ...
                                // prototype tables
                                tablePathList.addAll(tableTree
                                        .getTableTreePathArray(tableInfo.getPrototypeName(),
                        ...
                        try
                        {
                            ...
                            for (int i = 0; i < modifications.size(); i++)
                            {
                                ...
                                if (variableName.contentEquals(oldVariableName))
                                {
                                    ...
                                    String arraySize = (String) rowData[(int) modifications.get(i)
                                            .getArraySizeColumn()];
                            ...
                            if (!wasShifted)
                            {
                                for (Object[] path : tablePathList)
                                {
                                    ...
                                    {
                                        // Create the commands to update the internal tables for instances of non-array
                                        // member variables of the prototype
                                        // table
                                        if (!orgVarPathEsc.contains("[") || orgVarPathEsc.contains("[0"))
                                        {
                                            ...
                                            groupsModCmd.append(updateVarNameAndDataType(orgVarPathEsc, newVariablePath,
                                                                                         InternalTable.GROUPS
                                                                                                 .getTableName(),
                                                                                         GroupsColumn.MEMBERS
                                                                                                 .getColumnName(),
                                                                                         "", "", true));
                                            ordersModCmd.append(updateVarNameAndDataType(orgVarPathEsc, newVariablePath,
                                                                                         InternalTable.ORDERS
                                                                                                 .getTableName(),
                                                                                         OrdersColumn.TABLE_PATH
                                                                                                 .getColumnName(),
                                                                                         "", "", true));
                                        }
                                        String orgPathWithChildren = orgVarPathEsc + "(," + PATH_IDENT + ")?";

                                        // The associations table should only be updated if it contains a reference to
                                        // the table path
                                        ...
                                        for (int index = 0; index < assnsMemberTables.length; index++)
                                        {
                                            if (assnsMemberTables[index].contains((String) path[path.length - 1]))
                                            {
                                                assnsModCmd.append("UPDATE ")
                                                        ...
                                                        .append(", E'(?:^").append(orgPathWithChildren).append("|(")
                                                        .append(assnsSeparator).append(")").append(orgPathWithChildren)
                                                        .append(")', E'\\\\2").append(newVariablePath)
                                                        .append("\\\\1\\\\3', 'ng'); ");
                                        ...
                                        if (!dataTypeChanged && !bitLengthChanged && !rateChanged)
                                        {
                                            ...
                                            linksModCmd
                                                    .append(updateVarNameAndDataType(orgVarPathEsc, newVariablePath,
                                    ...
                                    if (bitLengthChanged && !dataTypeChanged && !rateChanged)
                                    {
                                        ...
                                        linksModCmd
                                                .append(updateVarNameAndDataType(orgVarPathEscBit, newVariablePathBit,
                                                                                 InternalTable.LINKS.getTableName(),
                                                                                 LinksColumn.MEMBER.getColumnName(), "",
                                                                                 "", true));
                                        ...
                                        && !dataTypeHandler.isPrimitive(oldDataType))
                                    {
                                        ...
                                        valuesModCmd.append("DELETE FROM ").append(InternalTable.VALUES.getTableName())
                                                .append(" WHERE ").append(ValuesColumn.TABLE_PATH.getColumnName())
                                                .append(" ~ E'^").append(orgVarPathEsc).append(",'; ")
                                                .append(updateVarNameAndDataType(orgVarPathEsc, newVariablePath,
                                        ...
                                        groupsModCmd.append("DELETE FROM ").append(InternalTable.GROUPS.getTableName())
                                                .append(" WHERE ").append(GroupsColumn.MEMBERS.getColumnName())
                                                .append(" ~ E'^").append(pathMatch).append("'; ");

                                        fieldsModCmd.append("DELETE FROM ").append(InternalTable.FIELDS.getTableName())
                                                .append(" WHERE ").append(FieldsColumn.OWNER_NAME.getColumnName())
                                                .append(" ~ E'^").append(pathMatch).append("'; ");
                                        ...
                                        ordersModCmd.append("DELETE FROM ").append(InternalTable.ORDERS.getTableName())
                                                .append(" WHERE ").append(OrdersColumn.TABLE_PATH.getColumnName())
                                                .append(" ~ E'^").append(pathMatch).append("'; ");
                                        ...
                                        assnsModCmd.append("UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName())
                                                ...
                                                .append(" = regexp_replace(")
                                                .append(AssociationsColumn.MEMBERS.getColumnName())
                                                .append(", E'^" + orgPathWithChildren).append("', E'', 'ng'); UPDATE ")
                                                .append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                                                .append(AssociationsColumn.MEMBERS.getColumnName())
                                                .append(" = regexp_replace(")
                                                .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'")
                                                .append(assnsSeparator).append(orgPathWithChildren + "', E'', 'ng'); ");
                                        ...
                                        && (oldArraySize.isEmpty() || newArraySize.isEmpty()))
                                    {
                                        ...
                                        if (!newArraySize.isEmpty())
                                        {
                                            ...
                                            assnsModCmd.append("UPDATE ")
                                                    ...
                                                    .append(" = regexp_replace(")
                                                    .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'^")
                                                    .append(orgVarPathEsc).append("(?:").append(assnsSeparator)
                                                    .append("|,").append(PATH_IDENT).append("|$)', E'', 'ng'); ");
                                        }
                                    ...
                                    // If the variable was completely removed from the table then we need to clean up
                                    // the
                                    ...
                                    if (variableStillExists == false)
                                    {
                                        fieldsModCmd.append("DELETE FROM ").append(InternalTable.FIELDS.getTableName())
                                                .append(" WHERE ").append(FieldsColumn.OWNER_NAME.getColumnName())
                                                .append(" LIKE '%").append(orgVariablePath).append("%'; ");
                                    }
                                }
                            }
                        }
                        ...
                        )
                        {
                            // Inform the user that the database command failed
                            e.printStackTrace();
                            eventLog.logFailEvent(parent, "Building command failed cause " + e.getMessage() + ".",
                                                  "<html><b>SQL command failed</b>");
                        }
                        ...
                        if (variableChanged || arraySizeChanged)
                        {
                            ...
                            modifyVariableReference(tableInfo, oldVariableName, newVariableName, oldDataType,
                                                    newDataType, varRefChkResults, valuesModCmd, fieldsModCmd);
                        }
                    }
                }
                ...
                else if (typeDefn.isCommand())
                {
                        ...
                        || !oldCommandArg.equals(newCommandArg))
                    {
                        ...
                        // arguments change
                        modifyCommandReference(tableInfo.getTablePath(), oldCommandName, newCommandName, oldCommandCode,
                                               newCommandCode, oldCommandArg, newCommandArg, cmdRefChkResults,
                                               valuesModCmd, fieldsModCmd);
                    }
                }
                ...
                if (assnsModCmd.length() != 0)
                {
                    ...
                    assnsModCmd.append("UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                            .append(AssociationsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                            .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'^").append(assnsSeparator)
                            .append("', E'', 'ng'); UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName())
                            .append(" SET ").append(AssociationsColumn.MEMBERS.getColumnName())
                            .append(" = regexp_replace(").append(AssociationsColumn.MEMBERS.getColumnName())
                            .append(", E'").append(assnsSeparator).append("$', E'', 'ng'); ");
                }
                ...
                modCmd = CcddUtilities.removeTrailer(modCmd, ", ").append(" WHERE ")
                        .append(typeDefn.getColumnNamesDatabase()[DefaultColumn.PRIMARY_KEY.ordinal()]).append(" = ")
                        .append(mod.getRowData()[DefaultColumn.PRIMARY_KEY.ordinal()]).append("; ");
            }
            ...
            else if (!skipInternalTables && typeDefn.isStructure())
            {
                ...
                for (int column = 0; column < mod.getRowData().length; column++)
                {
                    ...
                    if (!mod.getOriginalRowData()[column].equals(mod.getRowData()[column]))
                    {
                        ...
                        // custom values table
                        modCmd.append("DELETE FROM ").append(InternalTable.VALUES.getTableName()).append(" WHERE ")
                                .append(ValuesColumn.TABLE_PATH.getColumnName()).append(" = '").append(variablePath)
                                .append("' AND ").append(ValuesColumn.COLUMN_NAME.getColumnName()).append(" = '")
                                .append(typeDefn.getColumnNamesUser()[column]).append("';");
                        ...
                        {
                            modCmd.append(" INSERT INTO ").append(InternalTable.VALUES.getTableName()).append(" (")
                                    .append(ValuesColumn.TABLE_PATH.getColumnName()).append(", ")
                                    .append(ValuesColumn.COLUMN_NAME.getColumnName()).append(", ")
                                    .append(ValuesColumn.VALUE.getColumnName()).append(") VALUES ('")
                                    .append(variablePath).append("', '").append(typeDefn.getColumnNamesUser()[column])
                                    .append("', ").append(delimitText(mod.getRowData()[column])).append("); ");
                        }
                    }
                }
                ...
                for (int rateIndex : mod.getRateColumn())
                {
                    ...
                    if (!oldRate.equals(newRate))
                    {
                        ...
                        deleteLinkAndTlmPathRef(mod.getRowData()[mod.getArraySizeColumn()].toString(),
                                                mod.getRowData()[mod.getVariableColumn()].toString(), variablePath,
                                                variablePathEsc, linksDelCmd, tlmDelCmd);
                        break;
                    }
                }
            }
        ...
        modCmd.append(valuesModCmd).append(groupsModCmd).append(fieldsModCmd).append(ordersModCmd).append(assnsModCmd)
                .append(linksModCmd).append(tlmModCmd).append(linksDelCmd).append(tlmDelCmd);
     ...
     *
     * @param parentPath  If this data type is used to define a member of a root table then what is the
     *                    path to the root table?
     *
     * @param currVarName If this data type is used to define a member of a root table then what is the
     *                    name of the member?
     *
     * @param parent      GUI component over which to center any error dialog
     *
     ...
     *********************************************************************************************/
    private StringBuilder updateTableFields(String dataType, String parentPath, String currVarName, Component parent,
            ...
            for (int x = 0; x < tableData.size(); x++)
            {
                ...
                if (!dataTypeHandler.isPrimitive(tableData.get(x)[dataTypeIndex].toString()))
                {
                    ...
                    modCmd.append(copyDataFieldCommand(CcddFieldHandler.getFieldTypeName(currTableInfo.getType()),
                                                       childTableDbName, parent));

                    // Grab the table data for this non-primitive data type
                    List<Object[]> childTableData = loadTableData(tableData.get(x)[dataTypeIndex].toString(), true,
                                                                  true, false, parent).getData();
                    ...
                    for (int i = 0; i < childTableData.size(); i++)
                    {
                        ...
                        if (!dataTypeHandler.isPrimitive(childTableData.get(i)[dataTypeIndex].toString()))
                        {
                            ...
                            modCmd.append(updateTableFields(childTableData.get(i)[dataTypeIndex].toString(),
                                                            childTableDbName,
                                                            childTableData.get(i)[variableNameIndex].toString(), parent,
                                                            modCmd));
                        }
                    }
                }
    ...
    private boolean buildAndExecuteDeletionCommand(TableInfo tableInfo, List<TableModification> deletions,
                                                   String dbTableName, TypeDefinition typeDefn,
                                                   boolean skipInternalTables, ReferenceCheckResults varRefChkResults,
                                                   ReferenceCheckResults cmdRefChkResults, Component parent)
    {
        ...
        for (TableModification del : deletions)
        {
            ...
            delCmd.append("DELETE FROM ").append(dbTableName).append(" WHERE ")
                    .append(typeDefn.getColumnNamesDatabase()[DefaultColumn.PRIMARY_KEY.ordinal()]).append(" = ")
                    .append(del.getRowData()[DefaultColumn.PRIMARY_KEY.ordinal()]).append("; ");
            ...
            if (!skipInternalTables && typeDefn.isStructure())
            {
                ...
                String instanceVarPathEsc = ".+," + tableInfo.getPrototypeName() + "\\\\.[^,]*," + dataType + "\\\\."
                ...
                // values table
                valuesDelCmd.append("DELETE FROM ").append(InternalTable.VALUES.getTableName()).append(" WHERE ")
                        .append(ValuesColumn.TABLE_PATH.getColumnName()).append(" ~ E'^").append(protoVarPathEsc)
                        .append("(?:,|:|$)' OR ").append(ValuesColumn.TABLE_PATH.getColumnName()).append(" ~ E'^")
                        .append(instanceVarPathEsc).append("(?:,|:|$)'; ");
                ...
                deleteLinkAndTlmPathRef(del.getRowData()[del.getArraySizeColumn()].toString(), variableName,
                                        variablePath, protoVarPathEsc, linksDelCmd, tlmDelCmd);

                // Append the command to delete the variable from the link and telemetry
                // scheduler tables for all references to the variable in instances of the
                // prototype table
                deleteLinkAndTlmPathRef(del.getRowData()[del.getArraySizeColumn()].toString(), variableName,
                                        variablePath, instanceVarPathEsc, linksDelCmd, tlmDelCmd);
                ...
                if (!dataTypeHandler.isPrimitive(dataType))
                {
                    ...
                    groupsDelCmd.append("DELETE FROM ").append(InternalTable.GROUPS.getTableName()).append(" WHERE ")
                            .append(GroupsColumn.MEMBERS.getColumnName()).append(" ~ E'^").append(protoVarPathEsc)
                            .append("(?:,|$)' OR ").append(GroupsColumn.MEMBERS.getColumnName()).append(" ~ E'^")
                            .append(instanceVarPathEsc).append("(?:,|$)'; ");

                    fieldsDelCmd.append("DELETE FROM ").append(InternalTable.FIELDS.getTableName()).append(" WHERE ")
                            .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" ~ E'^").append(protoVarPathEsc)
                            .append("(?:,|$)' OR ").append(FieldsColumn.OWNER_NAME.getColumnName()).append(" ~ E'^")
                            .append(instanceVarPathEsc).append("(?:,|$)'; ");

                    ordersDelCmd.append("DELETE FROM ").append(InternalTable.ORDERS.getTableName()).append(" WHERE ")
                            .append(OrdersColumn.TABLE_PATH.getColumnName()).append(" ~ E'^").append(protoVarPathEsc)
                            .append("(?:,|$)' OR ").append(OrdersColumn.TABLE_PATH.getColumnName()).append(" ~ E'^")
                            .append(instanceVarPathEsc).append("(?:,|$)'; ");

                    String protoPathWithChildren = protoVarPathEsc + "(?:," + PATH_IDENT + ")?";
                    String instancePathWithChildren = instanceVarPathEsc + "(?:," + PATH_IDENT + ")?";
                    assnsDelCmd.append("UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                            .append(AssociationsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                            .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'^")
                            .append(protoPathWithChildren).append("', E'', 'ng'); UPDATE ")
                            .append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                            .append(AssociationsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                            .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'^")
                            .append(instancePathWithChildren).append("', E'', 'ng'); ");
                }

                // Blank any table cells or data fields with the variable reference input type
                // containing the deleted variable
                deleteVariableReference(tableInfo.getPrototypeName(), variableName, varRefChkResults, valuesDelCmd,
                                        fieldsDelCmd);
            }
            // Check if this is a command table
            else if (typeDefn.isCommand())
            {
                ...
                deleteCommandReference(del.getRowData()[typeDefn
                        .getColumnIndexByInputType(DefaultInputType.COMMAND_NAME)].toString(), cmdRefChkResults,
                                       valuesDelCmd, fieldsDelCmd);
            }
        }
        ...
        if (assnsDelCmd.length() != 0)
        {
            ...
            assnsDelCmd.append("UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                    .append(AssociationsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                    .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'^").append(assnsSeparator)
                    .append("', E'', 'ng'); UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                    .append(AssociationsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                    .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'").append(assnsSeparator)
                    .append("$', E'', 'ng'); ");
        }
        ...
        delCmd.append(valuesDelCmd).append(groupsDelCmd).append(fieldsDelCmd).append(ordersDelCmd).append(assnsDelCmd)
                .append(linksDelCmd).append(tlmDelCmd);
     ...
     *
     * @param columnName      name of the column in the internal table that contains the variable path
     *
     * @param captureIn       pattern match to capture any character(s) that precede(s) the variable
     *                        path; blank if none
     *
     * @param captureOut      pattern for replacing the captured character(s) specified in captureIn;
     *                        blank if none
     *
     ...
     *********************************************************************************************/
    private String updateVarNameAndDataType(String orgVariablePath, String newVariablePath, String tableName,
                                            String columnName, String captureIn, String captureOut,
                                            boolean includeChildren)
    {
        ...
        if (orgVariablePath.contains("["))
        {
            ...
            if (!orgVariablePath.matches(".+\\[[1-9].*"))
            {
                ...
                // Create the command to update the internal table for instances of array variables of
                // the prototype table
                command.append("UPDATE ").append(tableName).append(" SET ").append(columnName)
                        .append(" = regexp_replace(").append(columnName).append(", E'^").append(captureIn)
                        .append(orgVariablePath).append((includeChildren ? "(,.*|$)" : "$")).append("', E'")
                        .append(captureOut).append(newVariablePath).append((includeChildren ? "\\\\" + captureGrp : ""))
                        .append("'); ");
            }
        }
        ...
        else
        {
            ...
            command.append("UPDATE ").append(tableName).append(" SET ").append(columnName).append(" = regexp_replace(")
                    .append(columnName).append(", E'^").append(captureIn).append(orgVariablePath)
                    .append((includeChildren ? "(,.*|:\\\\d+|$)" : "$")).append("', E'").append(captureOut)
                    .append(newVariablePath).append((includeChildren ? "\\\\" + captureGrp : "")).append("'); ");
        }
    ...
    private ToolTipTreeNode copyPrototypeTableTreeNode(String tableName, CcddTableTreeHandler tableTree)
    {
        ...
        if (tableNode != null)
        {
            ...
            for (int childIndex = 0; childIndex < tableNode.getChildCount(); childIndex++)
            {
                ...
                orgTableNode.add(new ToolTipTreeNode(((ToolTipTreeNode) tableNode.getChildAt(childIndex))
    ...
    /**********************************************************************************************
     * Build the commands to delete variable references in the links and telemetry scheduler tables that
     * are no longer valid due to changes in bit-packing
     *
     * @param orgTableNode reference to the node prior to the update
     *
     * @param parent       GUI component over which to center any error dialog
     *
     * @return Commands to delete variable references in the links and telemetry scheduler tables that
     *         are no longer valid due to changes in bit-packing
     *********************************************************************************************/
    private String updateLinksAndTlmForPackingChange(ToolTipTreeNode orgTableNode, Component parent)
    {
        ...
        CcddTableTreeHandler tableTree = new CcddTableTreeHandler(ccddMain, TableTreeType.STRUCTURES_WITH_PRIMITIVES,
                                                                  parent);
        ...
        if (updTableNode != null)
        {
            ...
            for (int childIndex = 0; childIndex < updTableNode.getChildCount(); childIndex++)
            {
                ...
                BitPackNodeIndex nodeIndex = tableTree
                        .getBitPackedVariables((ToolTipTreeNode) updTableNode.getChildAt(childIndex));
                ...
                if (nodeIndex.getFirstIndex() != nodeIndex.getLastIndex())
                {
                    ...
                    for (int packIndex = nodeIndex.getFirstIndex(); packIndex <= nodeIndex.getLastIndex(); packIndex++)
                    {
                        ...
                        packMembers.add(((ToolTipTreeNode) updTableNode.getChildAt(packIndex)).getUserObject()
        ...
        // ////////////////////////////////////////////////////////////////////////////////////////
        // Create lists of each group of bit-packed variables in the table as it existed
        // prior to
        // the updates being applied. Also add the variables to the list containing
        // every
        // bit-packed variable (if not already in the list). Check if the group still
        // exists in the
        // updated table (i.e. contains the same bit-packed members; order or position
        // is ignored).
        // If the group does exist then remove its members from the list of variables to
        // be removed
        // ////////////////////////////////////////////////////////////////////////////////////////
        ...
        for (int childIndex = 0; childIndex < orgTableNode.getChildCount(); childIndex++)
        {
            ...
            if (nodeIndex.getFirstIndex() != nodeIndex.getLastIndex())
            {
                ...
                for (int packIndex = nodeIndex.getFirstIndex(); packIndex <= nodeIndex.getLastIndex(); packIndex++)
                {
                    ...
                    String variable = ((ToolTipTreeNode) orgTableNode.getChildAt(packIndex)).getUserObject().toString()
                            .replaceFirst(":\\d+", "");
                ...
                // ////////////////////////////////////////////////////////////////////////////////
                // Compare the bit-packed group of variables in the table originally to the
                // groups
                // in the table with the updates. If the group still exists then remove its
                // members
                ...
                // bit-packed status, and therefore need to be removed from the links and
                // telemetry
                // scheduler tables
                // ////////////////////////////////////////////////////////////////////////////////
        ...
        // Step through the list of variables that have changed their bit-pack status
        // (no longer
        ...
        for (String variable : removeMembers)
        {
            ...
            String packPath = orgTableNode.getUserObject().toString() + "(?:\\\\.[^,]*)?,"
                              + CcddUtilities.escapePostgreSQLReservedChars(variable) + "(?::\\\\d+|$)";
    ...
    /**********************************************************************************************
     * Update the link definitions when a string variable's size is increased (i.e., a new member is
     * added to the array)
     *
     ...
     *********************************************************************************************/
    private void updateLinksForStringMemberAddition(String protoTable, String dataType, String variableName)
    {
        ...
        // Get the reference to the link definitions in order to shorten subsequent
        // calls
        List<String[]> linkDefns = addLinkHandler.getLinkDefinitions();

        // Get the variable name without the string dimension (leaving any other array
        // dimensions)
        String stringVarNameDefn = ArrayVariable.removeStringSize(variableName);

        // Extract the string dimension value from the variable name, then decrement it
        // by one -
        // this is used to find the existing string array member immediately preceding
        // the new
        // member
                ...
                addLinkHandler.getLinkDefinitions()
                        .add(index + 1,
                             new String[] {linkDefns.get(index)[LinksColumn.RATE_NAME.ordinal()],
                                           ...
                                           linkMember
                                                   .replaceFirst("(" + protoTable + "(?:,|\\.[^,]+,)" + dataType + "\\."
                                                                 + stringVarNameDefn + "\\[)" + stringIndex + "(\\])",
                                                                 "$1" + (stringIndex + 1) + "$2")});
     ...
     *
     * @param linksPath table/variable path to remove from the links table. Leading or trailing regular
     *                  expression constructs must surround the path, and any reserved PostgreSQL
     *                  characters in the path must be escaped
     *
     * @param linksCmd  StringBuilder containing the existing links table deletion command. If empty
     *                  then a new command is initiated; otherwise the deletions are appended to the
     *                  existing command
     *********************************************************************************************/
    private void deleteLinkPathRef(String linksPath, StringBuilder linksCmd)
    {
        linksCmd.append("DELETE FROM ").append(InternalTable.LINKS.getTableName()).append(" WHERE ")
                .append(LinksColumn.MEMBER.getColumnName()).append(" ~ E'").append(linksPath).append("'; ");
    }

    /**********************************************************************************************
     * Build the commands to delete the specified table/variable references from the telemetry scheduler
     * table
     *
     ...
     *
     * @param tlmCmd  StringBuilder containing the existing telemetry scheduler table deletion command.
    ...
    {
        tlmCmd.append("DELETE FROM ").append(InternalTable.TLM_SCHEDULER.getTableName()).append(" WHERE ")
                .append(TlmSchedulerColumn.MEMBER.getColumnName()).append(" ~ E'^.*").append(tlmSchSeparator)
                .append(tlmPath).append("'; ");
    }

    /**********************************************************************************************
     * Build the command to delete the specified variable references from the telemetry scheduler table
     *
     * @param variablePaths list of variable paths to remove from the telemetry scheduler table
     *
     * @return Command to delete the specified variable references from the telemetry scheduler table
     *********************************************************************************************/
     ...
     * tables. If the variable is an array definition then amend the regular expression to match all
     * members of the array; skip any subsequent members of the array passed to this method
     *
     ...
     *
     * @param variablePathEsc variable path with any PostgreSQL reserved characters escaped so that the
     ...
     *
     * @param tlmDelCmd       StringBuilder containing the existing telemetry scheduler table deletion
     *                        command
     *********************************************************************************************/
    private void deleteLinkAndTlmPathRef(String arraySize, String variableName, String variablePath,
                                         String variablePathEsc, StringBuilder linksDelCmd, StringBuilder tlmDelCmd)
    {
        ...
        if (!arraySize.isEmpty())
        {
            ...
            {
                // Get the variable path without the array index (this is what appears as the
                // array
                // definition's path)
        ...
        // Check if the variable can be deleted (i.e., this is not an array variable, is
        // an array
        // definition, or is an array member that doesn't have its definition already
        // set to delete
        if (isOkayToDelete)
        {
            // Remove all references to the structure and its children from the links and
            // telemetry
            // scheduler tables
    ...
    /**********************************************************************************************
     * Build the command to remove all references for the specified rate column name in tables of the
     * changed table type in the links and telemetry scheduler tables
     *
     * @param rateColName rate column name
     *
     * @param linksCmd    sub-command for references in the links table to tables of the target type
     *
     * @param tlmSchCmd   sub-command for references in the telemetry scheduler table to tables of the
     *                    target type
     *
     * @return Command to remove all references for the specified rate column name in tables of the
     *         changed table type in the links and telemetry scheduler tables
     *********************************************************************************************/
    private String deleteLinkAndTlmRateRef(String rateColName, StringBuilder linksCmd, StringBuilder tlmSchCmd)
    {
        StringBuilder command = new StringBuilder();
        command.append("DELETE FROM ").append(InternalTable.LINKS.getTableName()).append(" WHERE ")
                .append(LinksColumn.RATE_NAME.getColumnName()).append(" = E'").append(rateColName).append("'")
                .append(linksCmd).append("; DELETE FROM ").append(InternalTable.TLM_SCHEDULER.getTableName())
                .append(" WHERE ").append(TlmSchedulerColumn.RATE_NAME.getColumnName()).append(" = '")
                .append(rateColName).append("'").append(tlmSchCmd).append("; ");

        return command.toString();
    }

    /**********************************************************************************************
     * Build the command to remove all references to the specified rate column name in the links and
     * telemetry scheduler tables
     *
     * @param rateColName rate column name
     *
    ...
    private String deleteAllLinkAndTlmRateRefs(String rateColName)
    {
        StringBuilder command = new StringBuilder();
        command.append("DELETE FROM ").append(InternalTable.LINKS.getTableName()).append(" WHERE ")
                .append(LinksColumn.RATE_NAME.getColumnName()).append(" = '").append(rateColName)
                .append("'; DELETE FROM ").append(InternalTable.TLM_SCHEDULER.getTableName()).append(" WHERE ")
                .append(TlmSchedulerColumn.RATE_NAME.getColumnName()).append(" = '").append(rateColName).append("'; ");

        return command.toString();
    }

    /**********************************************************************************************
     * Update any references to a variable that had its name changed in every table cell and data field
     ...
     * @param varRefChkResults results of the variable reference input type search
     *
     * @param valuesModCmd     StringBuilder containing the existing values table modification command
     *
     * @param fieldsModCmd     StringBuilder containing the existing fields table modification command
     *********************************************************************************************/
    private void modifyVariableReference(TableInfo tableInfo, String oldVariableName, String newVariableName,
                                         String oldDataType, String newDataType, ReferenceCheckResults varRefChkResults,
                                         StringBuilder valuesModCmd, StringBuilder fieldsModCmd)
    {
        ...
        {
            targetVars.add(new String[] {
                                         CcddUtilities.escapePostgreSQLReservedChars(tableInfo.getPrototypeName() + ","
                                                                                     + oldVariableName),
                    ...
                    if (entry.contains(escapedOldVariableName))
                    {
                        if (entry.matches(match))
                        {
                            ...
                            if (!targetVars.contains((Object) targetVar))
                            {
                                ...
                                targetVars.add(new String[] {targetVar, variableHandler
                                        .removeDataTypeFromVariablePath(entry
                                                .replaceFirst(match,
        ...
        for (InputTypeReference varRef : varRefChkResults.getReferences())
        {
            ...
            for (String table : varRef.getTables())
            {
                ...
                for (String[] targetVar : targetVars)
                {
                    ...
                    valuesModCmd.append("UPDATE ").append(dbControl.getQuotedName(table)).append(" SET ")
                            .append(varRef.getColumnDb()).append(" = '").append(targetVar[1]).append("' WHERE ")
                            .append(varRef.getColumnDb()).append(" = E'").append(targetVar[0]).append("'; ");
        ...
        if (varRefChkResults.isFieldUsesType())
        {
            ...
            for (String targetVar[] : targetVars)
            {
                ...
                fieldsModCmd.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                        .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = E'").append(targetVar[1])
                        .append("' WHERE ").append(FieldsColumn.FIELD_TYPE.getColumnName()).append(" = E'")
                        .append(DefaultInputType.VARIABLE_REFERENCE.getInputName()).append("' AND ")
                        .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = E'").append(targetVar[0])
                        .append("'; ");
            }
        }
    }
    ...
    /**********************************************************************************************
     * Blank any references to a variable that has been deleted in every table cell and data field with
     * a variable reference input type
     *
     ...
     *********************************************************************************************/
    private void deleteVariableReference(String protoTableName, String variableName,
                                         ReferenceCheckResults varRefChkResults, StringBuilder valuesDelCmd,
                                         StringBuilder fieldsDelCmd)
    {
        // Get the path without the data type(s)
        String instanceVarPathEscNoDt = "(?:^|.+,)" + protoTableName + ","
                                        + CcddUtilities.escapePostgreSQLReservedChars(variableName) + "(?:,|$)";
        ...
        for (InputTypeReference varRef : varRefChkResults.getReferences())
        {
            ...
            for (String table : varRef.getTables())
            {
                ...
                valuesDelCmd.append("UPDATE " + dbControl.getQuotedName(table) + " SET " + varRef.getColumnDb()
                                    + " = '' WHERE " + varRef.getColumnDb() + " ~ E'" + instanceVarPathEscNoDt + "'; ");

                // Check if the table isn't a root structure
                if (!rootStructures.contains(table))
                {
                    // Remove references to the variable reference from the instances
                    valuesDelCmd.append("UPDATE ").append(InternalTable.VALUES.getTableName()).append(" SET ")
                            .append(ValuesColumn.VALUE.getColumnName()).append(" = '' WHERE ")
                            .append(ValuesColumn.TABLE_PATH.getColumnName()).append(" ~ E'^.+,").append(table)
                            .append("\\\\..*$' AND ").append(ValuesColumn.COLUMN_NAME.getColumnName()).append(" = '")
                            .append(varRef.getColumnVisible()).append("' AND ")
                            .append(ValuesColumn.VALUE.getColumnName()).append(" ~ E'").append(instanceVarPathEscNoDt)
                            .append("'; ");
                }
            }
        }
        ...
        if (varRefChkResults.isFieldUsesType())
        {
            ...
            fieldsDelCmd.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                    .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = '' WHERE ")
                    .append(FieldsColumn.FIELD_TYPE.getColumnName()).append(" = E'")
                    .append(DefaultInputType.VARIABLE_REFERENCE.getInputName()).append("' AND ")
                    .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" ~ E'").append(instanceVarPathEscNoDt)
                    .append("'; ");
        }
    }

    /**********************************************************************************************
     * Update any references to a command that had its name, code, or argument path changed in every
     * table cell and data field with a command reference input type
     *
     ...
     * @param fieldsModCmd     StringBuilder containing the existing fields table modification command
     *********************************************************************************************/
    private void modifyCommandReference(String tablePath, String oldCommandName, String newCommandName,
                                        String oldCommandCode, String newCommandCode, String oldCommandArg,
                                        String newCommandArg, ReferenceCheckResults cmdRefChkResults,
                                        StringBuilder valuesModCmd, StringBuilder fieldsModCmd)
    {
        // Build the original and new command references
        String oldCmdRef = commandHandler.buildCommandReference(oldCommandName, oldCommandCode, oldCommandArg,
                                                                tablePath);
        String newCmdRef = commandHandler.buildCommandReference(newCommandName, newCommandCode, newCommandArg,
                                                                tablePath);
        ...
        for (InputTypeReference cmdRef : cmdRefChkResults.getReferences())
        {
            ...
            for (String table : cmdRef.getTables())
            {
                ...
                valuesModCmd.append("UPDATE ").append(dbControl.getQuotedName(table)).append(" SET ")
                        .append(cmdRef.getColumnDb()).append(" = '").append(newCmdRef).append("' WHERE ")
                        .append(cmdRef.getColumnDb()).append(" = E'").append(oldCmdRef).append("'; ");
            }
        }
        ...
        if (cmdRefChkResults.isFieldUsesType())
        {
            ...
            fieldsModCmd.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                    .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = E'").append(newCmdRef)
                    .append("' WHERE ").append(FieldsColumn.FIELD_TYPE.getColumnName()).append(" = E'")
                    .append(DefaultInputType.COMMAND_REFERENCE.getInputName()).append("' AND ")
                    .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = E'").append(oldCmdRef).append("'; ");
        }
    }

    /**********************************************************************************************
     * Blank any references to a command that has been deleted in every table cell and data field with a
     ...
     * @param fieldsDelCmd     StringBuilder containing the existing fields table deletion command
     *********************************************************************************************/
    private void deleteCommandReference(String commandName, ReferenceCheckResults cmdRefChkResults,
                                        StringBuilder valuesDelCmd, StringBuilder fieldsDelCmd)
    {
        // Append the regular expression needed to include the command code, table, and
        // number of
        // arguments
        commandName += " \\(.*";
        ...
        for (InputTypeReference cmdRef : cmdRefChkResults.getReferences())
        {
            ...
            for (String table : cmdRef.getTables())
            {
                ...
                valuesDelCmd.append("UPDATE ").append(dbControl.getQuotedName(table)).append(" SET ")
                        .append(cmdRef.getColumnDb()).append(" = '' WHERE ").append(cmdRef.getColumnDb())
                        .append(" ~ E'").append(commandName).append("'; ");
            }
        }
        ...
        if (cmdRefChkResults.isFieldUsesType())
        {
            ...
            fieldsDelCmd.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                    ...
                    .append(FieldsColumn.FIELD_TYPE.getColumnName()).append(" = E'")
                    .append(DefaultInputType.COMMAND_REFERENCE.getInputName()).append("' AND ")
                    .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" ~ E'").append(commandName).append("'; ");
        }
    }

    /**********************************************************************************************
     * Build the commands to update any references to a message name or ID that has been modified or
     * deleted in every table cell and data field with a message name and ID reference input type
     *
     * @param tableInfo          table information
     *
        ...
        if (!msgIDRefChkResults.getReferences().isEmpty() || msgIDRefChkResults.isFieldUsesType())
        {
            ...
            for (int index = 0; index < msgNameIDs.size(); index++)
            {
                ...
                if (!msgNameIDs.get(index).isEmpty())
                {
                    ...
                    String msgNameIDOwner = nameAndID[0] + " (ID: " + nameAndID[1] + ", owner: "
                                            + tableInfo.getTablePath() + ")";
                    String newMsgNameIDOwner = newMsgNameIDs.get(index).isEmpty() ? ""
                                                                                  : newNameAndID[0] + " (ID: "
                                                                                    + newNameAndID[1] + ", owner: "
                                                                                    + tableInfo.getTablePath() + ")";
                    ...
                    for (InputTypeReference msgIDRef : msgIDRefChkResults.getReferences())
                    {
                        ...
                        for (String table : msgIDRef.getTables())
                        {
                            ...
                            msgIDCmd.append("UPDATE ").append(dbControl.getQuotedName(table)).append(" SET ")
                                    .append(msgIDRef.getColumnDb()).append(" = '").append(newMsgNameIDOwner)
                                    .append("' WHERE ").append(msgIDRef.getColumnDb()).append(" = E'")
                                    .append(msgNameIDOwner).append("'; ");
                        }
                    }
                    ...
                    if (msgIDRefChkResults.isFieldUsesType())
                    {
                        ...
                        msgIDCmd.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                                .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = '")
                                .append(newMsgNameIDOwner).append("' WHERE ")
                                .append(FieldsColumn.FIELD_TYPE.getColumnName()).append(" = E'")
                                .append(DefaultInputType.MESSAGE_REFERENCE.getInputName()).append("' AND ")
                                .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = E'").append(msgNameIDOwner)
                                .append("'; ");
                    }
                }
            }
    ...
    /**********************************************************************************************
     * Retrieve a list of internal table data from the database, ignoring script file information tables
     *
     * @param intTable   type of internal table to retrieve
     *
     * @param includeOID true to read in the OID column in addition to the information table columns
     *
     * @param parent     GUI component over which to center any error dialog
     *
     * @return List of the items in the internal table. An empty list is returned if the specified table
     *         is empty or doesn't exist
     *********************************************************************************************/
    protected List<String[]> retrieveInformationTable(InternalTable intTable, boolean includeOID, Component parent)
    {
     ...
     *         is empty or doesn't exist
     *********************************************************************************************/
    protected List<String[]> retrieveInformationTable(InternalTable intTable, boolean includeOID, String scriptName,
                                                      Component parent)
    {
        ...
        try
        {
            ...
            if (isTableExists(intTableName, parent))
            {
                ...
                ResultSet infoData = dbCommand
                        .executeDbQuery(new StringBuilder("SELECT *").append((includeOID ? ", OID" : ""))
                                .append(" FROM ").append(intTableName).append(" ORDER BY OID;"), parent);
        ...
        )
        {
            ...
            eventLog.logFailEvent(parent,
                                  "Cannot load internal table '" + intTableName + "'; cause '" + se.getMessage() + "'",
                                  "<html><b>Cannot load internal table '</b>" + intTableName + "<b>'");
        }
    ...
    /**********************************************************************************************
     * Store the internal table into the database. This command is executed in a separate thread since
     * it can take a noticeable amount time to complete, and by using a separate thread the GUI is
     * allowed to continue to update. The GUI menu commands, however, are disabled until the database
     * command completes execution
     *
     ...
     * @param parent       GUI component over which to center any error dialog
     *********************************************************************************************/
    protected void storeInformationTableInBackground(final InternalTable intTable, final List<String[]> tableData,
                                                     final String tableComment, final Component parent)
    {
        storeInformationTableInBackground(intTable, tableData, null, null, null, tableComment, parent);
    }
     ...
     *
     * @param fieldInformationList list containing the list of data field information for each group
     *                             with a data field update (only applicable to the groups table); null
     *                             if none
     *
     * @param deletedGroups        list containing the names of groups that have been deleted
     *
     * @param invalidLinkVars      list containing the link member variables that should be removed from
     *                             the telemetry scheduler due to the addition of one or more variables;
     *                             null or an empty list if there are no invalid link members
     *
     * @param tableComment         table comment; null if unchanged
     *
     * @param parent               GUI component over which to center any error dialog
    ...
    protected void storeInformationTableInBackground(final InternalTable intTable, final List<String[]> tableData,
                                                     ...
                                                     final List<String> invalidLinkVars, final String tableComment,
                                                     final Component parent)
    {
        // Execute the command in the background
        CcddBackgroundCommand.executeInBackground(ccddMain, parent, new BackgroundCommand()
        {
            ...
            protected void execute()
            {
                storeInformationTable(intTable, tableData, fieldInformationList, deletedGroups, invalidLinkVars,
                                      tableComment, parent);
            }
        });
    }
     ...
     * @param intTable     type of internal table to store
     *
     * @param tableData    array containing the table data to store; unused for the table types internal
     *                     table
     *
     * @param tableComment table comment; null if unchanged
     *
     ...
     *
     *********************************************************************************************/
    protected void updateGroupsTable(List<List<FieldInformation>> fieldInformationList, List<String> deletedGroups,
                                     Component parent)
    {
        StringBuilder command = new StringBuilder();
        ...
        try
        {
            ...
            if (fieldInformationList != null)
            {
                ...
                for (List<FieldInformation> fieldInformation : fieldInformationList)
                {
                    ...
                    command.append(modifyFieldsCommand(fieldInformation.get(0).getOwnerName(), fieldInformation));
                }
            }
            ...
            if (deletedGroups != null)
            {
                ...
                for (String groupName : deletedGroups)
                {
                    ...
                    command.append(modifyFieldsCommand(CcddFieldHandler.getFieldGroupName(groupName), null));
                }
            }
        ...
        {
            // Inform the user that the database command failed
            eventLog.logFailEvent(parent, "Cannot store internal table '" + InternalTable.GROUPS.getTableName()
                                          + "'; cause '" + se.getMessage() + "'",
                                  "<html><b>Cannot store internal table '</b>" + InternalTable.GROUPS
                                          .getTableName() + "<b>'");
        }
    }
     ...
     * @param intTable             type of internal table to store
     *
     * @param tableData            list containing the table data to store; unused for the table types
     *                             internal table
     *
     * @param fieldInformationList list containing the list of data field information for each group
     *                             with a data field update (only applicable to the groups table); null
     ...
     * @param parent               GUI component over which to center any error dialog
     *********************************************************************************************/
    protected void storeInformationTable(InternalTable intTable, List<String[]> tableData,
                                         List<List<FieldInformation>> fieldInformationList, List<String> deletedGroups,
                                         List<String> invalidLinkVars, String tableComment, Component parent)
    {
        boolean errorFlag = false;
        ...
        try
        {
            ...
            switch (intTable)
            {
                ...
                case TLM_SCHEDULER:
                case USERS:
                    // Build the command for storing the script configurations, groups, links, etc. table
                    command.append(storeNonTableTypesInfoTableCommand(intTable, tableData, tableComment, parent));
                    break;
                ...
                case LINKS:
                    ...
                    command.append(storeNonTableTypesInfoTableCommand(intTable, tableData, tableComment, parent))
                            .append(deleteTlmPathRefs(invalidLinkVars));

                    break;

                case TABLE_TYPES:
                    // Build the command for storing the table type definitions table
                    command = new StringBuilder(storeTableTypesInfoTableCommand());
                    break;
            ...
            // Update the table tree information
            if (intTable == InternalTable.GROUPS)
            {
                updateTableTree();
            }
        }
        ...
        )
        {
            // Inform the user that the database command failed
            eventLog.logFailEvent(parent,
                                  "Cannot store internal table '" + intTableName + "'; cause '" + se.getMessage() + "'",
                                  "<html><b>Cannot store internal table '</b>" + intTableName + "<b>'");
            errorFlag = true;
        }
        catch (
            Exception e
        ...
        switch (intTable)
        {
            ...
            case RESERVED_MSG_IDS:
                // Check if the store request originated from the reserved message ID editor
                // dialog
            ...
            case USERS:
                // Check if the store request originated from the user access level editor
                // dialog
     ...
     *********************************************************************************************/
    String storeNonTableTypesInfoTableCommand(InternalTable intTable, List<String[]> tableData, String tableComment,
                                              Component parent)
    {
        // Get the internal table's name
        String tableName = intTable.getTableName(tableComment);

        // Build the command to delete the information list table if it exists, then the creation commands
        StringBuilder command = new StringBuilder("DROP TABLE IF EXISTS ").append(tableName).append("; CREATE TABLE ")
                .append(tableName).append(" ").append(intTable.getColumnCommand(false))
        ...
        try
        {
            ...
            if (tableComment == null)
            {
                ...
                ResultSet comment = dbCommand.executeDbQuery(new StringBuilder("SELECT obj_description('public.")
                        .append(tableName).append("'::regclass, 'pg_class');"), parent);
            ...
            if (tableComment != null)
            {
                ...
                // recreated
                command.append("COMMENT ON TABLE ").append(tableName).append(" IS ").append(delimitText(tableComment))
                        .append("; ");
            }
        }
        catch (
        ...
        )
        {
            ...
            eventLog.logFailEvent(parent,
                                  "Cannot obtain comment for internal table '" + tableName + "'; cause '"
                                          + se.getMessage() + "'",
                                  "<html><b>Cannot obtain comment for internal table '</b>" + tableName + "<b>'");
        }
    ...
    {
        // Build the command to delete the existing table type definitions table and
        // create the new
        // one
        String cmd = "DROP TABLE IF EXISTS " + InternalTable.TABLE_TYPES.getTableName() + "; CREATE TABLE "
        ...
        for (TypeDefinition typeDefn : tableTypeHandler.getTypeDefinitions())
        {
            ...
            for (int index = 0; index < typeDefn.getColumnNamesDatabase().length; index++)
            {
                ...
                command.append("(").append(delimitText(typeDefn.getName())).append(", ").append(index).append(", '")
                        .append(typeDefn.getColumnNamesDatabase()[index]).append("', '")
                        .append(typeDefn.getColumnNamesUser()[index]).append("', ")
                        .append(delimitText(typeDefn.getColumnToolTips()[index])).append(", '")
                        .append(typeDefn.getInputTypes()[index].getInputName()).append("', ")
                        .append(typeDefn.isRowValueUnique()[index]).append(", ").append(typeDefn.isRequired()[index])
                        .append(", ").append(typeDefn.isStructureAllowed()[index]).append(", ")
                        .append(typeDefn.isPointerAllowed()[index]).append("), ");
            }
        }

        // Replace the trailing comma with a semicolon
        command = CcddUtilities.removeTrailer(command, ", ").append("; ")
                .append(dbControl.buildOwnerCommand(DatabaseObject.TABLE, InternalTable.TABLE_TYPES.getTableName()));
                                       ...
                                       final Component parent)
    {
        ...
        try
        {
            ...
            dbCommand.executeDbUpdate(new StringBuilder(dbControl
                    .buildDatabaseCommentCommandAndUpdateInternalTable(dbControl.getProjectName(),
                                                                       dbControl.getDatabaseAdmins(dbControl
                                                                               .getDatabaseName()),
                                                                       true, description)),
                                      parent);
    ...
    /**********************************************************************************************
     * Build the command for updating the data field definitions table for the specified data table,
     * table type, group, or project
     *
     * @param ownerName        name of the table, including the path if this table represents a
     *                         structure, table type, group, or project to which the field(s) belong
     *
    ...
    private String modifyFieldsCommand(String ownerName, List<FieldInformation> fieldInformation)
    {
        ...
        StringBuilder command = new StringBuilder("DELETE FROM ").append(InternalTable.FIELDS.getTableName())
                .append(" WHERE ").append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = '").append(ownerName)
                .append("'; ");
        ...
        if (fieldInformation != null && !fieldInformation.isEmpty())
        {
            ...
            for (FieldInformation fieldInfo : fieldInformation)
            {
                ...
                command.append("('").append(ownerName).append("', ").append(delimitText(fieldInfo.getFieldName()))
                        .append(", ").append(delimitText(fieldInfo.getDescription())).append(", ")
                        .append(fieldInfo.getSize()).append(", ")
                        .append(delimitText(fieldInfo.getInputType().getInputName())).append(", ")
                        .append(String.valueOf(fieldInfo.isRequired())).append(", ")
                        .append(delimitText(fieldInfo.getApplicabilityType().getApplicabilityName())).append(", ")
                        .append(delimitText(fieldInfo.getValue())).append(", ")
    ...
    /**********************************************************************************************
     * Build the command for updating the internal fields table when a root table is modified to be the
     * child of a new table
     *
     ...
     * command is executed in a separate thread since it can take a noticeable amount time to complete,
     * and by using a separate thread the GUI is allowed to continue to update. The GUI menu commands,
     ...
     *********************************************************************************************/
    protected void renameTableType(final String typeName, final String newName,
        ...
        CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand()
        {
            ...
            protected void execute()
            {
                try
                {
                    ...
                    command.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                            .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = ")
                            .append(delimitText(CcddFieldHandler.getFieldTypeName(newName))).append(" WHERE ")
                            .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = ")
                            .append(delimitText(CcddFieldHandler.getFieldTypeName(typeName))).append("; ");

                    // Execute the command to change the table's type name
                    dbCommand.executeDbCommand(command, typeDialog);

                    // Log that renaming the table succeeded
                    eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Table '").append(tableName)
                            .append("' type renamed to '").append(newName).append("'"));
                }
                catch (
                    SQLException se
                )
                {
                    ...
                    eventLog.logFailEvent(typeDialog,
                                          "Cannot rename table type for table '" + tableName + "'; cause '"
                                                      + se.getMessage() + "'",
                                          "<html><b>Cannot rename table type for table '</b>" + tableName + "<b>'");
                    errorFlag = true;
                }
                catch (
                    Exception e
    ...
    /**********************************************************************************************
     * Create a copy of an existing table type. This command is executed in a separate thread since it
     * can take a noticeable amount time to complete, and by using a separate thread the GUI is allowed
     * to continue to update. The GUI menu commands, however, are disabled until the database command
     * completes execution
     *
     ...
     * @param typeDialog reference to the type manager dialog
     *********************************************************************************************/
    protected void copyTableType(final String typeName, final String copyName,
                                 final CcddTableTypeManagerDialog typeDialog)
    {
        // Execute the command in the background
        CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand()
        {
            ...
            protected void execute()
            {
                try
                {
                    ...
                    StringBuilder command = new StringBuilder(storeTableTypesInfoTableCommand())
                            .append(storeNonTableTypesInfoTableCommand(InternalTable.FIELDS, fieldDefinitions, null,
                                                                       typeDialog));
                    ...
                    // Log that renaming the table succeeded
                    eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Table type '").append(typeName)
                            .append("' copied to '").append(copyName).append("'"));
                }
                catch (
                    SQLException se
                )
                {
                    ...
                    eventLog.logFailEvent(typeDialog,
                                          "Cannot copy table type '" + typeName + "'; cause '" + se.getMessage() + "'",
                                          "<html><b>Cannot copy table type '</b>" + typeName + "<b>'");
                    errorFlag = true;
                }
                catch (
                    Exception e
    ...
    /**********************************************************************************************
     * Delete a table type and all tables of the deleted type. This command is executed in a separate
     * thread since it can take a noticeable amount time to complete, and by using a separate thread the
     * GUI is allowed to continue to update. The GUI menu commands, however, are disabled until the
     * database command completes execution
     *
     ...
     * @param parent     GUI component over which to center any error dialog
     *********************************************************************************************/
    protected void deleteTableType(final TypeDefinition typeDefn, final CcddTableTypeManagerDialog typeDialog,
                                   final Component parent)
    {
        // Set the flags to indicate if the type represents a structure and a telemetry
        // structure
            ...
            protected void execute()
            {
                try
                {
                    ...
                    if (tableNames.length != 0)
                    {
                        ...
                        if (new CcddDialogHandler()
                                .showMessageDialog(parent,
                                                   "<html><b>Delete table(s) '</b>" + names + "<b>' of type '</b>"
                                                           + typeDefn.getName()
                                                           + "<b>'?<br><br><i>Warning: This action cannot be undone!",
                                                   "Delete Table(s)", JOptionPane.QUESTION_MESSAGE,
                                                   DialogOption.OK_CANCEL_OPTION) == OK_BUTTON)
                        {
                            ...
                            // internal tables
                            command.append(deleteTableCommand(tableNames, true)).append(" ");
                        }
                    ...
                    if (!errorFlag)
                    {
                        ...
                        eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Table type '").append(typeDefn.getName())
                                .append("'").append(CcddUtilities.removeHTMLTags(names)).append(" deleted"));
                    }
                }
                catch (
                    SQLException se
                )
                {
                    ...
                    eventLog.logFailEvent(parent,
                                          "Cannot delete table type '" + typeDefn.getName() + "'"
                                                  + CcddUtilities.removeHTMLTags(names) + "; cause '" + se.getMessage()
                                                  + "'",
                                          "<html><b>Cannot delete table type '</b>" + typeDefn
                                                  .getName() + "<b>'</b>" + names + "<b>'");
                    errorFlag = true;
                }
                catch (
     ...
     * existing tables of the specified type. This command is executed in a separate thread since it can
     ...
     *                           overwrite only those fields with a matching value, EMPTY to overwrite
     *                           only fields with blank values, or NONE to not overwrite any field
     *                           values
     *
     * @param typeAdditions      list of new columns to add to the tables. Each list item is an array
     *                           containing: [0] column name (user), [1] column input type
     *
     * @param typeModifications  list of name changes of existing columns in the tables. Each list item
     *                           is an array containing: [0] original column name (user), [1] new column
     *                           name (user), [2] column input type
     *
     * @param typeDeletions      list of columns to remove from the tables. Each list item is an array
     *                           containing: [0] column name (user), [1] column input type
     *
     ...
     *
     * @param fieldAdditions     list of new fields to add to tables of this type
     *
     * @param fieldModifications list of changes to existing fields in tables of this type
     *
     * @param fieldDeletions     list of fields to remove from tables of this type
     *
     ...
     *********************************************************************************************/
    protected void modifyTableTypeInBackground(final String tableType, final List<FieldInformation> fieldInformation,
                                               final OverwriteFieldValueType overwriteFields,
                                               final List<String[]> typeAdditions,
                                               final List<String[]> typeModifications,
                                               final List<String[]> typeDeletions, final boolean columnOrderChange,
                                               ...
                                               final CcddTableTypeEditorHandler editor)
    {
        ...
        CcddBackgroundCommand.executeInBackground(ccddMain, new BackgroundCommand()
        {
            ...
            protected void execute()
            {
                modifyTableType(tableType, fieldInformation, overwriteFields, typeAdditions, typeModifications,
                                typeDeletions, columnOrderChange, originalDefn, null, editorDialog, editor);
            }
        });
    }
     ...
     * @param overwriteFields   OverwriteFieldValueType: ALL to overwrite all field values, SAME to
     *                          overwrite only those fields with a matching value, EMPTY to overwrite
     *                          only fields with blank values, or NONE to not overwrite any field values
     *
     * @param typeAdditions     list of new columns to add to the tables. Each list item is an array
     *                          containing: [0] column name (user), [1] column input type
     *
     ...
     * @param columnOrderChange true if the table type's column order changed
     *
     * @param originalDefn      reference to the table type definition prior to making any changes; null
     *                          if this is a new table type
     *
     * @param newDataFields     Reference to the new data fields. This is only needed when no editor is
     *                          provided
     *
     * @param editorDialog      reference to the table type editor dialog
     *
     * @param editor            reference to the table type editor where the change(s) occurred
     *********************************************************************************************/
    protected void modifyTableType(String typeName, List<FieldInformation> fieldInformation,
                                   OverwriteFieldValueType overwriteFields, List<String[]> typeAdditions,
                                   List<String[]> typeModifications, List<String[]> typeDeletions,
                                   boolean columnOrderChange, TypeDefinition originalDefn, List<String[]> newDataFields,
                                   CcddTableTypeEditorDialog editorDialog, CcddTableTypeEditorHandler editor)
    {
        try
        {
            boolean errorFlag = false;
            ...
            if (originalDefn != null)
            {
                ...
                for (String protoName : protoTableNames)
                {
                    ...
                    for (String add[] : typeAdditions)
                    {
                        ...
                        String dbColumn = tableTypeHandler.convertVisibleToDatabase(add[0], add[1], isStructure);

                        // Append the add command
                        command.append("ALTER TABLE ").append(dbTable).append(" ADD COLUMN ").append(dbColumn)
                                .append(" text DEFAULT ''; ");
                    }
                    ...
                    for (String[] mod : typeModifications)
                    {
                        ...
                        String oldDbColumn = tableTypeHandler.convertVisibleToDatabase(mod[0], mod[2], isStructure);
                        String newDbColumn = tableTypeHandler.convertVisibleToDatabase(mod[1], mod[3], isStructure);

                        // Check if the database form of the name changed
                        if (!oldDbColumn.equals(newDbColumn))
                        {
                            // Append the modify command
                            command.append("ALTER TABLE ").append(dbTable).append(" RENAME COLUMN ").append(oldDbColumn)
                                    .append(" TO ").append(newDbColumn + "; ");
                        }
                            ...
                            && inputTypeHandler.getInputTypeByName(mod[3]).getInputFormat() != InputTypeFormat.TEXT)
                        {
                            ...
                            command.append("UPDATE ").append(dbTable).append(" SET ").append(newDbColumn)
                                    .append(" = '' WHERE ").append(newDbColumn).append(" != '' AND ")
                                    .append(newDbColumn).append(" !~ E'^")
                                    .append(inputTypeHandler.getInputTypeByName(mod[3]).getInputMatch()
                    ...
                    for (String[] del : typeDeletions)
                    {
                        ...
                        command.append("ALTER TABLE ").append(dbTable).append(" DROP COLUMN ").append(dbColumn)
                                .append("; ");
                    }
                    ...
                    if (columnOrder.length() != 0)
                    {
                        ...
                        command.append("UPDATE ").append(InternalTable.ORDERS.getTableName()).append(" SET ")
                                .append(OrdersColumn.COLUMN_ORDER.getColumnName()).append(" = '").append(columnOrder)
                                .append("' WHERE ").append(OrdersColumn.TABLE_PATH.getColumnName()).append(" ~ E'^")
                                .append(protoName).append("(,|$)'; ");
                    }
                }
                ...
                // ////////////////////////////////////////////////////////////////////////////////
                // Check if there are modifications or deletions, and if any tables of this type exist
                if ((!typeModifications.isEmpty() || !typeDeletions.isEmpty()) && protoTableNames.length != 0)
                {
                    // ////////////////////////////////////////////////////////////////////////////
                    ...
                    for (String protoName : protoTableNames)
                    {
                        ...
                        valuesCmd.append(ValuesColumn.TABLE_PATH.getColumnName()).append(" ~ E'[^,]+,")
                                .append(protoName).append("\\.[^,]+,[^,]+$' OR ");
                    }

                    // Remove the trailing 'OR' and prepend an 'AND' to complete the custom values command
                    ...
                    if (isStructure && wasStructure)
                    {
                        ...
                        for (String protoName : protoTableNames)
                        {
                            // Build the table name comparison command for the links and telemetry scheduler tables
                            linksCmd.append(LinksColumn.MEMBER.getColumnName()).append(" ~ E'(?:").append(protoName)
                                    .append(",|[^,]+,").append(protoName).append("\\.[^,]+,[^,]+$)' OR ");
                            tlmSchCmd.append(TlmSchedulerColumn.MEMBER.getColumnName()).append(" ~ E'(?:")
                                    .append(tlmSchSeparator).append(protoName).append(",|[^,]+,").append(protoName)
                                    .append("\\.[^,]+,[^,]+$)' OR ");
                        }
                    ...
                    // ////////////////////////////////////////////////////////////////////////////
                    // If the table type no longer represents a structure then all references in the
                    // links and telemetry scheduler tables to tables of the affected type are removed
                    // ////////////////////////////////////////////////////////////////////////////
                    ...
                    for (String[] mod : typeModifications)
                    {
                        ...
                        if (!mod[0].equals(mod[1]))
                        {
                            ...
                            command.append("UPDATE ").append(InternalTable.VALUES.getTableName()).append(" SET ")
                                    .append(ValuesColumn.COLUMN_NAME.getColumnName()).append(" = '").append(mod[1])
                                    .append("' WHERE ").append(ValuesColumn.COLUMN_NAME.getColumnName()).append(" = '")
                                    .append(mod[0]).append("'").append(valuesCmd).append("; ");
                        }

                        // Check if the table type represents a structure and the column input type was a rate
                        if (isStructure && wasStructure && mod[2].equals(DefaultInputType.RATE.getInputName()))
                        {
                            ...
                            {
                                // Check if the original rate name is used by another structure table type
                                if (rateHandler.getRateInformationByRateName(mod[0]).getNumSharedTableTypes() > 1)
                                {
                                    // Remove all references to tables of the changed type in the
                                    // links and telemetry scheduler tables
                                ...
                                else if (rateHandler.getRateInformationByRateName(mod[1]) == null)
                                {
                                    ...
                                    // scheduler table
                                    command.append("UPDATE ").append(InternalTable.LINKS.getTableName()).append(" SET ")
                                            .append(LinksColumn.RATE_NAME.getColumnName()).append(" = '").append(mod[1])
                                            .append("' WHERE ").append(LinksColumn.RATE_NAME.getColumnName())
                                            .append(" = '").append(mod[0]).append("'; UPDATE ")
                                            .append(InternalTable.TLM_SCHEDULER.getTableName()).append(" SET ")
                                            .append(TlmSchedulerColumn.RATE_NAME.getColumnName()).append(" = '")
                                            .append(mod[1]).append("' WHERE ")
                                            .append(TlmSchedulerColumn.RATE_NAME.getColumnName()).append(" = '")
                                            .append(mod[0]).append("'; ");
                                }
                    ...
                    for (String[] del : typeDeletions)
                    {
                        ...
                        command.append("DELETE FROM ").append(InternalTable.VALUES.getTableName()).append(" WHERE ")
                                .append(ValuesColumn.COLUMN_NAME.getColumnName()).append(" = '").append(del[0])
                                .append("'").append(valuesCmd).append("; ");

                        // Check if the table type represents a structure and a rate column is deleted
                    ...
                    if (newFields != null)
                    {
                        ...
                        for (String tableName : tableNamesList)
                        {
                            ...
                            for (int currIndex = 0; currIndex < currentFields.size(); currIndex++)
                            {
                                ...
                                {
                                    // Before deleting the field it is important to check if this field is a custom
                                    // field that
                                    // was not inherited from the table type, but rather added directly to this
                                    // individual table
                                    // by a user. If no field with the same name is found in the tableTypeFields then
                                    // that means
                                    ...
                                    {
                                        if (tableTypeFields.get(index).getFieldName()
                            ...
                            for (int newIndex = 0; newIndex < newFields.length; newIndex++)
                            {
                                            ...
                                            .equals(currentFields.get(currIndex).getFieldName()))
                                    {
                                        ...
                                        switch (overwriteFields)
                                        {
                                            case ALL:
                                                ...
                                                fieldValue = newFields[newIndex][FieldEditorColumnInfo.VALUE.ordinal()]
                                                        .toString();
                                                break;
                                            ...
                                            case SAME:
                                                for (int index = 0; index < tableTypeFields.size(); index++)
                                                {
                                                    if (tableTypeFields.get(index).getFieldName()
                                                            .equals(currentFields.get(currIndex).getFieldName()))
                                                    {
                                                        fieldValue = newFields[newIndex][FieldEditorColumnInfo.VALUE
                                                                .ordinal()].toString();
                                                        break;
                                                    }
                                                }
                                        ...
                                        fieldHandler.getFieldInformation()
                                                .set(fieldHandler.getFieldInformation()
                                                        .indexOf(currentFields.get(currIndex)),
                                                     new FieldInformation(tableName,
                                                                          newFields[newIndex][FieldEditorColumnInfo.NAME
                                                                                  .ordinal()].toString(),
                                                                          newFields[newIndex][FieldEditorColumnInfo.DESCRIPTION
                                                                                  .ordinal()].toString(),
                                                                          inputTypeHandler
                                                                                  .getInputTypeByName(newFields[newIndex][FieldEditorColumnInfo.INPUT_TYPE
                                                                                          .ordinal()].toString()),
                                                                          Integer.parseInt(newFields[newIndex][FieldEditorColumnInfo.CHAR_SIZE
                                                                                  .ordinal()].toString()),
                                                                          Boolean.parseBoolean(newFields[newIndex][FieldEditorColumnInfo.REQUIRED
                                                                                  .ordinal()].toString()),
                                                                          ApplicabilityType
                                                                                  .getApplicabilityByName(newFields[newIndex][FieldEditorColumnInfo.APPLICABILITY
                                                                                          .ordinal()].toString()),
                                                                          fieldValue, true,
                                ...
                                if (fieldMatched == false)
                                {
                                    ...
                                    else if ((newFields[newIndex][FieldEditorColumnInfo.APPLICABILITY.ordinal()]
                                            .toString().equals(ApplicabilityType.CHILD_ONLY.getApplicabilityName()))
                                             && !isRootStructure(tableName))
                                    {
                                        addField = true;
                                    }
                                    else if (newFields[newIndex][FieldEditorColumnInfo.APPLICABILITY.ordinal()]
                                            .toString().equals(ApplicabilityType.ALL.getApplicabilityName()))
                                    {
                                        addField = true;
                                    }
                                    ...
                                    if (addField == true)
                                    {
                                        // Add the data field to the table and set the flag indicating a change has been
                                        // made
                                        fieldHandler.getFieldInformation()
                                                .add(new FieldInformation(tableName,
                                                                          ...
                                                                          ApplicabilityType
                                                                                  .getApplicabilityByName(newFields[newIndex][FieldEditorColumnInfo.APPLICABILITY
                                                                                          .ordinal()].toString()),
                                                                          newFields[newIndex][FieldEditorColumnInfo.VALUE
                                                                                  .ordinal()].toString(),
                                                                          true, null, -1));
                                        fieldsUpdated = true;
                                    }
            ...
            try
            {
                ...
                eventLog.logEvent(SUCCESS_MSG, new StringBuilder("Table type '").append(typeName).append("'")
                        .append(CcddUtilities.removeHTMLTags(names)).append(" updated"));
            }
            catch (
                SQLException se
            )
            {
                ...
                eventLog.logFailEvent(editorDialog,
                                      "Cannot update table type '" + typeName + "'"
                                                    + CcddUtilities.removeHTMLTags(names) + "; cause '"
                                                    + se.getMessage() + "'",
                                      "<html><b>Cannot update table type '</b>" + typeName + "<b>'" + names);
                errorFlag = true;
            }
            catch (
                Exception e
            ...
            // Check that no errors occurred and the table type represents or represented a structure
     ...
     *                   prototype table each array in the list contains the prototype name and a null.
     *                   If the deletion is due to a change in a variable's structure data type then the
     *                   array contains the immediate parent table name and the original data type
     *                   (i.e., structure table name) of the variable
     *
     * @param parent     GUI component over which to center any error dialog
     *********************************************************************************************/
    protected void closeDeletedTableEditors(List<String[]> tableNames, Component parent)
    {
        ...
        if (tableNames != null && !tableNames.isEmpty() && !ccddMain.getTableEditorDialogs().isEmpty())
        {
            String pattern;
                ...
                for (CcddTableEditorHandler editor : editorDialog.getTableEditors())
                {
                    ...
                    for (String[] name : tableNames)
                    {
                        ...
                        else
                        {
                            ...
                            // name is provided
                            pattern = "^([^,]*,)*" + Pattern.quote(name[0]) + "(\\.[^,]*,|,)" + Pattern.quote(name[1]);
    ...
    protected void updateDataTypeColumns(Component parent)
    {
        ...
        if (!ccddMain.getTableEditorDialogs().isEmpty())
        {
            ...
            CcddTableTreeHandler newTableTree = new CcddTableTreeHandler(ccddMain, TableTreeType.INSTANCE_TABLES,
                                                                         parent);
    ...
    /**********************************************************************************************
     * Modify table data field values in the data fields table. This command is executed in a separate
     * thread since it can take a noticeable amount time to complete, and by using a separate thread the
     * GUI is allowed to continue to update. The GUI menu commands, however, are disabled until the
     * database command completes execution
     *
     ...
     *********************************************************************************************/
    protected void modifyDataFieldValues(final List<String[]> modifications, final List<String[]> deletions,
            ...
            protected void execute()
            {
                try
                {
                    ...
                    for (String[] mod : modifications)
                    {
                        ...
                        command.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                                .append(FieldsColumn.FIELD_VALUE.getColumnName()).append(" = ")
                                .append(delimitText(mod[2])).append(" WHERE ")
                                .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = '").append(mod[0])
                                .append("' AND ").append(FieldsColumn.FIELD_NAME.getColumnName()).append(" = '")
                                .append(mod[1]).append("'; ");
                    }
                    ...
                    for (String[] del : deletions)
                    {
                        ...
                        command.append("DELETE FROM ").append(InternalTable.FIELDS.getTableName()).append(" WHERE ")
                                .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = '").append(del[0])
                                .append("' AND ").append(FieldsColumn.FIELD_NAME.getColumnName()).append(" = '")
                                .append(del[1]).append("'; ");
                    }
                ...
                )
                {
                    ...
                    eventLog.logFailEvent(editorWindow, "Cannot update data fields; cause '" + se.getMessage() + "'",
    ...
    /**********************************************************************************************
     * Modify all tables affected by changes to the user-defined data type names, or macro names and/or
     * macro values. This command is executed in a separate thread since it can take a noticeable amount
     * time to complete, and by using a separate thread the GUI is allowed to continue to update. The
     * GUI menu commands, however, are disabled until the database command completes execution
     *
                                                                     ...
                                                                     final CcddDialogHandler dialog)
    {
        ...
        CcddBackgroundCommand.executeInBackground(ccddMain, dialog, new BackgroundCommand()
        {
            ...
            protected void execute()
            {
                ...
                errorFlag = modifyTablesPerDataTypeOrMacroChanges(modifications, updates, dialog);
            }
    ...
    protected boolean modifyTablesPerDataTypeOrMacroChanges(final List<TableModification> modifications,
                                                            final List<String[]> updates, final Component dialog)
    {
        boolean errorFlag = false;
        ...
        {
            // Create new macro and variable size handlers using the updates from the macro
            // editor
            newDataTypeHandler = dataTypeHandler;
        ...
        /******************************************************************************************
         * Class for table information for those tables modified due to changes in a data type (macro)
         * definition
         *****************************************************************************************/
        class ModifiedTable
        {
            ...
            /**************************************************************************************
             * Class constructor for table information for those tables modified due to changes in a data type
             * (macro) definition
             *
        ...
        for (TableModification mod : modifications)
        {
            ...
            if (isDataType)
            {
                ...
                oldName = CcddDataTypeHandler
                        .getDataTypeName(mod.getOriginalRowData()[DataTypesColumn.USER_NAME.ordinal()].toString(),
                                         mod.getOriginalRowData()[DataTypesColumn.C_NAME.ordinal()].toString());
                newName = CcddDataTypeHandler
                        .getDataTypeName(mod.getRowData()[DataTypesColumn.USER_NAME.ordinal()].toString(),
            ...
            else
            {
                ...
                oldNameDelim = CcddMacroHandler
                        .getFullMacroName(mod.getOriginalRowData()[MacrosColumn.MACRO_NAME.ordinal()].toString());
                newNameDelim = CcddMacroHandler
                        .getFullMacroName(mod.getRowData()[MacrosColumn.MACRO_NAME.ordinal()].toString());
                ...
                if (nameChangeOnly)
                {
                    ...
                    nameChangeOnly = macroHandler
                            .getMacroExpansion(mod.getOriginalRowData()[MacrosColumn.VALUE.ordinal()].toString())
                            .equals(newMacroHandler
                                    .getMacroExpansion(mod.getRowData()[MacrosColumn.VALUE.ordinal()].toString()));
                }
            }
            ...
            for (String ref : references)
            {
                ...
                boolean isValueChange = (isDataType
                                         && (dataTypeHandler.getSizeInBytes(oldName) != newDataTypeHandler
                                                 .getSizeInBytes(newName)
                                             || !dataTypeHandler.getBaseDataType(oldName)
                                                     .equals(newDataTypeHandler.getBaseDataType(newName))))
                ...
                if (isNameChange || isValueChange)
                {
                    ...
                    for (int row = 0; row < tableData.size(); row++)
                    {
                        ...
                        if (isPrototype ? matchColumn.equals(tableData.get(row)[DefaultColumn.PRIMARY_KEY.ordinal()])
                                        : matchColumn.equals(tableData.get(row)[typeDefn
                                        ...
                                        if (isNameChange)
                                        {
                                            ...
                                            if (isDataType)
                                            {
                                                ...
                                                if (!oldValue.equals(oldName))
                                                {
                                                    ...
                                                    if (!CcddVariableHandler.hasSizeof(oldValue, oldName, macroHandler))
                                                    {
                                                                                         ...
                                                                                         newMacroHandler))
                                                    {
                                                        ...
                                                        newValue = newValue.replaceFirst(CcddVariableHandler
                                                                .getSizeofDataTypeMatch(oldName, macroHandler),
                                                                                         "sizeof(" + newName + ")");
                                                    }
                                                }
                                                ...
                                                {
                                                    // Set the new cell value to the new data type name
                                                    newValue = newName;
                                                }
                                            }
                                            // This is a macro change
        ...
        try
        {
            ...
            if (nameChangeOnly)
            {
                ...
                for (TableModification mod : modifications)
                {
                    ...
                    if (isDataType)
                    {
                        ...
                        String oldName = CcddDataTypeHandler
                                .getDataTypeName(mod.getOriginalRowData()[DataTypesColumn.USER_NAME
                                        .ordinal()].toString(),
                                                 mod.getOriginalRowData()[DataTypesColumn.C_NAME.ordinal()].toString());
                        String newName = CcddDataTypeHandler
                                .getDataTypeName(mod.getRowData()[DataTypesColumn.USER_NAME.ordinal()].toString(),
                                                 mod.getRowData()[DataTypesColumn.C_NAME.ordinal()].toString());
                        ...
                        String replaceCommand = ", E'(.*,)" + oldName + "(\\..*)'" + ", E'\\\\1" + newName
                                                + "\\\\2'); ";
                        ...
                        {
                            sqlCommand.append("UPDATE ").append(v.left);
                            sqlCommand.append(" SET ").append(v.middle);
                            sqlCommand.append(" = regexp_replace(").append(v.right).append(replaceCommand);
                            sqlCommand.append(" ");
                        }
                    ...
                    else
                    {
                        ...
                        String oldName = CcddMacroHandler
                                .getFullMacroName(mod.getOriginalRowData()[MacrosColumn.MACRO_NAME.ordinal()]
                                        .toString())
                                .replaceAll("([\\(\\)])", "\\\\\\\\$1");
                        String newName = CcddMacroHandler
                                .getFullMacroName(mod.getRowData()[MacrosColumn.MACRO_NAME.ordinal()].toString());

                        // Execute the command to update the internal tables that reference
                        // variable and table paths
                        StringBuilder command = new StringBuilder();
                        command.append("UPDATE ").append(InternalTable.ASSOCIATIONS.getTableName()).append(" SET ")
                                .append(AssociationsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                                .append(AssociationsColumn.MEMBERS.getColumnName()).append(", E'(.*)").append(oldName)
                                .append("(.*)'").append(", E'\\\\1").append(newName).append("\\\\2'); UPDATE ")
                                .append(InternalTable.FIELDS.getTableName()).append(" SET ")
                                .append(FieldsColumn.OWNER_NAME.getColumnName()).append(" = regexp_replace(")
                                .append(FieldsColumn.OWNER_NAME.getColumnName()).append(", E'(.*)").append(oldName)
                                .append("(.*)'").append(", E'\\\\1").append(newName).append("\\\\2'); UPDATE ")
                                .append(InternalTable.GROUPS.getTableName()).append(" SET ")
                                .append(GroupsColumn.MEMBERS.getColumnName()).append(" = regexp_replace(")
                                .append(GroupsColumn.MEMBERS.getColumnName()).append(", E'(.*)").append(oldName)
                                .append("(.*)'").append(", E'\\\\1").append(newName).append("\\\\2'); UPDATE ")
                                .append(InternalTable.LINKS.getTableName()).append(" SET ")
                                .append(LinksColumn.MEMBER.getColumnName()).append(" = regexp_replace(")
                                .append(LinksColumn.MEMBER.getColumnName()).append(", E'(.*)").append(oldName)
                                .append("(.*)'").append(", E'\\\\1").append(newName).append("\\\\2'); UPDATE ")
                                .append(InternalTable.TLM_SCHEDULER.getTableName()).append(" SET ")
                                .append(TlmSchedulerColumn.MEMBER.getColumnName()).append(" = regexp_replace(")
                                .append(TlmSchedulerColumn.MEMBER.getColumnName()).append(", E'(.*)").append(oldName)
                                .append("(.*)'").append(", E'\\\\1").append(newName).append("\\\\2'); UPDATE ")
                                .append(InternalTable.VALUES.getTableName()).append(" SET ")
                                .append(ValuesColumn.TABLE_PATH.getColumnName()).append(" = regexp_replace(")
                                .append(ValuesColumn.TABLE_PATH.getColumnName()).append(", E'(.*)").append(oldName)
                                .append("(.*)'").append(", E'\\\\1").append(newName).append("\\\\2'); ");
            ...
            for (ModifiedTable modTbl : modifiedTables)
            {
                ...
                if (modifyTableData(modTbl.getTableInformation(), modTbl.getEditor().getAdditions(),
                                    modTbl.getEditor().getModifications(), modTbl.getEditor().getDeletions(), true,
                                    nameChangeOnly, false, false, false, newDataTypeHandler, true, true, false, dialog))
                {
            ...
            if (isDataType)
            {
                ...
                dbCommand
                        .executeDbUpdate(new StringBuilder(storeNonTableTypesInfoTableCommand(InternalTable.DATA_TYPES,
                                                                                              CcddUtilities
                                                                                                      .removeArrayListColumn(updates,
                                                                                                                             DataTypesColumn.OID
                                                                                                                                     .ordinal()),
                                                                                              null, dialog)),
                                         dialog);
                dbCommand
                        .executeDbUpdate(new StringBuilder(storeNonTableTypesInfoTableCommand(InternalTable.MACROS,
                                                                                              CcddUtilities
                                                                                                      .removeArrayListColumn(newMacroHandler
                                                                                                              .getMacroData(),
                                                                                                                             MacrosColumn.OID
                                                                                                                                     .ordinal()),
                                                                                              null, dialog)),
            ...
            eventLog.logEvent(SUCCESS_MSG, new StringBuilder(changeName).append(" and all affected tables updated"));
                ...
                CCDDException | SQLException cse
        )
        {
            // Inform the user that updating the data types (macros) failed
            eventLog.logFailEvent(dialog,
                                  "Cannot update " + changeName.toLowerCase() + "; cause '" + cse.getMessage() + "'",
        ...
        if (errorFlag)
        {
            ...
            )
            {
                ...
                eventLog.logFailEvent(dialog, "Cannot revert changes to project; cause '" + se.getMessage() + "'",
    ...
    /**********************************************************************************************
     * Modify all tables affected by changes to the user-defined input types. This command is executed
     ...
     *
     * @param updates       array of string arrays reflecting the content of the input types after being
    ...
    protected void modifyTablesPerInputTypeChanges(final List<TableModification> modifications,
                                                   final String[][] updates, final CcddInputTypeEditorDialog dialog)
    {
        ...
        final CcddTableTypeHandler newTableTypeHandler = new CcddTableTypeHandler(ccddMain, newInputTypeHandler);

        /******************************************************************************************
         * Class for table information for those tables modified due to changes in an input type definition
         *****************************************************************************************/
        class ModifiedTable
        {
            private final TableInfo tableInformation;
            ...
            /**************************************************************************************
             * Class constructor for table information for those tables modified due to changes in an input type
             * definition
             *
             * @param tablePath table path (if applicable) and name
             *************************************************************************************/
            ModifiedTable(String tablePath)
            {
                ...
                editor = new CcddTableEditorHandler(ccddMain, tableInformation, newTableTypeHandler,
                                                    newInputTypeHandler, dialog);
            }
        ...
        CcddBackgroundCommand.executeInBackground(ccddMain, dialog, new BackgroundCommand()
        {
            ...
            protected void execute()
            {
                ...
                try
                {
                    ...
                    for (TableModification mod : modifications)
                    {
                        ...
                        if (!oldName.equals(newName))
                        {
                            ...
                            tableTypeCommand.append("UPDATE ").append(InternalTable.TABLE_TYPES.getTableName())
                                    .append(" SET ").append(TableTypesColumn.INPUT_TYPE.getColumnName()).append(" = ")
                                    .append(delimitText(newName)).append(" WHERE ")
                                    .append(TableTypesColumn.INPUT_TYPE.getColumnName()).append(" = ")
                                    .append(delimitText(oldName)).append("; ");

                            fieldCommand.append("UPDATE ").append(InternalTable.FIELDS.getTableName()).append(" SET ")
                                    .append(FieldsColumn.FIELD_TYPE.getColumnName()).append(" = ")
                                    .append(delimitText(newName)).append(" WHERE ")
                                    .append(FieldsColumn.FIELD_TYPE.getColumnName()).append(" = ")
                                    .append(delimitText(oldName)).append("; ");
                                    ...
                                    .equals(mod.getRowData()[InputTypesColumn.ITEMS.ordinal()]))
                        {
                            ...
                            for (String inputTypeRef : inputTypeHandler.searchInputTypeReferences(oldName, dialog))
                            {
                                ...
                                String[] refColumns = CcddUtilities
                                        .splitAndRemoveQuotes(tblColCmtAndCntxt[SearchResultsQueryColumn.CONTEXT
                                                .ordinal()]);
                                ...
                                {
                                    // Check if the table type name hasn't already been added to the list
                                    if (!tableTypes.contains(refColumns[TableTypesColumn.TYPE_NAME.ordinal()]))
                                    {
                                        // Add the table type name to the list of those using the input type
                                ...
                                {
                                    // Add the data field definition to the list of those using the input type
                            ...
                            for (String tableType : tableTypes)
                            {
                                ...
                                if (tables.length != 0)
                                {
                                    ...
                                    for (String tableName : tables)
                                    {
                                        ...
                                        for (int row = 0; row < tableData.size(); row++)
                                        {
                                            // Step through each column with the modified input type
                                            for (Integer column : columns)
                                            {
                                                // Make the change to the cell
                                                showMessage = table.validateCellContent(tableData, row, column, "",
                            ...
                            {
                                // Check if the field's value no longer matches the regular expression
                                        ...
                                        .matches(mod.getRowData()[InputTypesColumn.MATCH.ordinal()].toString()))
                                {
                                    ...
                                    fieldCommand.append("UPDATE ").append(InternalTable.FIELDS.getTableName())
                                            .append(" SET ").append(FieldsColumn.FIELD_VALUE.getColumnName())
                                            .append(" = '' WHERE ").append(FieldsColumn.OWNER_NAME.getColumnName())
                                            .append(" = ").append(delimitText(field[FieldsColumn.OWNER_NAME.ordinal()]))
                                            .append(" AND ").append(FieldsColumn.FIELD_NAME.getColumnName())
                                            .append(" = ").append(delimitText(field[FieldsColumn.FIELD_NAME.ordinal()]))
                                            .append("; ");
                                }
                            }
                        }
                    ...
                    for (ModifiedTable modTbl : modifiedTables)
                    {
                        ...
                        if (modifyTableData(modTbl.getTableInformation(), modTbl.getEditor().getAdditions(),
                                            modTbl.getEditor().getModifications(), modTbl.getEditor().getDeletions(),
                                            true, false, false, false, false, null, true, true, false, dialog))
                        {
                    ...
                    dbCommand
                            .executeDbUpdate(new StringBuilder(storeNonTableTypesInfoTableCommand(InternalTable.INPUT_TYPES,
                                                                                                  CcddUtilities
                                                                                                          .removeArrayListColumn(Arrays
                                                                                                                  .asList(dialog
                                                                                                                          .getUpdatedData()),
                                                                                                                                 InputTypesColumn.OID
                                                                                                                                         .ordinal()),
                                                                                                  null, dialog)),
                                             dialog);
                ...
                )
                {
                    ...
                    eventLog.logFailEvent(dialog, "Cannot update input types; cause '" + se.getMessage() + "'",
    ...
    /**********************************************************************************************
     * Get an array containing all prototype tables that are of the specified table type (combining all
     * structure and all command tables)
     *
     ...
     *                  TYPE_OTHER to get all tables that are neither a structure or command table, or
     *                  the table type name to get all tables for the specified type
     *
     * @return Array containing all prototype tables that represent the specified type. Returns an empty
     *         array if no table of the specified type exists in the project database
     *********************************************************************************************/
        ...
        for (String type : tableTypeHandler.getTableTypeNames())
        {
            ...
            if (isOfType)
            {
                ...
                tablesOfType = CcddUtilities.concatenateArrays(tablesOfType,
                                                               queryTablesOfTypeList(type, ccddMain.getMainFrame()));
            }
        }
     ...
     *
     * @param protoTableNames names of the prototype tables of the specified table type; null to load
     *                        the list
     *
     * @param parent          GUI component over which to center any error dialog
     *
     * @return List containing all tables that are of the specified table type. Returns an empty list if
     *         no table of the specified type exists in the project database
     *********************************************************************************************/
    protected List<String> getAllTablesOfType(String typeName, String[] protoTableNames, Component parent)
    {
    ...
    /**********************************************************************************************
     * Update the table tree information including the pre-loaded information
     *********************************************************************************************/
    protected void updateTableTree()
    {
        try
        {
            // Build the table tree handler
            ccddMain.buildTableTreeHandler();

            if (ccddMain.getTableTreeHandler() != null)
            {
                // Update the pre-loaded table members
                updatePreLoadedTableMembers();

                String updating = defaultTableMembersUpdating();

                // Wait till the table members finish updating
                while (updating.contentEquals("Updating"))
                {
                    TimeUnit.SECONDS.sleep(1);
                    updating = defaultTableMembersUpdating();
                }
            }
        }
        catch (
