...
public class CcddXTCEHandler extends CcddImportSupportHandler implements CcddImportExportInterface {
    ...
    CcddXTCEHandler(CcddMain ccddMain, ScriptEngine scriptEngine, Component parent) throws CCDDException {
        ...
        try {
            // Create the XML marshaller used to convert the CCDD project data into XTCE XML
            // format
            ...
            // Create the XML unmarshaller used to convert XTCE XML data into CCDD project
            // data
            // format
     ...
     *
     * @param replaceExistingMacros not used for XTCE import
     *
     * @param replaceExistingGroups not used for XTCE import
     *
     * @param ignoreErrors          true to ignore all errors in the import file
     *
     ...
     *
     * @param rootSystem     root space system
     *
     * @param importFile     reference to the user-specified XML input file
     *
     * @param importType     ImportType.IMPORT_ALL to import the table type, data
     ...
     *********************************************************************************************/
    private void createTableTypeDefinitions(SpaceSystemType rootSystem, FileEnvVar importFile, ImportType importType,
        ...
        if (targetIsCommand) {
            ...
            if (importType == ImportType.IMPORT_ALL || targetIsStructure) {
                ...
                TableTypeDefinition tableTypeDefn = new TableTypeDefinition(typeName, "XTCE import command table type");
    ...
    private void unbuildSpaceSystems(SpaceSystemType system, String systemPath, ImportType importType,
        ...
        // The full table name, with path, should be stored in the space system's short
        // description
        // (the space system name doesn't allow the commas and periods used by the table
        // path so it
        // has to go elsewhere; the export operation does this). If the short
        // description doesn't
        // exist, or isn't in the correct format, then the table name is extracted from
        // the space
        // system name; however, this creates a 'flat' table reference, making it a
        // prototype
        ...
        // Check if the telemetry metadata information is present and a structure table
        // type
        // definition exists to define it (the structure table type won't exists if
        // importing into
        // a single command table). If the telemetry metadata is present the assumption
        // is made
    ...
    private void importStructureTable(SpaceSystemType system, TelemetryMetaDataType tlmMetaData, String tableName,
        ...
        if (tlmMetaData.getContainerSet() != null) {
            ...
            // Get the system under which the space systems in the container references are
            // to be
            // found. Specific instance tables are a sub-space system of the parent tables'
            // space
            // system, but If the table is a child of a non-root structure then the space
            // system
            ...
            for (SequenceContainerType seqContainer : tlmMetaData.getContainerSet().getSequenceContainer()) {
                ...
                if (baseContainer != null && baseContainer.getContainerRef() != null
                        && TableInformation.getPrototypeName(baseContainer.getContainerRef())
    ...
    private void importCommandTable(SpaceSystemType system, CommandMetaDataType cmdMetaData, String tableName,
        ...
        if (metaCmdSet != null) {
            ...
            // includes a
            // telemetry metadata (which creates a structure table) then ensure the two
            // tables have
            // different names
                ...
                if (cmd instanceof MetaCommandType) {
                    ...
                    if (!metaCmd.isAbstract()) {
                        ...
                        if (!onlyCmdToStruct) {
                            ...
                            cmdTableDefn.addData(Arrays.copyOf(cmdRowData, numCommandColumns));
                        }
                    }
                ...
                if (matchParmType.equals(parmType.getName())) {
                    ...
                    // Check if the parameter is a string data type
                        ...
                        dataTypeBitSize = Integer
                                .valueOf(stlm.getStringDataEncoding().getSizeInBits().getFixed().getFixedValue());
            ...
            if (argument.getArgumentTypeRef().equals(argType.getName())) {
                ...
                if (argType != null) {
                    ...
                    // Check if the argument is a string data type
                        ...
                        dataTypeBitSize = Integer
                                .valueOf(scmd.getStringDataEncoding().getSizeInBits().getFixed().getFixedValue());
    ...
    private ParameterInformation processContainerReference(SpaceSystemType system,
        ...
        if (childSystem != null && childSystem.getShortDescription() != null
            ...
            String[] typeAndName = TableInformation.getProtoVariableName(childSystem.getShortDescription())
                    .split("\\.");
        ...
        if (useInternal) {
            ...
            if (arraySize != null && !arraySize.isEmpty()) {
                ...
                spaceSystem.getTelemetryMetaData().getParameterTypeSet()
                        .getStringParameterTypeOrEnumeratedParameterTypeOrIntegerParameterType().add(arrayType);
            }
            ...
            // Set the parameter's data type information
            spaceSystem.getTelemetryMetaData().getParameterTypeSet()
                    .getStringParameterTypeOrEnumeratedParameterTypeOrIntegerParameterType().add(parameterType);
        }
    }
            ...
            if (argumentNames.length != 0) {
                ...
                for (String argumentName : argumentNames) {
                    ...
                    if (dataTypeHandler.isPrimitive(argDataType)) {
                        ...
                        if (isArray) {
                            ...
                            arrayRef.setDimensionList(dimList);
                            JAXBElement<ArrayParameterRefEntryType> arrayRefElem = factory
                                    .createCommandContainerEntryListTypeArrayArgumentRefEntry(arrayRef);
                        ...
                        // Not an array
                            ...
                            JAXBElement<ArgumentRefEntry> argumentRefElem = factory
                                    .createCommandContainerEntryListTypeArgumentRefEntry(argumentRef);
                            entryList.getParameterRefEntryOrParameterSegmentRefEntryOrContainerRefEntry()
                                    .add(argumentRefElem);
                        }
                    }
                ...
                // table name are provided
                    ...
                    ArgumentAssignmentList argAssnList = factory
                            .createMetaCommandTypeBaseMetaCommandArgumentAssignmentList();
                    ArgumentAssignment argAssn = factory
    ...
    protected NameDescriptionType setArgumentDataType(SpaceSystemType spaceSystem, String argumentName, String dataType,
        ...
        if (useInternal) {
            ...
            if (arraySize != null && !arraySize.isEmpty()) {
                ...
                spaceSystem.getCommandMetaData().getArgumentTypeSet()
                        .getStringArgumentTypeOrEnumeratedArgumentTypeOrIntegerArgumentType().add(arrayType);
            }
                ...
                do {
                    ...
                    for (currentIndices[0] = 0; currentIndices[0] < totalDims[totalDims.length
                        ...
                        // Check if this is a command list container
                            ...
                            ((CommandContainerEntryListType) entryList)
                                    .getParameterRefEntryOrParameterSegmentRefEntryOrContainerRefEntry()
                                    .add(containerRefElem);
                        }
                    }
            ...
            // Not an array parameter
                ...
                // Check if this is a command list container
                    ...
                    JAXBElement<ContainerRefEntryType> containerRefElem = factory
                            .createCommandContainerEntryListTypeContainerRefEntry(containerRefEntry);
                    ((CommandContainerEntryListType) entryList)
                            .getParameterRefEntryOrParameterSegmentRefEntryOrContainerRefEntry().add(containerRefElem);
                }
            }
        }
    }
