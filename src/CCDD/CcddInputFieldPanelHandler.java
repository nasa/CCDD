...
/**************************************************************************************************
 * CFS Command and Data Dictionary description and data field panel handler
...
public abstract class CcddInputFieldPanelHandler {
    ...
    /**********************************************************************************************
     * Get the JPanel containing the table (if applicable), description, and data
     * fields
     *
     * @return JPanel containing the table (if applicable), description, and data
     *         fields
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Set the references to the description and data field undo/redo manager and
     * handler
     *
     ...
     *
     * @param ownerName name of the owner of this description and data field panel
     *                  handler
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Check if the description field is the current focus owner (i.e., editing is
     * active for the field)
     *
    ...
    /**********************************************************************************************
     * Enable/disable the description field, set its background color based on the
     * enable status, and set the description text
     *
    ...
    /**********************************************************************************************
     * Update the description field text. Put the edit on the undo/redo stack based
     * on the input flag
     *
    ...
    /**********************************************************************************************
     * Get the width of the widest data field (including its label), plus padding on
     * either side
     *
     * @return Width of the widest data field (including its label), plus padding on
     *         either side
     *********************************************************************************************/
     ...
     *
     * @param fieldPnlHndlrOwner reference to the owner of this description and data
     *                           field handler
     *
     * @param upperPane          component to display above the description and data
     *                           fields scroll pane; null if this field panel
     *                           handler does not contain a table
     *
     * @param ownerName          name of the owner of this field panel handler; null
     *                           if no owner name is associated with it
     *
     * @param description        description field text; null if the description is
     *                           initially blank and disabled
     *
     * @param ownerFieldInfo     list of field information to use to build the data
     *                           fields; null if no data field is associated with
     *                           owner
     *********************************************************************************************/
    protected void createDescAndDataFieldPanel(CcddMain ccddMain, final Component fieldPnlHndlrOwner,
            final JComponent upperPane, String ownerName, String description, List<FieldInformation> ownerFieldInfo) {
        ...
        // The editor contains an upper pane
            ...
            // Place the description field within a scroll pane and add the field to the
            // editor
            descriptionFld.setToolTipText(
                    CcddUtilities.wrapText("Table description", ModifiableSizeInfo.MAX_TOOL_TIP_LENGTH.getSize()));
        ...
        inputScrollPane.setMinimumSize(
                new Dimension(inputScrollPane.getMinimumSize().width, descriptionPnl.getPreferredSize().height
                ...
                SwingUtilities.invokeLater(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (fieldPnl != null) {
                            ...
                            fieldPnl.setSize(new Dimension(
                                    inputScrollPane.getWidth()
                                            - (inputScrollPane.getVerticalScrollBar().isShowing() ? LAF_SCROLL_BAR_WIDTH
                                                    : 0)
     ...
     *
     * @param undoable       true if the change(s) to the data fields should be
     *                       stored for possible undo/redo operations; false to not
     *                       store the changes
     *
     * @param ownerFieldInfo list of field information to use to build the data
    ...
    protected void createDataFieldPanel(boolean undoable, List<FieldInformation> ownerFieldInfo) {
        ...
        if (!fieldInformation.isEmpty()) {
            ...
            fieldPnl.setBorder(
                    BorderFactory.createEmptyBorder(-ModifiableSpacingInfo.LABEL_VERTICAL_SPACING.getSpacing(),
                    ...
                    default:
                        ...
                        if (fieldInfo.getInputType()
                            ...
                            UndoableTextArea undoableTxtArea = undoHandler.new UndoableTextArea(fieldInfo.getValue(),
                                    ModifiableSizeInfo.MAX_DATA_FIELD_ROWS.getSize(), fieldInfo.getSize());
                        ...
                        inputFld.setBackground(fieldInfo.getValue().isEmpty() && fieldInfo.isRequired()
                        ...
                        if (inputFld instanceof JTextComponent) {
                            ...
                            inputFld.setInputVerifier(new InputVerifier() {
                                ...
                                public boolean verify(JComponent input) {
                                    ...
                                    if (!fieldInfo.getInputType().equals(
                                            inputTypeHandler.getInputTypeByDefaultType(DefaultInputType.TEXT_WHT_SPC))
                                            && !fieldInfo.getInputType().equals(inputTypeHandler
                                                    .getInputTypeByDefaultType(DefaultInputType.TEXT_MULTI_WHT_SPC))) {
    ...
    /**********************************************************************************************
     * Set the data field background color for all fields based each field's value
     * and required flag
     *********************************************************************************************/
    protected void setFieldBackgound() {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            /**************************************************************************************
             * Set the data field colors after other pending events are complete. If this
             * isn't done following other events then the colors aren't updated consistently
             *************************************************************************************/
            @Override
            public void run() {
    ...
    /**********************************************************************************************
     * Set the specified data field's background color based the field's value and
     * required flag
     *
    ...
    private void setFieldBackground(FieldInformation fieldInfo) {
        ...
        // required
        // then set the background to indicate a value should be supplied
        ((JComponent) fieldInfo.getInputFld()).setBackground(
                fieldInfo.getValue().isEmpty() && fieldInfo.isRequired() ? ModifiableColorInfo.REQUIRED_BACK.getColor()
                        : ModifiableColorInfo.INPUT_BACK.getColor());
    }
}
