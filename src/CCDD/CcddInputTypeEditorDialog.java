...
public class CcddInputTypeEditorDialog extends CcddDialogHandler {
    ...
    /**********************************************************************************************
     * Perform the steps needed following execution of input type updates to the
     * database
     *
    ...
    /**********************************************************************************************
     * Create the input type editor dialog. This is executed in a separate thread
            ...
            protected void execute() {
                ...
                btnDeleteRow.addActionListener(new ValidateCellActionListener(inputTypeTable) {
                    ...
                    protected void performAction(ActionEvent ae) {
                        ...
                        for (int row : inputTypeTable.getSelectedRows()) {
                            ...
                            String inputTypeName = inputTypeTable.getValueAt(row, InputTypesColumn.NAME.ordinal())
                                    .toString();
                            ...
                            if (!inputTypeName.isEmpty()
                                ...
                                if (!tableTypes.isEmpty()) {
                                    ...
                                    inputTypeUsers = "table type(s) '</b>" + CcddUtilities
                                            .convertArrayToStringTruncate(tableTypes.toArray(new String[0])) + "<b>'";
                                }
                                ...
                                new CcddDialogHandler().showMessageDialog(CcddInputTypeEditorDialog.this,
                                        "<html><b>Cannot delete input type '</b>" + inputTypeName
                                                + "<b>'; input type is referenced by " + inputTypeUsers,
                                        "Delete Input Type", JOptionPane.ERROR_MESSAGE, DialogOption.OK_OPTION);
                            }
                        }
                ...
                btnStore.addActionListener(new ValidateCellActionListener(inputTypeTable) {
                    ...
                    protected void performAction(ActionEvent ae) {
                        ...
                        // the action
                        if (inputTypeTable.isTableChanged(committedData) && !checkForMissingColumns()
                                && new CcddDialogHandler().showMessageDialog(CcddInputTypeEditorDialog.this,
                            ...
                            dbTable.modifyTablesPerInputTypeChanges(modifications, getUpdatedData(),
    ...
    private JScrollPane createInputTypeTable() {
        ...
        inputTypeTable = new CcddJTableHandler(DefaultPrimitiveTypeInfo.values().length) {
            ...
            public boolean isCellEditable(int row, int column) {
                ...
                if (inputTypeTable.getModel() != null && inputTypeTable.getModel().getRowCount() != 0) {
                    ...
                    isEditable = isDataAlterable(((List<?>) ((UndoableTableModel) inputTypeTable.getModel())
                            .getDataVector().elementAt(modelRow)).toArray(new Object[0]), modelRow, modelColumn);
                }
                    ...
                    isAlterable = // ... the regular expression match column if the selection items
                            ...
                            (column != InputTypeEditorColumnInfo.MATCH.ordinal()
                                    || (rowData[InputTypeEditorColumnInfo.ITEMS.ordinal()].toString().isEmpty()
                                            && !InputType.getInputFormatByName(
                                                    rowData[InputTypeEditorColumnInfo.FORMAT.ordinal()].toString())
                                                    .equals(InputTypeFormat.BOOLEAN)));
                }
            ...
            /**************************************************************************************
             * Override getCellEditor so that for a input type format column cell the input
             * type format combo box cell editor is returned; for all other cells return the
             * normal cell editor
             *
                    ...
                    if (!newValueS.isEmpty()) {
                        ...
                        if (column == InputTypeEditorColumnInfo.NAME.ordinal()) {
                            ...
                            for (int otherRow = 0; otherRow < getRowCount(); otherRow++) {
                                ...
                                if (otherRow != row && newValueS.equalsIgnoreCase(
                                        tableData.get(otherRow)[InputTypeEditorColumnInfo.NAME.ordinal()].toString())) {
                        ...
                        // Check if the regular expression match string has been changed
                            ...
                            try {
                                ...
                                throw new CCDDException(
                                        "Invalid regular expression; cause '</b>" + pse.getMessage() + "<b>'");
                            }
                        }
                        // Check if the regular expression match string has been changed
                        ...
                        // value
                                ...
                                && InputType.getInputFormatByName(tableData.get(row)[column].toString())
                                        .equals(InputTypeFormat.BOOLEAN)) {
                            // Set the regular expression to that for a boolean value
                            tableData.get(row)[InputTypeEditorColumnInfo.MATCH.ordinal()] = DefaultInputType.BOOLEAN
                                    .getInputMatch();
                        }
                    }
            ...
            protected void loadAndFormatData() {
                ...
                setUpdatableCharacteristics(committedData, InputTypeEditorColumnInfo.getColumnNames(), null,
                        InputTypeEditorColumnInfo.getToolTips(), true, true, true);
            }
            ...
            public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
                ...
                if (!(isFocusOwner() && isRowSelected(row)
                    ...
                    // The cell value can be changed
                        ...
                        if ((modelColumn == InputTypeEditorColumnInfo.NAME.ordinal()) && inputTypeTable
                                .getValueAt(row, InputTypeEditorColumnInfo.NAME.ordinal()).toString().isEmpty()) {
        ...
        inputTypeTable.setFixedCharacteristics(scrollPane, true, ListSelectionModel.MULTIPLE_INTERVAL_SELECTION,
    ...
    protected void windowCloseButtonAction() {
        ...
        // discarding the
        // changes
        if (inputTypeTable.isLastCellValid() && (!inputTypeTable.isTableChanged(committedData)
                || new CcddDialogHandler().showMessageDialog(CcddInputTypeEditorDialog.this,
            ...
            if (row < inputTypeTable.getRowCount()) {
                ...
                for (int column = 0; column < inputTypeTable.getColumnCount() && !stopCheck; column++) {
                    ...
                    if (InputTypeEditorColumnInfo.values()[column].isRequired()
                        ...
                        if (new CcddDialogHandler().showMessageDialog(CcddInputTypeEditorDialog.this,
                                "<html><b>Data must be provided for column '</b>" + inputTypeTable.getColumnName(column)
                                        + "<b>' [row </b>" + (row + 1) + "<b>]",
                                "Missing Data", JOptionPane.WARNING_MESSAGE,
                                DialogOption.OK_CANCEL_OPTION) == CANCEL_BUTTON) {
                            // Set the stop flag to prevent further error checking
    ...
    /**********************************************************************************************
     * Compare the current input type table data to the committed data and create
     * lists of the changed values necessary to update the table in the database to
        ...
        for (int tblRow = 0; tblRow < tableData.length; tblRow++) {
            ...
            if (!tableData[tblRow][InputTypesColumn.OID.ordinal()].toString().isEmpty()) {
                ...
                for (int comRow = 0; comRow < numCommitted && !matchFound; comRow++) {
                    ...
                    if (tableData[tblRow][InputTypesColumn.OID.ordinal()]
                            .equals(committedData[comRow][InputTypesColumn.OID.ordinal()])) {
