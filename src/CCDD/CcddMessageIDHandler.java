...
public class CcddMessageIDHandler {
     ...
     *               {@literal <}ID{@literal >}. A null, blank input, or an input
     *               containing only the name or ID is accepted
     *
     * @return Array containing the message name and ID. The name, ID,or both the
     *         name and ID can be blank
     *********************************************************************************************/
     ...
     *
     * @param nameID message name and ID in the format {@literal <}name{@literal >}
     *               {@literal <}ID{@literal >}. A blank input, or an input
     *               containing only the ID is accepted
     *
     ...
     *
     * @param includeStructures    true to include message IDs assigned to tables
     *                             that represent structures
     *
     * @param includeCommands      true to include message IDs assigned to tables
     *                             that represent commands
     *
     * @param includeOthers        true to include message IDs assigned to tables
     *                             that do not represent structures or commands
     *
     * @param includeGroups        true to include message IDs assigned to groups
     *
     * @param useTlmMsgIDsFromDb   true to include message IDs assigned to telemetry
     *                             messages stored in the project database; false to
     *                             use the IDs from the currently open telemetry
     *                             scheduler
     *
     * @param isOverwriteTlmMsgIDs true to allow overwriting the telemetry message
     *                             IDs for the currently selected data stream in the
     *                             open telemetry scheduler; false to not allow
     *                             overwriting. This value is only used if
     *                             useTlmMsgIDsFromDb is false
     *
     * @param tlmSchedulerDlg      Reference to the currently open telemetry
     *                             scheduler. This value is only used if
     *                             useTlmMsgIDsFromDb is false, in which case it can
     *                             be set to null
     *
     * @param isGetDuplicates      true to create a list of duplicate IDs. The flags
     *                             for including tables and for using the telemetry
     *                             message IDs from the database should be set to
     *                             true when getting the list of duplicates
     *
     ...
     *********************************************************************************************/
    protected List<Integer> getMessageIDsInUse(boolean includeStructures, boolean includeCommands,
            boolean includeOthers, boolean includeGroups, boolean useTlmMsgIDsFromDb, boolean isOverwriteTlmMsgIDs,
    ...
    /**********************************************************************************************
     * Get the message ID, minus the auto-assign protection flag (if present), from
     * the supplied message ID
     *
    ...
    /**********************************************************************************************
     * Get the list containing every message owner, name, and ID from every table
     ...
     * field
     *
     * @param sortOrder          order in which to sort the message ID list:
     *                           BY_OWNER or BY_NAME (MessageIDSortOrder)
     *
     * @param hideProtectionFlag true to not display the flag character that
     *                           protects a message ID from being changed by the
     *                           auto-update methods; false to allow the flag to
     *                           remain
     *
     * @param parent             GUI component over which to center any error dialog
     *
     * @return List containing every message name and its corresponding message ID,
     *         and the owning entity
     *********************************************************************************************/
    protected List<String[]> getMessageOwnersNamesAndIDs(MessageIDSortOrder sortOrder, boolean hideProtectionFlag,
            Component parent) {
        ...
        case BY_OWNER:
            ...
            ownersNamesAndIDs.setComparisonColumn(MsgIDListColumnIndex.OWNER.ordinal(),
                    MsgIDListColumnIndex.MESSAGE_ID.ordinal(), MsgIDListColumnIndex.MESSAGE_NAME.ordinal());
        ...
        case BY_NAME:
            ...
            ownersNamesAndIDs.setComparisonColumn(MsgIDListColumnIndex.MESSAGE_NAME.ordinal(),
                    MsgIDListColumnIndex.MESSAGE_ID.ordinal(), MsgIDListColumnIndex.OWNER.ordinal());
            ownersNamesAndIDs.sort(ArrayListMultipleSortType.STRING);
            break;
        }
    ...
    /**********************************************************************************************
     * Update the list of message IDs in use and, based on the input flag, update
     * the duplicate IDs list
     *
     * @param ownerType       message ID owner type (Table or Message)
     *
     * @param ownerAndID      array where the first member is the owner (table name
     *                        or telemetry message name) and the second element is
     *                        the message ID
     *
