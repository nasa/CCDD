...
public class CcddReservedMsgIDEditorDialog extends CcddDialogHandler {
    ...
    /**********************************************************************************************
     * Perform the steps needed following execution of reserved message ID updates
     * to the database
     *
    ...
    /**********************************************************************************************
     * Copy the reserved message ID data so it can be used to determine if changes
     * are made
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Create the reserved message ID editor dialog. This is executed in a separate
                    ...
                    protected void performAction(ActionEvent ae) {
                        ...
                        if (msgIDTable.isTableChanged(committedData) && !checkForMissingColumns()
                            ...
                            dbTable.storeInformationTableInBackground(InternalTable.RESERVED_MSG_IDS,
                                    ...
                                    null, CcddReservedMsgIDEditorDialog.this);
                        }
                    }
                });
    ...
    private JScrollPane createReservedMsgIDTable() {
        ...
        msgIDTable = new CcddJTableHandler() {
                        ...
                        if (column == ReservedMsgIDEditorColumnInfo.MSG_ID.ordinal()) {
                            ...
                            if (lowHigh[1] != -1 && lowHigh[0] > lowHigh[1]) {
                                ...
                                throw new CCDDException(
                                        "Invalid message ID range; lower value must be &lt;= upper value");
                            }
            ...
            /**************************************************************************************
             * Load the table reserved message ID definition values into the table and
            ...
            public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
                ...
                if (!(isFocusOwner() && isRowSelected(row)
                    ...
                    if (ReservedMsgIDEditorColumnInfo.values()[msgIDTable.convertColumnIndexToModel(column)]
                            .isRequired() && msgIDTable.getValueAt(row, column).toString().isEmpty()) {
    ...
    protected void windowCloseButtonAction() {
        ...
        // discarding the
        // changes
        if (msgIDTable.isLastCellValid() && (!msgIDTable.isTableChanged(committedData)
                || new CcddDialogHandler().showMessageDialog(CcddReservedMsgIDEditorDialog.this,
                ...
                for (int column = 0; column < msgIDTable.getColumnCount() && !stopCheck; column++) {
                    ...
                    if (ReservedMsgIDEditorColumnInfo.values()[column].isRequired()
                        ...
                        if (new CcddDialogHandler().showMessageDialog(CcddReservedMsgIDEditorDialog.this,
                                "<html><b>Data must be provided for column '</b>" + msgIDTable.getColumnName(column)
                                        + "<b>' [row </b>" + (row + 1) + "<b>]",
