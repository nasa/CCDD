...
public class CcddDataTypeEditorDialog extends CcddDialogHandler {
    ...
    /**********************************************************************************************
     * Create the data type editor dialog. This is executed in a separate thread
                ...
                btnDeleteRow.addActionListener(new ValidateCellActionListener(dataTypeTable) {
                    ...
                    protected void performAction(ActionEvent ae) {
                        ...
                        for (int row : dataTypeTable.getSelectedRows()) {
                            ...
                            String dataType = CcddDataTypeHandler.getDataTypeName(
                                    dataTypeTable.getValueAt(row, DataTypesColumn.USER_NAME.ordinal()).toString(),
                            ...
                            if (!dataType.isEmpty()) {
                                ...
                                for (String dataTypeRef : getDataTypeReferences(dataType).getReferences()) {
                                    ...
                                    if (CcddVariableHandler.hasSizeof(
                                            tblColDescAndCntxt[SearchResultsQueryColumn.CONTEXT.ordinal()], dataType,
                                            macroHandler)) {
                                    ...
                                    String[] refColumns = CcddUtilities.splitAndRemoveQuotes(
                                            tblColDescAndCntxt[SearchResultsQueryColumn.CONTEXT.ordinal()]);
                                ...
                                if (!tables.isEmpty()) {
                                    ...
                                    new CcddDialogHandler().showMessageDialog(CcddDataTypeEditorDialog.this,
                                            "<html><b>Cannot delete data type '</b>" + dataType
                                                    ...
                                                    + "<b>'",
                                            "Delete Data Type", JOptionPane.ERROR_MESSAGE, DialogOption.OK_OPTION);
                                }
                            }
                        }
    ...
    private JScrollPane createDataTypeTable() {
        ...
        dataTypeTable = new CcddJTableHandler(DefaultPrimitiveTypeInfo.values().length) {
            ...
            /**************************************************************************************
             * Override getCellEditor so that for a base data type column cell the base data
             * type combo box cell editor is returned; for all other cells return the normal
             * cell editor
             *
            ...
            protected Boolean validateCellContent(List<Object[]> tableData, int row, int column, Object oldValue,
                ...
                try {
                    ...
                    if (!newValueS.isEmpty()) {
                        ...
                        // a non-integer
                        if (!oldValueS.isEmpty() && ((column == DataTypeEditorColumnInfo.SIZE.ordinal()
                            ...
                            for (int commRow = 0; commRow < committedData.length; commRow++) {
                                ...
                                if (index.equals(committedData[commRow][DataTypeEditorColumnInfo.OID.ordinal()])) {
                                    ...
                                    // tables
                                    String dataTypeName = CcddDataTypeHandler.getDataTypeName(
                                            CcddUtilities.convertObjectToString(committedData[commRow]));
                                    ...
                                    if (!tableNames.isEmpty()) {
                                        ...
                                        if (column == DataTypeEditorColumnInfo.SIZE.ordinal()) {
                                            throw new CCDDException(
                                                    "Bit length exceeds the size of the data type in table(s) '</b>"
                                                            + CcddUtilities.convertArrayToStringTruncate(
                                        ...
                                        // The base type changed
                                            ...
                                            throw new CCDDException(
                                                    "Base data type inconsistent with data type usage in table(s) '</b>"
                                                            + CcddUtilities.convertArrayToStringTruncate(
                                                                    tableNames.toArray(new String[0]))
                                                            + "<b>'");
                                        }
            ...
            if (row < dataTypeTable.getRowCount()) {
                ...
                for (int column = 0; column < dataTypeTable.getColumnCount() && !stopCheck; column++) {
                    ...
                    if (DataTypeEditorColumnInfo.values()[column].isRequired()
                        ...
                        if (new CcddDialogHandler().showMessageDialog(CcddDataTypeEditorDialog.this,
                                "<html><b>Data must be provided for column '</b>" + dataTypeTable.getColumnName(column)
                                        + "<b>' [row </b>" + (row + 1) + "<b>]",
                                "Missing Data", JOptionPane.WARNING_MESSAGE,
                                DialogOption.OK_CANCEL_OPTION) == CANCEL_BUTTON) {
                            // Set the stop flag to prevent further error checking
    ...
    /**********************************************************************************************
     * Compare the current data type table data to the committed data and create
