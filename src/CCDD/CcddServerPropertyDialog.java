...
public class CcddServerPropertyDialog extends CcddDialogHandler {
     ...
     *
     * @param useActiveDatabase true to use the active database; false to use the
     *                          default database. This is only used when opening the
     *                          database after changing the login credentials
     *
     * @param dialogType        database dialog type: LOGIN, PASSWORD, DB_SERVER, or
     *                          WEB_SERVER
     *
     * @param errorMessage      message to display following an unsuccessful login;
     *                          null if no error has occurred or this isn't the
     *                          login dialog
     *********************************************************************************************/
     ...
     *
     * @return true if the password is set in the Password dialog; false if Cancel
     *         was selected or the dialog type is not Password
     *********************************************************************************************/
     ...
     * Create the server properties dialog
     *
     * @param errorMessage message to display following an unsuccessful login; null
     *                     if no error has occurred or this isn't the login dialog
     *********************************************************************************************/
    private void initialize(String errorMessage) {
        ...
        case LOGIN:
            ...
            if (addLoginPanel(selectPnl, gbc, border)) {
                ...
                if (showOptionsDialog(ccddMain.getMainFrame(), selectPnl, "Select User", DialogOption.OK_CANCEL_OPTION,
                    ...
                    dbControl.openDatabaseInBackground(
                            useActiveDatabase ? dbControl.getProjectName() : DEFAULT_DATABASE);
                }
            }
            // No other user exists to choose
            else {
                // Inform the user that no other user exists on the server
                new CcddDialogHandler().showMessageDialog(ccddMain.getMainFrame(), "<html><b>No other user exists",
                        "Server Login", JOptionPane.WARNING_MESSAGE, DialogOption.OK_OPTION);
            }
        ...
        case PASSWORD:
            ...
            // Display the user & password dialog
            if (new CcddDialogHandler().showOptionsDialog(ccddMain.getMainFrame(), selectPnl, "Enter Password",
        ...
        case DB_SERVER:
            ...
            servers.addAll(
                    Arrays.asList(ccddMain.getProgPrefs().get(SERVER_STRINGS, "").split(STRING_LIST_TEXT_SEPARATOR)));
            ...
            if (showOptionsDialog(ccddMain.getMainFrame(), selectPnl, "Database Server",
                    DialogOption.OK_CANCEL_OPTION) == OK_BUTTON) {
                ...
                dbControl.openDatabaseInBackground(DEFAULT_DATABASE, hostFld.getText(), portFld.getText(),
        ...
        case WEB_SERVER:
            ...
            // Display the server properties parameter dialog
            if (showOptionsDialog(ccddMain.getMainFrame(), selectPnl, "Web Server",
                    DialogOption.OK_CANCEL_OPTION) == OK_BUTTON) {
     ...
     *
     * @return true if at least one user exists in the server or if the user can be
     *         entered manually
     *********************************************************************************************/
    private boolean addLoginPanel(JPanel selectPnl, GridBagConstraints gbc, Border border) {
        ...
        // No server connection exists; the user must enter a user name
            ...
            userFld.setBackground(
                    dialogType == ServerPropertyDialogType.LOGIN ? ModifiableColorInfo.INPUT_BACK.getColor()
