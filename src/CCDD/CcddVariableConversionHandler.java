...
public class CcddVariableConversionHandler
{
    ...
    // Lists that show all the original variables' full names, and the variable's full name before
    // and after converting any commas and brackets to underscores. Only variable's where the
    // converted name matches another variable's are saved in the latter two lists
    ...
    /**********************************************************************************************
     * Variable conversion handler class constructor for a list of supplied variables
     *
    ...
    CcddVariableConversionHandler(CcddMain ccddMain, TableTreeType treeType, boolean cleanName)
    {
        ...
        // Check if the path should have the macros expanded and the bit length removed
        if (cleanName)
        {
    ...
    /**********************************************************************************************
     * Variable conversion handler class constructor for all variables in the project database
     *
     * @param ccddMain
     *            main class reference
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Variable conversion handler class constructor for the tables and/or variables in the project
     * database as specified by the tree type
     *
     * @param ccddMain
     *            main class reference
     *
    ...
    /**********************************************************************************************
     * Get the reference to the table tree of instance tables, including the primitive variables
     *
     * @return Reference to the table tree of instance tables, including the primitive variables
     *********************************************************************************************/
                                         ...
                                         String typeNameSeparator)
    {
        ...
        if (fullName != null && !fullName.isEmpty())
        {
            ...
            // Check if the separator character is an underscore and that data types are to be
            // removed
            if (varPathSeparator.equals("_") && excludeDataTypes == true)
            {
                ...
                {
                    // Create the conversion list. The conversion list is needed since it's
                    // possible that duplicate variable path + names can occur if underscores are
                    // part of the names. The lists ensure that no duplicate is returned; instead,
                    // a unique name is created by appending one or more underscores to the
                    // otherwise duplicate name
            ...
            // The separator character isn't an underscore or the variable name isn't in the list
            else
            {
                // Convert the variable path + name using the separator character(s) to separate
    ...
    private void createConvertedVariableNameList(List<String> allVariableNameList)
    {
        ...
        for (int index = convertedVariableNameList.size() - 1; index >= 0; index--)
        {
            ...
            {
                // Remove the variable from the list. This shortens the list and allows all other
                // variables to have their full name built "on-the-fly"
                convertedVariableNameList.remove(index);
            }
            // This variable was modified
            else
            {
                // Add the variable to the list (add at the beginning to keep the order consistent
                // with the converted list)
