...
public class CcddMacroHandler {
    ...
    /**********************************************************************************************
     * Macro handler class constructor used when setting the macros from a source
     * other than those in the project database
     *
     * @param ccddMain main class
     *
     * @param macros   list of string arrays containing macro names and the
    ...
    CcddMacroHandler(CcddMain ccddMain, List<String[]> macros) {
        ...
        macroPattern = Pattern.compile(
                "^.*?(" + MACRO_IDENTIFIER + "([^" + MACRO_IDENTIFIER + "]+)" + MACRO_IDENTIFIER + ").*$",
     ...
     * Get the macro data
     *
     * @return List of string arrays containing macro names and the corresponding
     *         macro values
     *********************************************************************************************/
     ...
     *
     * @return List of macro names, sorted alphabetically; an empty list if no
     *         macros are defined
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Clear the list of expanded macro values. This should be done following any
     * change to a macro's unexpanded value so that the unexpanded value is
    ...
    /**********************************************************************************************
     * Get the the stored version of the supplied macro name. Macro names are case
     * insensitive; however the name as used in the internal table can be needed
     *
     * @param inputName name of the macro for which to get the stored name
     *
     * @return Macro name as stored in the internal macros table; null if the macro
     *         doesn't exist
     *********************************************************************************************/
     ...
     *
     * @return The supplied text string with all references to the specified macro
     *         replaced by the new macro name
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Replace all instances in all macros with sizeof() calls using the specified
     * data type name with the new data type name
     *
            ...
            if (CcddVariableHandler.hasSizeof(macros.get(index)[MacrosColumn.VALUE.ordinal()], oldDataTypeName,
                ...
                macros.set(index,
                        new String[] { macros.get(index)[MacrosColumn.MACRO_NAME.ordinal()],
                                macros.get(index)[MacrosColumn.VALUE.ordinal()].replaceAll(
                                        "(sizeof\\(+?\\s*)(?:" + oldDataTypeName + ")(\\s*\\))",
                                        "$1" + newDataTypeName + "$2") });
            }
        }
    }
     ...
     * expanded value is already known (from a previous value request) then this
     * known value is used; otherwise the macro's expanded value is evaluated from
     * its unexpanded value
     *
     * @param macroName name of the macro for which the value is needed
     *
     * @return Expanded value associated with the specified macro name; returns null
     *         if the macro doesn't exist. The isMacroRecursive flag will be set to
    ...
    /**********************************************************************************************
     * Get the value associated with the specified macro name. This is a recursive
     * method for macros referencing other macros
     *
     * @param macroName        macro name
     *
     * @param referencedMacros list containing the macros references in a macro
     *                         value path. This list is used to detect if a macro
     *                         references itself, which would cause an infinite
     *                         loop. Set to null to not test for recursion
     *
     * @return Value associated with the specified macro name; returns null if the
     *         macro doesn't exist
     *********************************************************************************************/
                        ...
                        if (!refMacros.isEmpty()) {
                            ...
                            for (int argIndex = 0; argIndex < refMacros.size(); argIndex++) {
                                ...
                                macroValue = macroValue.replaceAll(
                                        MACRO_IDENTIFIER + refMacros.get(argIndex) + MACRO_IDENTIFIER, value);
                            }
                        }
     ...
     * @param macroName macro name
     *
     * @return Index for the macro with the supplied name; -1 if no macro with this
     *         name exists
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Replace any macro names and sizeof() calls embedded in the supplied text with
     * the associated macro values and data type sizes
     *
     * @param text text string possibly containing macro names and/or sizeof() calls
     *
     * @return Text string with any embedded macro names and sizeof() calls replaced
    ...
    /**********************************************************************************************
     * Replace any macro names and sizeof() calls embedded in the supplied text with
     * the associated macro values and data type sizes. If a list of invalid data
     * types is supplied, sizeof() calls set an error flag if the referenced data
     * type is in the list
     *
     * @param text             text string possibly containing macro names and/or
     *                         sizeof() calls
     *
     ...
     *
     * @return Text string with any embedded macro names and sizeof() calls replaced
     *         with the associated macro values and data type sizes; if no macro or
     *         sizeof() call is present the text is returned unchanged
     *********************************************************************************************/
     ...
     * @param text text string containing macro names
     *
     * @return List of macros referenced in the specified text string. A macro name
     *         only appears once in the list even if referenced multiple times in
     *         the text string
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Get the list of structure names referenced in sizeof() calls in the specified
     * macro
     *
     * @param macroName macro name
     *
     * @return List of structure names referenced in sizeof() calls in the specified
     *         macro macro; and empty list if no structures are referenced or the
     *         macro doesn't exist
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Get a list containing the macros that are dependent on the specified data
     * type
     *
     * @param dataType data type name for which to search
     *
     * @return List containing the macros that are dependent on the specified data
     *         type
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Add the name of specified macro and the names of all macros that depend on
     * this macro's value to the supplied list. This is a recursive method
     *
     * @param macroName       name of the macro for which to find all dependent
     *                        macros
     *
     * @param dependentMacros List of macros that have a value that is dependent on
     *                        a specified macro (list includes the specified macro)
     *********************************************************************************************/
    private void getDependentMacros(String macroName, List<String> dependentMacros) {
        ...
        if (!dependentMacros.contains(macroName)) {
            ...
            // Step through each macro definition
            for (String[] macro : macros) {
                // Check that this isn't the definition for the specified macro. Since a macro
                // is
                // programatically prevented from referencing itself (since that would
                // constitute a
    ...
    /**********************************************************************************************
     * Get a list containing the search results in the project database for tables
     * that reference the specified macro name. Include references in the custom
     * values table
     *
     ...
     * @param parent    GUI component over which to center any error dialog
     *
     * @return List containing the search results in the project database for tables
    ...
    /**********************************************************************************************
     * (Re)initialize the lists for the macro modifications and macro definitions.
     * This must be called prior to the first call to updateMacros(), or to clear
     * the lists
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Initialize the list of already loaded macro references. This list is used
     * when macros are altered via the macro editor or an import operation
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Get the references to the specified macro in the tables. A list of loaded
     * reference results is maintained so that a previous search for a macro can be
     * reused (initializeReferences() must be called to clear this list, such as
     * after a macro has been altered)
     *
     * @param macroName macro name
     *
     * @param parent    GUI component over which to center any error dialog
    ...
    /**********************************************************************************************
     * Get the list containing the name of every table that references the specified
     * macro
     *
     * @param macroName macro name
     *
     * @param parent    GUI component over which to center any error dialog
     *
     * @return Reference to the specified macro in the tables; an empty list if the
     *         macro is unused
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Verify that the updated macros are valid for each instance where the macro is
     * used (e.g., a table column with an input type of "Integer" can't accept a
     * text string)
     *
     * @param parent GUI component over which to center any error dialog
     *
     * @throws CCDDException If a macro's value is invalid in an instance where the
     *                       macro is used
     *********************************************************************************************/
     ...
     *
     * @param newMacroHandler reference to the macro handler that incorporates the
     *                        macro updates
     *
     * @param parent          GUI component over which to center any error dialog
     *
     ...
     *                       macro is used
     *********************************************************************************************/
    protected void validateMacroUsage(String macroName, CcddMacroHandler newMacroHandler, Component parent)
                ...
                if (!tableNames.contains(refNameAndType[0])) {
                    ...
                    for (String oldName : getReferencedMacros(refContext[columnIndex])) {
                        ...
                        refContext[columnIndex] = replaceMacroName(CcddMacroHandler.getFullMacroName(oldName),
                                CcddMacroHandler.getFullMacroName(newName), refContext[columnIndex]);
                    }

                    // Check if the expanded value of the updated macro doesn't match the input
                    // type required by the macro's user
                    if (!newMacroHandler.getMacroExpansion(refContext[columnIndex])
                            .matches(inputType.getInputMatch())) {
        ...
        if (!tableNames.isEmpty()) {
            throw new CCDDException("Macro value is not consistent with macro usage in table(s) '</b>"
                    + CcddUtilities.convertArrayToStringTruncate(tableNames.toArray(new String[0])) + "<b>'");
        }
    }

    /**********************************************************************************************
     * Update all existing macro instances where the macro name or value has been
     * changed. This method relies on calling updateMacros() to set the lists of
     * modifications and updated macros
     *
     * @param parent GUI component over which to center any error dialog
     *
     * @throws CCDDException If an error occurs updating an existing reference to a
     *                       modified macro
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Tool tip text showing any macro names embedded in the supplied text replaced
     * with the associated macro values
     *
     * @param text text string containing macro names
     *
     * @return Tool tip text string showing any embedded macro names in the supplied
     *         text replaced with the associated macro values
     *********************************************************************************************/
    protected String getMacroToolTipText(String text) {
        ...
        if (text != null && (hasMacro(text) || CcddVariableHandler.hasSizeof(text))) {
            ...
            if (text != null) {
                ...
                // text
                text = CcddUtilities.wrapText(
                        "<html><i><b>Macro Expansion:</b></i>" + "<br><p style=\"margin-left: 5px\">" + text,
     ...
     *
     * @param textComp       text component over which to display the pop-up combo
     *                       box and insert the selected macro name
     *
     ...
     *********************************************************************************************/
    protected List<String> getMacroPopUpItems(JTextComponent textComp, InputType inputType,
        ...
        if (!macros.isEmpty()) {
            ...
            for (String[] macro : macros) {
                ...
                // Initialize the parentheses counters. For each left (right) parenthesis a
                // right
                // (left) one is added to the end (beginning) of the string. This ensures the
                // parentheses are balanced if the number of left and right parentheses are
                // unequal
                ...
                // parentheses; it serves only to ensure that the resulting string a valid
                // formula.
