...
public class CcddKeyboardHandler {
    ...
    /**********************************************************************************************
     * Set the references for the undo manager and table (if applicable) to those of
     * the active modal dialog (e.g., the group manager or macro editor dialogs).
     * Since the modal dialogs lock the user interface while active a reference to
     * them cannot be set for the code to access as it can with the non-modal
     * dialogs (i.e., the table editors and table type editor). The modal undo
     * manager must be set to null after the dialog closes so that the non-modal
     * dialogs (e.g., table and table type editors) undo managers can handle
     * undo/redo operations
     *
     * @param undoManager modal dialog undo manager; null to disable
     *
     * @param table       modal dialog table reference; null if the modal dialog has
     *                    no table
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Adjust the handling of Enter and space key inputs in order to activate dialog
     * controls, and keyboard focus changes in order to use the arrow keys like Tab
     * keys
     *********************************************************************************************/
    private void setKeyboardHandler() {
        ...
        focusManager.addKeyEventDispatcher(new KeyEventDispatcher() {
            ...
            /**************************************************************************************
             * Alter the response to the Enter key to act like the Space key to activate
             * button and check box controls, and the arrow keys so as to mimic the Tab and
             * Shift+Tab keys, unless the key press originates within a table or combo box.
             * For a tabbed pane the left/right arrows are left unchanged so that these are
             * used for traversing the tabbed panes, but the down and up arrows act like Tab
            ...
            public boolean dispatchKeyEvent(final KeyEvent ke) {
                ...
                if (ke.getID() == KeyEvent.KEY_PRESSED && !ke.isControlDown() && !ke.isShiftDown() && !ke.isAltDown()) {
                    ...
                    // produce a character and is not a modifier; this covers the arrow keys)
                        ...
                        // button, or check box
                        else if (comp instanceof JButton || comp instanceof JRadioButton || comp instanceof JCheckBox) {
                ...
                // Check if the Ctrl key is pressed
                    ...
                    if (ke.getKeyCode() == KeyEvent.VK_Z) {
                        ...
                        if (undoManager != null) {
                            ...
                            SwingUtilities.invokeLater(new Runnable() {
                                /******************************************************************
                                 * Execute after all pending Swing events are finished so that the events occur
                    ...
                    // Check if the Ctrl-F key is pressed
                        ...
                        if (ccddMain.getMainFrame().isFocused()) {
                            ...
                            ccddMain.showSearchDialog(SearchDialogType.LOG, null, ccddMain.getSessionEventLog(),
                        ...
                        else if (ccddMain.getTableTypeEditor() != null && ccddMain.getTableTypeEditor().isFocused()) {
                    ...
                    // insertion
                        ...
                        if (editorDialog != null && comp.getParent() instanceof CcddJTableHandler
                            ...
                            if (editorDialog.getTableEditor().getTable().isEditing()) {
                                ...
                                new PopUpComboBox(editorDialog, (JTextComponent) comp,
                        ...
                        // Check if this is a description or data field
                            ...
                            new PopUpComboBox(SwingUtilities.getWindowAncestor(comp), (JTextComponent) comp,
                        ...
                        // Check if editing is active in the data type editor
                            ...
                            String baseType = modalTable.getValueAt(row, DataTypeEditorColumnInfo.BASE_TYPE.ordinal())
                                    .toString();
                    ...
                    // and expansion
                        ...
                        // that doesn't contain a combo box or check box
                            ...
                            if (editorDialog != null) {
                                ...
                                if (table.isEditing()) {
                                    ...
                                    new PopUpComboBox(editorDialog, (JTextComponent) comp,
                                            macroHandler.getMacroPopUpItems((JTextComponent) comp, inputType,
                            ...
                            // column is being edited
                                    ...
                                    && modalTable.isEditing() && modalTable.convertColumnIndexToModel(
                                            modalTable.getEditingColumn()) == MacroEditorColumnInfo.VALUE.ordinal()) {
                        ...
                        // Check if this is a description or data field
                            ...
                            new PopUpComboBox(SwingUtilities.getWindowAncestor(comp), (JTextComponent) comp,
                                    macroHandler.getMacroPopUpItems((JTextComponent) comp,
                                            inputTypeHandler.getInputTypeByDefaultType(DefaultInputType.TEXT), null),
                                    macroHandler.getMacroPopUpToolTips(), ModifiableFontInfo.INPUT_TEXT.getFont()) {
                                /******************************************************************
                        ...
                        releaseTimer = new Timer(75, new ActionListener() {
                            ...
                            public void actionPerformed(ActionEvent ae) {
                                ...
                                // Check if this is the macro editor
                                        ...
                                        .getWindowAncestor(modalTable) instanceof CcddMacroEditorDialog) {
                                    // Expand the macros in the macro value column
                                    ((CcddMacroEditorDialog) SwingUtilities.getWindowAncestor(modalTable))
                                            .expandMacros(false);
                                    isShowMacros = false;
                                }
                            }
     ...
     *
     * @return The active table editor dialog; null if no table editor dialog is
     *         active or no editor dialog has focus
     *********************************************************************************************/
    ...
    private CcddUndoManager getActiveUndoManager() {
        ...
        // No table cell is being edited
            ...
            if (focusOwner != null && (focusOwner instanceof UndoableTextField || focusOwner instanceof UndoableTextArea
    ...
    /**********************************************************************************************
     * Handle Enter and space key press events in a table in order to activate check
     * box controls and initiate editing for editable fields. The space key would do
     * these without this method; it is included in order to eliminate the cell
     * background color 'flash' that occurs when a check box is toggled
     *
