...
/**************************************************************************************************
 * CFS Command and Data Dictionary script handler class. This class handles
...
public class CcddScriptHandler {
     ...
     *
     * @param scriptDialog reference to the active script manager or executive
     *                     dialog
     *********************************************************************************************/
    ...
    protected List<String[]> getScriptAssociations(Component parent) {
        ...
        List<String[]> associations = dbTable.retrieveInformationTable(InternalTable.ASSOCIATIONS, false, parent);
     ...
     * @param parent   GUI component over which to center any error dialog
     *
     * @return Array containing the script association having the same name; null if
     *         no association has the specified name
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Retrieve the script associations stored in the database and from these build
     * the array for display and selection of the script associations
     *
     * @param parent GUI component over which to center any error dialog
     *
    ...
    private Object[][] getScriptAssociationTableData(Component parent) {
        ...
        for (String[] assn : committedAssociations) {
            ...
            try {
                ...
                List<String> tablePaths = getAssociationTablePaths(
                        assn[AssociationsColumn.MEMBERS.ordinal()].toString(), groupHandler, false, parent);
            ...
            associationsData.add(new Object[] { assn[AssociationsColumn.NAME.ordinal()],
                    assn[AssociationsColumn.DESCRIPTION.ordinal()], assn[AssociationsColumn.SCRIPT_FILE.ordinal()],
                    CcddUtilities.highlightDataType(assn[AssociationsColumn.MEMBERS.ordinal()]), availableStatus });
        }
     ...
     *
     * @param title               text to display above the script associations
     *                            table; null or blank if no text is to be displayed
     *
     * @param allowSelectDisabled true if disabled associations can be selected;
     *                            false if not. In the script manager disabled
     *                            associations are selectable so that these can be
     *                            deleted if desired. Scripts that are selected and
    ...
    protected JPanel getAssociationsPanel(String title, final boolean allowSelectDisabled, final Component parent) {
        ...
        GridBagConstraints gbc = new GridBagConstraints(0, 0, 1, 1, 1.0, 0.0, GridBagConstraints.LINE_START,
                GridBagConstraints.BOTH, new Insets(ModifiableSpacingInfo.LABEL_VERTICAL_SPACING.getSpacing(), 0, 0, 0),
                0, 0);
        ...
        hideUnavailableAssns.setToolTipText(CcddUtilities.wrapText(
                "Remove associations that cannot be executed (due to a "
        ...
        rowFilter = new RowFilter<TableModel, Object>() {
            ...
            public boolean include(Entry<? extends TableModel, ? extends Object> entry) {
                ...
                return !(hideUnavailableAssns.isSelected() && !isAssociationAvailable((Integer) entry.getIdentifier()));
            }
        };
        ...
        assnsTable = new CcddJTableHandler() {
            ...
            protected void loadAndFormatData() {
                ...
                int totalWidth = setUpdatableCharacteristics(getScriptAssociationTableData(parent),
                        AssociationsTableColumnInfo.getColumnNames(), null, AssociationsTableColumnInfo.getToolTips(),
                        true, true, true);
                ...
                if (scriptDialog != null) {
                    ...
                    scriptDialog.setTableWidth(totalWidth + LAF_SCROLL_BAR_WIDTH
            ...
            protected void setTableSortable() {
                ...
                if (sorter != null) {
                    ...
                    sorter.setComparator(AssociationsTableColumnInfo.SCRIPT_FILE.ordinal(), new Comparator<String>() {
                        /**************************************************************************
                         * Override the comparison when sorting the script file column to ignore the
                         * script file paths if these are currently hidden
                         *************************************************************************/
                        @Override
        ...
        assnsTable.setSelectionModel(new DefaultListSelectionModel() {
             ...
             * that are flagged as unavailable
             *************************************************************************************/
            @Override
        ...
        assnsTable.setFixedCharacteristics(scrollPane, false, ListSelectionModel.MULTIPLE_INTERVAL_SELECTION,
                TableSelectionMode.SELECT_BY_ROW, true, ModifiableColorInfo.TABLE_BACK.getColor(), true, true,
        ...
        envVarOverrideFld.setBorder(BorderFactory.createCompoundBorder(
     ...
     * table is available. An association is unavailable if the script or tables is
     * not present
     *
     ...
     * referenced then its member tables are included. This method is also used to
     * parse a string of table names when exporting via the command line export
     * command
     *
     * @param associationMembers association members as a single string (as stored
     *                           in the database)
     *
     * @param groupHandler       group handler reference
     *
     * @param isForExport        true when using this method to build the list of
     *                           tables to export via the command line export
     *                           command. This flag should be set to false when
     *                           using this method for obtaining the tables for
     *                           script associations
     *
     * @param parent             GUI component over which to center any error dialog
     *
     * @return List containing the tables (path+name) from the association member
     *         string
     *********************************************************************************************/
    protected List<String> getAssociationTablePaths(String associationMembers, CcddGroupHandler groupHandler,
            boolean isForExport, Component parent) {
    ...
    /**********************************************************************************************
     * Get an array of all script file extensions supported by the available script
     * engines
     *
     * @return Array of all script file extensions supported by the available script
     *         engines
     *********************************************************************************************/
    protected FileNameExtensionFilter[] getExtensions() {
        ...
        for (ScriptEngineFactory factory : scriptFactories) {
            ...
            filters.add(new FileNameExtensionFilter(name + " files", factory.getExtensions().toArray(new String[0])));
        }
     ...
     *         numbers appropriate for display in the Help | About dialog; returns
     *         "none" if no scripting languages are installed
     *********************************************************************************************/
    protected String getEngineInformation() {
        ...
        for (ScriptEngineFactory factory : scriptFactories) {
            ...
            engineInfo.add(CcddUtilities.colorHTMLText(name + ": ", ModifiableColorInfo.SPECIAL_LABEL_TEXT.getColor())
    ...
    /**********************************************************************************************
     * Update the environment variable map with any variables specified by the user.
     * Update the association availability based on if the script exists, using the
     * current environment variables to expand its path
     *
     * @param parent GUI component over which to center any error dialog
     *
     * @return true if the each key has a corresponding value; false if a value is
     *         missing
     *********************************************************************************************/
            ...
            for (int row = 0; row < assnsTable.getRowCount(); row++) {
                ...
                if (assnsTable.getModel().getValueAt(row,
                    ...
                    ((UndoableTableModel) assnsTable.getModel()).setValueAt(
                            (file.exists() ? AvailabilityType.AVAILABLE : AvailabilityType.SCRIPT_MISSING), row,
                            AssociationsTableColumnInfo.AVAILABLE.ordinal(), false);
                }
            }
    ...
    /**********************************************************************************************
     * Get the index from the supplied script association list for the association
     * with the same script file name and members
     *
     ...
     *
     * @param ignoreRow    row to ignore when checking for an identical, existing
     *                     association (as is possible when replacing an
     *                     association, if no changes are made); -1 to prevent a
     *                     duplicate association (as when adding an association)
     *
     * @return The index number in the supplied list where the matching association
     *         occurs; -1 if no match is found
     *********************************************************************************************/
    protected static int getMatchingAssociation(List<String[]> associations, String scriptName, String[] tables,
            int ignoreRow) {
        int matchingIndex = -1;
    ...
    /**********************************************************************************************
     * Execute one or more scripts based on the script associations in the script
     * associations list
     *
     * @param tableTree CcddTableTreeHandler reference describing the table tree
     *
     * @param dialog    reference to the script dialog (manager or executive)
     *                  calling this method
     *********************************************************************************************/
     ...
     * by using a separate thread the GUI is allowed to continue to update. The
     * script execution command, however, is disabled until the this command
     * completes execution
     *
     * @param tree         table tree of the table instances (parent tables with
     *                     their child tables); null if the tree should be loaded
     *
     * @param associations list of script association to execute
     *
     * @param dialog       reference to the entity calling this method: the script
     *                     manager or executive dialog, or to the main window (if
     ...
     *
     * @param parent       GUI component over which to center any error dialog; null
     *                     if none (e.g., if called via the command line)
     *
     * @return Array containing flags that indicate, for each association, if the
     *         association did not complete successfully
     *********************************************************************************************/
    protected boolean[] getDataAndExecuteScript(CcddTableTreeHandler tree, List<Object[]> associations,
            Component parent) {
        int assnIndex = 0;
        int step = 0;
            ...
            try {
                ...
                if (!tablePaths.isEmpty()) {
                    ...
                    Collections.sort(tablePaths, new Comparator<String>() {
                        /**************************************************************************
                         * Sort the table paths so that the root tables are in alphabetical order and
                         * the child tables appear in the order defined by their table type definition
                         *************************************************************************/
                        @Override
                    ...
                    for (String tablePath : tablePaths) {
                        ...
                        if (tableInfo != null) {
                            ...
                            if (tableInfo.isErrorFlag()) {
                                throw new CCDDException("Table '" + tableInfo.getProtoVariableName()
                            ...
                            // The table loaded successfully
                                ...
                                // The table's type is invalid
                                    ...
                                    throw new CCDDException("Table '" + tableInfo.getProtoVariableName()
                                            + "' has unknown type '" + tableInfo.getType() + "'");
                                }
                            }
                        }
                    }
                ...
                logScriptError(
                        FileEnvVar.expandEnvVars(assn[AssociationsColumn.SCRIPT_FILE.ordinal()].toString(), envVarMap),
                        assn[AssociationsColumn.MEMBERS.ordinal()].toString(), ce.getMessage(), parent);
        ...
        for (Object[] assn : associations) {
            ...
            if (!isBad[assnIndex]) {
                ...
                String scriptFileName = FileEnvVar
                        .expandEnvVars(assn[AssociationsColumn.SCRIPT_FILE.ordinal()].toString(), envVarMap);
                ...
                try {
                    ...
                    if (haltDlg != null) {
                        ...
                        haltDlg.updateProgressBar((!assn[AssociationsColumn.NAME.ordinal()].toString().isEmpty()
                                ...
                                : scriptFileName), haltDlg.getNumDivisionPerStep() * step);
                        step++;
                    }

                    // Execute the script using the indicated table data
                    executeScript(scriptFileName, combinedTableInfo, groupNames, linkHandler, groupHandler, parent);
                } catch (CCDDException ce) {
                    // Inform the user that script execution failed
                    logScriptError(scriptFileName, assn[AssociationsColumn.MEMBERS.ordinal()].toString(),
                            ce.getMessage(), parent);
    ...
    /**********************************************************************************************
     * Convert the supplied string containing the association members from the
     * internal table to the viewable format, or vice versa, depending on the input
     * flag
     *
     * @param assnMembers tables and/or groups associated with the script, in either
     *                    the internal or viewable format
     *
     * @param toInternal  true to convert the member string to the internal format;
     *                    false to convert to the viewable format
     *
     * @return Supplied string containing the association members converted from the
     ...
     *
     * @param isBad        Array containing flags that indicate, for each
     *                     association, if the association did not complete
     *                     successfully
     *********************************************************************************************/
     ...
     *
     * @param members        tables and/or groups associated with the script, in the
     *                       internal format
     *
     ...
     * @param parent         GUI component over which to center any error dialog
     *********************************************************************************************/
    private void logScriptError(String scriptFileName, String members, String cause, Component parent) {
    ...
    /**********************************************************************************************
     * Create a script engine for the supplied script file name and table
     * information. Non-static and static script data access handlers are bound to
     * the engine so that the public access methods can be utilized
     *
     * @param scriptFileName   script file name. The file extension is used to
     *                         determine the script engine and therefore must
     *                         conform to standard extension usage
     *
    ...
    /**********************************************************************************************
     * Recursive method to load a table, and all the tables referenced within it and
     * its child tables. The data is combined into a single array
     *
     * @param tablePath table path
     *
     * @param parent    GUI component over which to center any error dialog
     *
     * @return A reference to the TableInformation for the parent table; null if the
