...
public class CcddRateParameterHandler {
    ...
    /**********************************************************************************************
     * Get the maximum number of seconds allowed between downlinking two of the same
     * message
     *
     * @return Maximum number of seconds allowed between downlinking two of the same
     *         message
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Get the value of the flag that indicates if unevenly time-space sample rates
     * are to be included
     *
     * @return true if unevenly time-spaced sample rate values are included; false
     *         if only sample rates that are evenly time-spaced are included
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Change the rate column name for in the rate information list. If the original
     * rate column name matches one in another table type then create a new rate
     * entry instead of renaming the existing one. If the new rate column name
     * matches one in another table type then merge it with the existing one and
     * delete the original
     *
     ...
     *
     * @return Rate information with the specified rate column; null if the rate
     ...
     *
     * @return Rate information with the data stream name; null if the data stream
     *         doesn't exist
     *********************************************************************************************/
     ...
     * @param rateColumnName rate column name
     *
     * @return Index of the rate information with the specified rate column name; -1
     *         if no rate information has this rate column name
     *********************************************************************************************/
     ...
     *
     * @return Index of the rate information with the specified stream name; -1 if
     *         no rate information has this stream name
     *********************************************************************************************/
    ...
    private void getRateParameters() {
        ...
        try {
            ...
            ccddMain.getSessionEventLog().logFailEvent(ccddMain.getMainFrame(), "Rate Parameter Error",
    ...
    /**********************************************************************************************
     * Set the rate parameters, store them in the project database, and calculate
     * the sample rates
     *
    ...
    protected void setRateParameters(int maxSecPerMsg, int maxMsgsPerSec, String[] streamName, int[] maxMsgsPerCycle,
        ...
        for (int index = 0; index < rateInformation.size(); index++) {
            ...
            rateInformation.get(index).setStreamName(
                    streamName[index].isEmpty() ? rateInformation.get(index).getRateName() : streamName[index]);
     ...
     * current rate parameters. Update open table editors that have a Rate column
     *********************************************************************************************/
    private void calculateSampleRates() {
        // Step through each data stream
        for (RateInformation rateInfo : rateInformation) {
            // Store the rates in array form in the rate information
            rateInfo.setSampleRates(
                    calculateSampleRates(maxSecPerMsg, maxMsgsPerSec, rateInfo.getMaxMsgsPerCycle(), includeUneven));
        }
    ...
    /**********************************************************************************************
     * Format a rate value as the specified number of samples over the specified
     * number of seconds. Use whole numbers if possible; otherwise use the format
     * '1/x', where 'x' is the number of seconds rounded to 5 decimal places. Remove
     * any extra trailing zeroes after the decimal and the decimal if no zeroes
     * remain for both the numerator and denominator
     *
    ...
    /**********************************************************************************************
     * Get the array of the sample rate values for the specified rate column name
     * with those rates not assigned to any telemetry parameter in the structure
     * tables grayed out
     *
     ...
     *
     * @return Array of the sample rate values for the specified rate column name
     *         with those rates not assigned to any telemetry parameter in the
     *         structure tables grayed out; an empty array if the rate name isn't
     *         valid
     *********************************************************************************************/
