...
public class CcddInputTypeHandler {
    ...
    /**********************************************************************************************
     * Variable (command) reference class; used to contain information on structure
     * (command) tables of the same table type having a column with the variable
    ...
    protected class InputTypeReference {
         ...
         *
         * @param columnDb      column name (as used in the database) having the
         *                      variable (command) reference input type
         *
         * @param columnVisible column name (as seen by the user) having the variable
         *                      (command) reference input type
         *
         * @param tables        array of prototype structure (command) tables of the
         *                      table type having a column with the variable (command)
         ...
         *
         * @return Column name (as used in the database) having the variable (command)
         *         reference input type
         *****************************************************************************************/
         ...
         *
         * @return Column name (as seen by the user) having the variable (command)
         *         reference input type
         *****************************************************************************************/
         ...
         *
         * @return Array of prototype structure (command) tables of the table type
    ...
    protected class ReferenceCheckResults {
         ...
         *
         * @param references       list of references in the table type and data field
         *                         internal tables that use the input type
         *
         * @param isRefFieldChange true to indicate that a data field is using the input
         *                         type
         *****************************************************************************************/
        ...
        /******************************************************************************************
         * Get the list of references in the table type and data field internal tables
         * that use the input type
         *
         * @return List of references in the table type and data field internal tables
         *         that use the input type
         *****************************************************************************************/
    ...
    /**********************************************************************************************
     * Set the input types by combining the default types with those in the supplied
     * array
     *
    ...
    protected void setInputTypeData(String[][] customInputTypes) {
        ...
        for (String[] customType : customInputTypes) {
            ...
            inputTypes.add(new InputType(customType[InputTypesColumn.NAME.ordinal()],
                    customType[InputTypesColumn.DESCRIPTION.ordinal()], customType[InputTypesColumn.MATCH.ordinal()],
                    customType[InputTypesColumn.ITEMS.ordinal()],
                    InputType.getInputFormatByName(customType[InputTypesColumn.FORMAT.ordinal()]), true));
        }
    ...
    /**********************************************************************************************
     * Update the variable references input type selection item list. Based on
     * whether or not the variable list is empty, add or remove the input type from
     * the list of those having selection items
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Update the message references input type selection item list with the
     * supplied message name, ID, and owner list. Based on whether or not the
     * message list is empty, add or remove the input type from the list of those
     * having selection items
     *
     ...
     * @param inputTypeName input type name to match (case insensitive)
     *
     * @return Input type with the name that matches the one specified; returns the
     *         input type for text if the input type doesn't exist
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Get the input type with the name that matches that of the specified default
     * input type
     *
    ...
    /**********************************************************************************************
     * Convert the supplied input selection item string to the corresponding input
     * match regular expression
     *
     * @param itemString string containing the acceptable values for this input
     *                   type, separated by the selection item list separator; null
     *                   or blank if the input type doesn't constrain the inputs to
     *                   items from a list
     *
    ...
    /**********************************************************************************************
     * Get an array of the input type names (all or user-defined only), excluding
     * separators and breaks
     *
     * @param customOnly true to only include user-defined input types; false to
     *                   include all input types
     *
     * @return Array of all or only user-defined input type names, sorted
     *         alphabetically; an empty list if no user-defined input types are
     *         defined and only user-defined types are requested
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Get an array of all of the input type descriptions, sorted based on the
        ...
        if (!valueS.isEmpty()) {
            try {
                ...
                case HEXADECIMAL:
                    ...
                    String valueSTemp = valueS.replaceFirst("^0x|^0X", "").replaceFirst("\\s*" + PROTECTED_MSG_ID_IDENT,
                            "");
                ...
                case MESSAGE_ID:
                    ...
                    // Check if a message name and ID are present
                        ...
                        valueS = values[0] + " " + formatInput(values[1], InputTypeFormat.HEXADECIMAL, preserveZeroes);
                    }
    ...
    /**********************************************************************************************
     * Get the references in the table type and data field internal tables that
     * match the specified input type name. Only the columns containing input types
     * in the table type and data field internal tables are searched
     *
     ...
     *         specified input type name; an empty array if no matches are found
     *********************************************************************************************/
    protected String[] searchInputTypeReferences(String inputTypeName, Component parent) {
        return dbCommand.getList(DatabaseListCommand.SEARCH,
                new String[][] {
                        { "_search_text_", "^" + CcddUtilities.escapePostgreSQLReservedChars(inputTypeName) + "$" },
    ...
    /**********************************************************************************************
     * Get the results of a search for references in the table type and data field
     * internal tables for the specified default input type
     *
     * @param inputType default input type for which to search (DefaultInputType)
     *
     * @param parent    GUI component over which to center any error dialog
     *
     * @return Reference to the ReferenceCheckResults containing the list of
     *         references in the table type and data field internal tables that use
     *         the specified input type (an empty list if there are no references)
     *         and the flag that indicates if a data field is using the input type
     *********************************************************************************************/
        ...
        for (String typeRef : searchInputTypeReferences(inputType.getInputName(), parent)) {
            ...
            if (tblColCmtAndCntxt[SearchResultsQueryColumn.TABLE.ordinal()]
                ...
                references.add(new InputTypeReference(refColumns[TableTypesColumn.COLUMN_NAME_DB.ordinal()],
                        refColumns[TableTypesColumn.COLUMN_NAME_VISIBLE.ordinal()], ccddMain.getDbTableCommandHandler()
