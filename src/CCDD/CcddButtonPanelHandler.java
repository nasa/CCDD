...
public class CcddButtonPanelHandler {
    ...
    /**********************************************************************************************
     * Get the button panel's close/cancel button which is assumed to be the last
    ...
    /**********************************************************************************************
     * Extract the button(s) from the supplied panel, then find the widest button
     * calculated from the button's text and icon. Set the width of the panel
    ...
    protected void setButtonWidth() {
        ...
        // heights
        Dimension buttonPnlCoord = new Dimension(
                maxWidth * numColumns + (ModifiableSpacingInfo.BUTTON_GAP.getSpacing() + 1) * (numColumns + 1),
                maxHeight * buttonRows + (ModifiableSpacingInfo.BUTTON_GAP.getSpacing() + 1) * (buttonRows + 1));
     ...
     *
     * @param toolTip    tool tip text to display when the pointer hovers over this
    ...
    /**********************************************************************************************
     * Create the window's button panel and combine with the upper components, set
     * up keyboard focus management, and determine the window's exit button. If no
     * button panel is provided then create the buttons and button listeners needed
     * based on the option type
     *
     * @param btnPanel       panel containing the window's buttons; null if a
     *                       defined option type is used
     *
     * @param defaultBtn     reference to the JButton that is actuated if the Enter
     *                       key is pressed; null to have no default button (unless
     *                       btnPanel is null as well, in which case the first
    ...
    protected JPanel assembleWindowComponents(JPanel btnPanel, JButton defaultBtn, JComponent upperComponent,
            DialogOption optionType, final Container contentPane, final JRootPane rootPane) {
        ...
        bpPanel.setBorder(
                BorderFactory.createEmptyBorder(0, 0, ModifiableSpacingInfo.DIALOG_BORDER_PAD.getSpacing(), 0));
