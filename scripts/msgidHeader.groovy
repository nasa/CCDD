...
def makeTelemetryFile(baseFileName)
{
    ...
    if (tlmFile != null)
    {
        ...
        def groupNames = ccdd.getGroupNames(false)

        def minimumLength = 12

        // Step through each structure name
        for (def nameIndex = 0; nameIndex < structureNames.length; nameIndex++)
        {
            // Get the value of the structure's message ID name data field
            def msgIDName = ccdd.getTableDataFieldValue(structureNames[nameIndex], "Message ID Name")
        ...
        for (def groupIndex = 0; groupIndex < groupNames.length; groupIndex++)
        {
            // Get the value of the group's message ID name data field
            def msgIDName = ccdd.getGroupDataFieldValue(groupNames[groupIndex], "Message ID Name")
        ...
        def format = "#define %-" + (minimumLength + 1) + "s %s\n"
        ...
        for (def nameIndex = 0; nameIndex < structureNames.length; nameIndex++)
        {
            ...
            // fields
            def msgID = ccdd.getTableDataFieldValue(structureNames[nameIndex], "Message ID")
            def msgIDName = ccdd.getTableDataFieldValue(structureNames[nameIndex], "Message ID Name")
        ...
        for (def groupIndex = 0; groupIndex < groupNames.length; groupIndex++)
        {
            ...
            def msgID = ccdd.getGroupDataFieldValue(groupNames[groupIndex], "Message ID")
            def msgIDName = ccdd.getGroupDataFieldValue(groupNames[groupIndex], "Message ID Name")
...
def makeCommandFile(baseFileName)
{
    ...
    def cmdFileName = ccdd.getOutputPath() + baseFileName + ".h"
    def headerIncludeFlag = "_" + baseFileName.toUpperCase() + "_H_"

    // Open the command codes output file
    def cmdFile = ccdd.openOutputFile(cmdFileName)
    ...
    if (cmdFile != null)
    {
        ...
        for (def row = 0; row < numCommandRows; row++)
        {
            ...
            def cmdName = ccdd.getCommandTableData("command name", row)
            def cmdCode = ccdd.getCommandTableData("command code", row)
