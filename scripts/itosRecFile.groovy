 ...
 *
 * This Groovy script generates an ITOS record file from the supplied
...
def isTelemetry(row)
{
    ...
    def rates = ccdd.getStructureRates(row)

    // Step through each rate column value
    for (def index = 0; index < rates.length; index++)
    {
...
def reorderRowsForByteOrder(endian)
{
    ...
    if (endian.equals("LE"))
    {
        ...
        for (def tgtRow = 0; tgtRow < numStructRows; tgtRow++)
        {
            ...
            def tgtStructName = ccdd.getStructureTableNameByRow(tgtRow)

            // Get the byte offset of the target variable
            def tgtVarPath = ccdd.getFullVariableNameRaw(tgtRow)
            def tgtOffset = ccdd.getVariableOffset(tgtVarPath)

            // Step through the remaining rows so that the structures can be
            // compared with the target
            for (def compRow = row + 1; compRow < numStructRows; compRow++)
            {
                ...
                if (tgtStructName.equals(ccdd.getStructureTableNameByRow(compRow)))
                {
                    ...
                    def compVarPath = ccdd.getFullVariableNameRaw(compRow)
                    def compOffset = ccdd.getVariableOffset(compVarPath)
            ...
            if (packCount > 1)
            {
                ...
                for (def index = 0; index < packCount; index++)
                {
...
def outputStructureDefinition(structureName, isPacket, outFile)
{
    ...
    for (def rowIndex = 0; rowIndex < numStructRows; rowIndex++)
    {
            ...
            if (!isFound)
            {
                ...
                if (isVariable(variableName, arraySize))
                {
                    ...
                    if (!skipStringMembers)
                    {
                        ...
                        def itosEncode2Char = ccdd.getITOSEncodedDataType(dataType, "TWO_CHAR")
...
def extractMessageID(msgID)
{
    ...
    if (msgID ==~ /(0x)?[0-9a-fA-F]+/)
    {
        ...
        def val = Integer.parseInt(msgID.replace("0x", ""), 16)

        // Strip off all but the last 12 bits and convert the value to a
        // hexadecimal string
        retVal = Integer.toHexString(val & 0x7ff)
...
def outputTelemetryPacket(prefix, structureName, msgID, msgIDOffset)
{
    ...
    def msgIDWithOffset = Integer.parseInt(msgIDOffset.replace("0x", ""), 16) + Integer.parseInt(msgID.replace("0x", ""), 16)

    // Output the packet definition
    ccdd.writeToFileLn(tlmFile, "\nCfeTelemetryPacket " + prefix + structureName)
    ccdd.writeToFileLn(tlmFile, "{")
    ccdd.writeToFileLn(tlmFile, "  applyWhen={FieldInRange{field = applicationId, range = " + extractMessageID(msgIDWithOffset.toString(msgIDWithOffset, 16)) + "}},")
...
def outputStructures(structureNames)
{
    ...
    for (def structIndex = 0; structIndex < structureNames.length; structIndex++)
    {
        ...
        def msgID = ccdd.getTableDataFieldValue(structureName, "Message ID")
...
def outputCommands(prefix, msgIDOffset, system)
{
    ...
    for (def row = 0; row < numCommandRows; row++)
    {
        ...
        if (system == null || (commandSystem != null && system.equals(commandSystem)))
        {
            ...
            // table
            def commandName = ccdd.getCommandName(row)
            def cmdCode = ccdd.getCommandCode(row)
            def msgID = ccdd.getTableDataFieldValue(ccdd.getCommandTableNameByRow(row), "Message ID")
            def msgIDWithOffset = Integer.parseInt(msgIDOffset.replace("0x", ""), 16) + Integer.parseInt(msgID.replace("0x", ""), 16)
            ...
            ccdd.writeToFileLn(cmdFile, "{")
            ccdd.writeToFileLn(cmdFile, "  applicationId {range=" + extractCommandID(msgIDWithOffset.toString(msgIDWithOffset, 16)) + "}")
            ccdd.writeToFileLn(cmdFile, "  commandCode {range=" + Integer.parseInt(cmdCode.replace("0x", ""), 16) + "}")
            ...
            for (def argumentNum = 0; argumentNum < ccdd.getNumCommandArguments(row); argumentNum++)
            {
                ...
                if (name != null && !name.isEmpty() && dataType != null && !dataType.isEmpty())
                {
                    ...
                    // type
                    def itosEncode1Char = ccdd.getITOSEncodedDataType(dataType, "SINGLE_CHAR")
                        ...
                        if (sizeInBytes != 0)
                        {
                            ...
                            if (minimumValue == null || minimumValue.isEmpty())
                            {
                                ...
                                if (itosEncode1Char.equals("I"))
                                {
                                    ...
                                    minimumValue = -new BigInteger("2").pow(sizeInBytes * 8) / 2
                                }
                            }
                            ...
                            if (maximumValue == null || maximumValue.isEmpty())
                            {
                                ...
                                maximumValue = new BigInteger("2").pow(sizeInBytes * 8) - 1
                                ...
                                if (itosEncode1Char.equals("I"))
                                {
                                    ...
                                    maximumValue = new BigInteger(((maximumValue - 1) / 2).toString())
                                }
                            }
...
def outputMnemonicDefinition(row)
{
    ...
    if (itosEncode != null && !itosEncode.equals(dataType))
    {
        ...
        def isVar = isVariable(variableName, arraySize)

        // Check if the variable is a string
        def isString = itosEncode.equals("S") && !arraySize.isEmpty()

        // Set the output flag if this is a non-string variable
        def isOutputMnemonic = isVar && !isString
        ...
        if (isOutputMnemonic)
        {
            ...
            def enumerations = ccdd.getStructureEnumerations(row)
...
def outputPolynomial(prefix, variableName, coeffs)
{
    ...
    for (def index = 1; index < coeffs.length; index++)
    {
...
def outputPolynomialConversion(row, polynomialCoefficients)
{
    ...
    if (isVariable(variableName, arraySize))
    {
        ...
        def polySets = polynomialCoefficients.split("\\;")

        // Get the number of coefficient sets
        def numPolySets = polySets.length
        ...
        for (polyIndex = 0; polyIndex < numPolySets; polyIndex++)
        {
            ...
            def coeffs = ccdd.getArrayFromString(polySets[polyIndex], "|")
        ...
        if (numPolySets > 1)
        {
            ...
            def lastPolyindex = (polyIndex - 1)

            // Separate the polynomial coefficients into an array
            def coeffs = ccdd.getArrayFromString(polySets[lastPolyindex], "|")
...
else
{
    ...
    def msgIDSkip = ccdd.getGroupDataFieldValue("globals", "MID_delta")
    ...
    if (numFlightComputers > 1)
    {
        ...
        for (def fcIndex = 0; fcIndex < numFlightComputers; fcIndex++)
        {
            ...
            // skip values
            def nextMsgID = Integer.parseInt(fcOffsetVal.replace("0x", ""), 16) + Integer.parseInt(msgIDSkip.replace("0x", ""), 16)
            fcOffsetVal = "0x" + nextMsgID.toString(nextMsgID, 16)
        }
    }
    ...
    if (selected != null)
    {
        ...
        if (numStructRows > 0)
        {
            ...
            def tlmOutputFile = ccdd.getOutputPath() + systemName + "_" + endianExtn + ".rec"
        ...
        if (numCommandRows > 0)
        {
            ...
            for (def fcIndex = 0; fcIndex < numFlightComputers; fcIndex++)
            {
                def msgIDOffset = fcOffset[fcIndex]
                def prefix = fcNames[fcIndex]

                // Build the command output file name and open the command
                // output file
                def cmdFileName = ccdd.getOutputPath() + prefix + firstSystemName + "_CMD" + "_" + endianExtn + ".rec"
                ...
                if (cmdFile != null)
                {
                    ...
                    for (def cmdTblIndex = 0; cmdTblIndex < ccdd.getCommandTableNames().length; cmdTblIndex++)
                    {
                        ...
                        def systemName = ccdd.getTableDataFieldValue(ccdd.getCommandTableNames()[cmdTblIndex], "System")
