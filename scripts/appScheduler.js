 ...
 * 
 * This JavaScript script generates the scheduler application's (SCH) message
...
var columnWidth = [10, 6, 6, 6];

// Create the message definition table output file name
var mdtFileName = ccdd.getOutputPath() + "sch_def_msgtbl.c";

// Open the message definition table output file
var mdtFile = ccdd.openOutputFile(mdtFileName);
...
if (mdtFile != null)
{
    ...
    var mdtEntries = ccdd.getApplicationMessageDefinitionTable();
    ...
    var formatUsed = "    { {%-" + columnWidth[ENABLE_STATUS] + "s, %" + columnWidth[TYPE] + "s, %" + columnWidth[FREQUENCY] + "s, %" + columnWidth[REMAINDER] + "s} } \n";
    ...
    for (var name = 0; name < applicationNames.length; name++)
    {
        ...
        // files
        ccdd.writeToFileLn(mdtFile, "#include \"" + applicationNames[name].toLowerCase() + "_msids.h\"");
    }
    ...
    for (var row = 0; row < mdtEntries.length; row++)
    {
...
var sdtFileName = ccdd.getOutputPath() + "sch_def_schtbl.c";

// Create the schedule definition table ****************************************

// Open the schedule definition table output file
var sdtFile = ccdd.openOutputFile(sdtFileName);
...
if (sdtFile != null)
{
    ...
    var defines = ccdd.getApplicationScheduleDefinitionTableDefines();

    // Build the format for the defined parameters
    var formatDefines = "#define %-" + columnWidth[ENABLE_STATUS] + "s  %s \n";

    // Step through each defined parameter
    for (var define = 0; define < defines.length; define++)
    {
    ...
    for (var timeSlot = 0; timeSlot < ccdd.getNumberOfTimeSlots(); timeSlot++)
    {
        ccdd.writeToFileLn(sdtFile, "");
        ccdd.writeToFileFormat(sdtFile, "    /* Slot #%s */\n", (timeSlot + 1).toString());

        // Get the schedule definition table entries
        var sdtEntries = ccdd.getApplicationScheduleDefinitionTable(timeSlot);
        ...
        var formatBody = "    {%-" + columnWidth[ENABLE_STATUS] + "s, %" + columnWidth[TYPE] + "s, %" + columnWidth[FREQUENCY] + "s, %" + columnWidth[REMAINDER] + "s, %" + columnWidth[MESSAGE_INDEX] + "s, %-" + columnWidth[GROUP_DATA] + "s}%s\n";

        // Step through each schedule definition table entry
        for (var row = 0; row < sdtEntries.length; row++)
        {
            var comma = ",";
