 ...
 * 
 * This JavaScript script generates an ITOS record file from the supplied table
 ...
 *****************************************************************************/
function outputStructureDefinition(structureName, isPacket)
{
    var termLine = false;
        ...
        if (ccdd.getStructureTableNameByRow(row).equals(structureName))
        {
            ...
            if (!isFound)
            {
                ...
                if (isVariable(variableName, arraySize))
                {
                    ...
                    var itosEncode = ccdd.getITOSEncodedDataType(ccdd.getStructureTableData("data type", row), endianess);
 ...
 *****************************************************************************/
function outputTelemetryPacket(structureNames)
{
 ...
 *            array of all structure table names
 *****************************************************************************/
function outputPrototypeStructures(structureNames)
{
...
function outputCommands()
{
    ...
    for (var row = 0; row < ccdd.getCommandTableNumRows(); row++)
    {
        ...
        while (name != null)
        {
            ...
            var argument = "arg " + argumentNum + " ";
        
            // Get the command argument's name, data type, and enumeration values
            name = ccdd.getCommandTableData(argument + "name", row);
            var dataType = ccdd.getCommandTableData(argument + "data type", row);
            var enumeration = ccdd.getCommandTableData(argument + "enumeration", row);
            ...
            {
                // Get the ITOS encoded form of the data type
                var itosEncode = ccdd.getITOSEncodedDataType(dataType, endianess);

                // Get the single character ITOS encoded form of the data type
                var itosEncodeChar = ccdd.getITOSEncodedDataType(dataType, "SINGLE_CHAR");
                ...
                if (itosEncodeChar.equals("I") || itosEncodeChar.equals("U"))
                {
                    ...
                    var minimumValue = ccdd.getCommandTableData(argument + "minimum", row);
                    var maximumValue = ccdd.getCommandTableData(argument + "maximum", row);
                    ...
                    if (sizeInBytes != 0)
                    {
                        ...
                        if (maximumValue == null || maximumValue.isEmpty())
                        {
                            ...
                            var maximumValue = Math.pow(2, sizeInBytes * 8);
...
{
    // Get the single character ITOS encoded form of the data type
    var itosEncode = ccdd.getITOSEncodedDataType(ccdd.getStructureTableData("data type", row), "SINGLE_CHAR");
    ...
    if (itosEncode != null)
    {
        ...
        if (isVariable(variableName, arraySize))
        {
            ...
            var structurePath = ccdd.getStructureTableITOSPathByRow(row);
                        
            // In case this is an array member replace the square brackets
            variableName = convertArrayMember(variableName);

            // Get the variable's structure name
            var structureName = ccdd.getStructureTableNameByRow(row);
            ...
            var enumeration = ccdd.getStructureTableData("enumeration", row);
...
function outputTelemetryDiscreteConversions()
{
    ...
    for (var row = 0; row < ccdd.getStructureTableNumRows(); row++)
    {
        ...
        var discreteConversion = ccdd.getStructureTableData("enumeration", row);
...
function outputCommandDiscreteConversions()
{
    ...
    for (var row = 0; row < ccdd.getCommandTableNumRows(); row++)
    {
        ...
        while (discreteConversion != null)
        {
            ...
            // for this command
            var discreteConversion = ccdd.getCommandTableData("arg " + argumentNum + " enumeration", row);
            ...
            if (discreteConversion != null && !discreteConversion.isEmpty())
            {
                ...
                // argument names
                var fullCommandName = ccdd.getCommandTableData("command name", row) 
                                      + "_"
                                      + ccdd.getCommandTableData("arg " + argumentNum + " name", row);
 ...
 *            conversion name
 *****************************************************************************/
function outputEnumeration(enumeration, enumerationName)
{
    ...
    for (var row = 0; row < ccdd.getCommandTableNumRows(); row++)
    {
        ...
        while (enumeration != null)
        {
            ...
            if (enumeration != null && !enumeration.isEmpty())
            {
                ...
                // argument names
                var enumerationName = getCommandEnumerationName(row, argumentNum);
 ...
 *****************************************************************************/
function outputLimitDefinition(row, isFirst)
{
 ...
 *****************************************************************************/
function outputPolynomialConversion(row, isFirst)
{
    ...
    if (polynomialCoefficients != null && !polynomialCoefficients.isEmpty())
    {
        ...
        if (isVariable(variableName, arraySize))
        {
            ...
            var coeffs = ccdd.getArrayFromString(polynomialCoefficients, "|");
            ...
            for (var index = 0; index < coeffs.length; index++)
            {
                // Check if this is not the first pass
                if (addComma)
                {
...
else
{
    ...
    if (selected != null)
    {
        ...
        {
            var endianess = "LITTLE_ENDIAN_SWAP";
            var endian_ext = "LE";
        }
        ...
        if (numStructRows > 0)
        {
            ...
            if (tlmFile != null)
            {
                ...
                var structureNames = ccdd.getStructureTableNames();
